...
%   1) handle to the color bar
    ...
    parse(P, varargin{:});
    P = P.Results;
    Specs = M23a.specify_flows;
    switch component
        case 'intrinsic'
            magnitude = log(M23a.intrinsic_flow_speed(hypothesis, 'Delta_scale', P.Delta_scale));
        case 'input'
            magnitude = log(M23a.input_flow_speed(hypothesis, 'Delta_scale', P.Delta_scale));
        case 'difference'
            magnitude = tanh(M23a.flow_speed_difference(hypothesis, 'Delta_scale', P.Delta_scale));
    end
    axes(P.axes)
    M23a.stylizeaxes    
