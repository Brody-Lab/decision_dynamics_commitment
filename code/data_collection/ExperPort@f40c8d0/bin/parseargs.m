...
if numel(arguments)==1 && isstruct(arguments{1})
    ...
    %% AGB: added checks of "argument" so that sensible errors/warnings can be thrown
    if ~iscell(arguments)
        warning('Unrecognized type: first input to parseargs must be a cell array or scalar struct.'); % allow this to only throw an error because code did not explicitly forbid this even though it is not accepted according to the documentation - AGb, 9/2020
    end
    arg = 1;  while arg <= length(arguments),
        if (length(arguments{arg}) ~= 1) && (~ischar(arguments{arg}))
            if isempty(arguments{arg})
                error('parseargs fatal error: Element %g of argument list is empty!',arg);
            else
                error('parseargs fatal error: Element %g of argument list is non-scalar and not a character vector!',arg);
            end
        end
        %%
