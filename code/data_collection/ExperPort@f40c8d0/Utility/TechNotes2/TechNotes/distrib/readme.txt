    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_TechNotes.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: TechNotes.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * TechNotes.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * TechNotes.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * TechNotes (application created by MATLAB Compiler)
        * run_TechNotes.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * TechNotes.dll (shared library, file extension varies by 
          platform)
        * TechNotes.h (library header file)
    ...
    Windows:
        * TechNotes.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * TechNotes.xml (documentation files)
        * TechNotes.pdb (program database file - if DEBUG option 
          selected)
        * TechNotes.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * TechNotes.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * TechNotes_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * TechNotes.jar (Java class files and compiled M-code)
    ...
    Windows:
        * TechNotes.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * TechNotes_1_0.dll (COM component)
        * TechNotes.xla (optionally created and placed in
          C:\ratter\ExperPort\Utility\TechNotes2\TechNotes\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
