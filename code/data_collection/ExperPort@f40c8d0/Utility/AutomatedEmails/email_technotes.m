...
try
    ...
    %setpref('Internet','E_mail',['TechNotesMeister',datestr(now,'yymm'),'@Princeton.EDU']);
    set_email_sender
    ...
    if ~isempty(TN)
        ...
        EE = bdata('SELECT DISTINCT email FROM ratinfo.contacts WHERE tech_afternoon=1 and is_alumni=0');
        EC = bdata('SELECT DISTINCT email FROM ratinfo.contacts WHERE tech_computer=1 and is_alumni=0');
        EM = bdata('SELECT DISTINCT email FROM ratinfo.contacts WHERE tech_morning=1 and is_alumni=0');
        EA = bdata('SELECT DISTINCT email FROM ratinfo.contacts WHERE subscribe_all=1 and is_alumni=0');
