    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_ShiftChecker.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: ShiftChecker.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * ShiftChecker.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * ShiftChecker.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * ShiftChecker (application created by MATLAB Compiler)
        * run_ShiftChecker.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * ShiftChecker.dll (shared library, file extension varies by 
          platform)
        * ShiftChecker.h (library header file)
    ...
    Windows:
        * ShiftChecker.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * ShiftChecker.xml (documentation files)
        * ShiftChecker.pdb (program database file - if DEBUG option 
          selected)
        * ShiftChecker.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * ShiftChecker.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * ShiftChecker_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * ShiftChecker.jar (Java class files and compiled M-code)
    ...
    Windows:
        * ShiftChecker.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * ShiftChecker_1_0.dll (COM component)
        * ShiftChecker.xla (optionally created and placed in
          C:\ratter\ExperPort\Utility\AutomatedEmails\ShiftChecker\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
