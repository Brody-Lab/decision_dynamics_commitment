    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_TrainingRoomStatus.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: TrainingRoomStatus.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * TrainingRoomStatus.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * TrainingRoomStatus.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * TrainingRoomStatus (application created by MATLAB Compiler)
        * run_TrainingRoomStatus.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * TrainingRoomStatus.dll (shared library, file extension varies by 
          platform)
        * TrainingRoomStatus.h (library header file)
    ...
    Windows:
        * TrainingRoomStatus.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * TrainingRoomStatus.xml (documentation files)
        * TrainingRoomStatus.pdb (program database file - if DEBUG option 
          selected)
        * TrainingRoomStatus.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * TrainingRoomStatus.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * TrainingRoomStatus_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * TrainingRoomStatus.jar (Java class files and compiled M-code)
    ...
    Windows:
        * TrainingRoomStatus.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * TrainingRoomStatus_1_0.dll (COM component)
        * TrainingRoomStatus.xla (optionally created and placed in
          C:\ratter\ExperPort\Utility\AutomatedEmails\TrainingRoomStatus\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
