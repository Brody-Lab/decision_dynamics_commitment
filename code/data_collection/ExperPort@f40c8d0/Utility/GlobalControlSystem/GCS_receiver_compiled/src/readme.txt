    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_GCS_receiver.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: GCS_receiver.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * GCS_receiver.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * GCS_receiver.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * GCS_receiver (application created by MATLAB Compiler)
        * run_GCS_receiver.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * GCS_receiver.dll (shared library, file extension varies by 
          platform)
        * GCS_receiver.h (library header file)
    ...
    Windows:
        * GCS_receiver.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * GCS_receiver.xml (documentation files)
        * GCS_receiver.pdb (program database file - if DEBUG option 
          selected)
        * GCS_receiver.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * GCS_receiver.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * GCS_receiver_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * GCS_receiver.jar (Java class files and compiled M-code)
    ...
    Windows:
        * GCS_receiver.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * GCS_receiver_1_0.dll (COM component)
        * GCS_receiver.xla (optionally created and placed in
          C:\ratter\ExperPort\Utility\GlobalControlSystem\GCS_receiver_compiled\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
