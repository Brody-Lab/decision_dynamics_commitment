    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_MouseMassMeister.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: MouseMassMeister.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * MouseMassMeister.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * MouseMassMeister.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * MouseMassMeister (application created by MATLAB Compiler)
        * run_MouseMassMeister.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * MouseMassMeister.dll (shared library, file extension varies by 
          platform)
        * MouseMassMeister.h (library header file)
    ...
    Windows:
        * MouseMassMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * MouseMassMeister.xml (documentation files)
        * MouseMassMeister.pdb (program database file - if DEBUG option 
          selected)
        * MouseMassMeister.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * MouseMassMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * MouseMassMeister_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * MouseMassMeister.jar (Java class files and compiled M-code)
    ...
    Windows:
        * MouseMassMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * MouseMassMeister_1_0.dll (COM component)
        * MouseMassMeister.xla (optionally created and placed in
          C:\ratter\ExperPort\Utility\MouseMassMeister\MouseMassMeister\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
