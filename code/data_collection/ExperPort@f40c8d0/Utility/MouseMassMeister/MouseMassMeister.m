function varargout = MouseMassMeister(varargin)
% MouseMassMeister M-file for MouseMassMeister.fig
%      MouseMassMeister, by itself, creates a new MouseMassMeister or raises the existing
%      singleton*.
%
%      H = MouseMassMeister returns the handle to a new MouseMassMeister or the handle to
%      the existing singleton*.
%
%      MouseMassMeister('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MouseMassMeister.M with the given input arguments.
%
%      MouseMassMeister('Property','Value',...) creates a new MouseMassMeister or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before MouseMassMeister_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to MouseMassMeister_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help MouseMassMeister
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @MouseMassMeister_OpeningFcn, ...
                   'gui_OutputFcn',  @MouseMassMeister_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before MouseMassMeister is made visible.
function MouseMassMeister_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<INUSL>
handles.output = hObject;

set(gcf,'name','MouseMassMesiter V3.1');
...
function varargout = MouseMassMeister_OutputFcn(hObject, eventdata, handles)  %#ok<INUSL>
...
if get(handles.start_toggle,'value') == 0
    ...
    while get(handles.start_toggle,'value') == 1
        ...
        if m > handles.minmass
            ...
            if foundstable == 0
                ...
                if length(M) < handles.numreads
                    ...
                    if score < handles.threshold / 100
                        ...
                        %Tell the user to remove the mouse
                        rignum=0;
                        try %#ok<TRYNC>
                        ...
                        if isempty(rignum) || rignum == 0
                            set(handles.status_text,'string','Weighing Complete. Return mouse to cage.',...
                                'backgroundcolor',[0 1 0]);
                        elseif weight < handles.smallrat
                            set(handles.status_text,'string',['Weighing Complete. Mouse is too small to train.',...
                                ' Return mouse to cage. Log a TechNote and flag as RECOVERY.'],...
                                'backgroundcolor',[0 1 0]);
                        else
                            try
            ...
            if foundstable == 1
                %The mouse is off the scale, jump to the next
                
                %We're only going to check if the weight is similar for
                %mice not scanned with RFID chips.
                
                
                %if the weight is similar to the last weight entry, let's
                %move on to the next unweighed mouse, if not we should prompt
                %the user to reweight this mouse
                ...
                %Free water mice have 0g entered in the mass log. Let's
                ...
                if ~strcmp(ratname,RFID_ratname)
                    if numel(oldmass) < 2 ||...
                       ...
                       doing_reweigh == 1
                        ...
                        %or this is our second attempt at weighing this mouse, 
                        %either way, let's move on 

                        if numel(oldmass) > 1 && doing_reweigh == 1 && ((oldmass(2)-weight) / oldmass(2)) > handles.error / 100
                            %This is our second weighing and the mouse has gone
                            %down by a significant amount 5%.  Instruct the tech
                            %to place the mouse on free water and contact the
                            %experimenter.

                            %Run this code to alert the tech to put the mouse on
                            %free water and stop mass meister from running

                            %set(handles.status_text,'string','Put mouse on free water. Call experimenter.',...
                            %    'backgroundcolor',[1 0 0]);
                            %set(handles.start_toggle,'value',0,'String','Start','backgroundcolor',[0 1 0]);
                            %return

                            %Run this code to send a text alert to the
                            %experimenter that the mouse has shown a >5% weight
                        ...
                        %Weight is out of range, let's reweigh this mouse once
                    ...
                    %This was a RFID scanned mouse, if weight dropped a lot
...
set(handles.status_text,'string','Please enter the mouse''s mass then click Enter',...
    'backgroundcolor',[1 1 1]);
...
try
    %If this mouse has an entry for this day we need to delete it
...
%Executes when the user is done entering the weight for the mouse manually.
