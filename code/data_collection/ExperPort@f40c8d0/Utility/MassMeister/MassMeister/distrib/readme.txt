    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_MassMeister.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: MassMeister.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * MassMeister.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * MassMeister.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * MassMeister (application created by MATLAB Compiler)
        * run_MassMeister.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * MassMeister.dll (shared library, file extension varies by 
          platform)
        * MassMeister.h (library header file)
    ...
    Windows:
        * MassMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * MassMeister.xml (documentation files)
        * MassMeister.pdb (program database file - if DEBUG option 
          selected)
        * MassMeister.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * MassMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * MassMeister_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * MassMeister.jar (Java class files and compiled M-code)
    ...
    Windows:
        * MassMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * MassMeister_1_0.dll (COM component)
        * MassMeister.xla (optionally created and placed in
          C:\ratter\ExperPort\Utility\MassMeister\MassMeister\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
