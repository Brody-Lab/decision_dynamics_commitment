    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_WaterMeister.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: WaterMeister.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * WaterMeister.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * WaterMeister.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * WaterMeister (application created by MATLAB Compiler)
        * run_WaterMeister.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * WaterMeister.dll (shared library, file extension varies by 
          platform)
        * WaterMeister.h (library header file)
    ...
    Windows:
        * WaterMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * WaterMeister.xml (documentation files)
        * WaterMeister.pdb (program database file - if DEBUG option 
          selected)
        * WaterMeister.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * WaterMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * WaterMeister_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * WaterMeister.jar (Java class files and compiled M-code)
    ...
    Windows:
        * WaterMeister.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * WaterMeister_1_0.dll (COM component)
        * WaterMeister.xla (optionally created and placed in
          C:\ratter\ExperPort\Utility\WaterMeister\WaterMeister\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
