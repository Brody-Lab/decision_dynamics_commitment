...
% protocol.
%
% The poissonbupswt plugin exists separately from poissonbups2 (from which
% it is derived) because PBupsWT requires two sounds to be prepared at all
% times--the one for this trial and the one for the next trial. That is
% because in PBupsWT, the prep next trial state happens at the beginning of
% each trial to insure that new trials can start quickly as soon as the rat
% stops waiting for reward and starts a new trial. If we prepare the next
% trial's sound at that moment, we overwrite the current trial's sound,
% breaking the correspondence between the SMA and the click train.
%
% PARAMETERS:
% -----------
%
...
% JTB, July, 2021
%       - incorporating all the imrpovements made by TZL and AGB since 2018
...
%% init    
  case 'init'
    ...
    % added by JTB 
    	ToggleParam(obj, 'prep_before', 0, x, y, 'position', [x+200 y 100 20], ...
		'OffString', 'Prep After', ...
		'OnString',  'Prep Before', ...
		'TooltipString', ['Use Prep After when prep next trial occurs at trial end, or anytime after playing current stimulus. '...
        'Use Prep Before when prep next trial occurs at trial start before playing this trial''s clicks. Otherwise, bups are overwritten.']);   
	next_row(y, 1.3);
...
%% set prep before (added by JTB)
    case 'set_prep_before'
        prep_before.value = 1;
...
%% make_sounds
  case 'make_this_sound'
      % added by JTB 
      if ~value(prep_before)
          soundToUpdate = 'PBupsSound';
      else
