...
%% get_gen_state  
  case 'get_gen_state',
    x = value(ThisGenState); %#ok<NODEF>
    return;
    
%% DYN OK    
%% get_all_gen_State
  case 'get_all_gen_state'
    x = get_history(ThisGenState); %#ok<NODEF>
    return;
...
%% next_trial
  case 'next_trial'  
      ...
      % hold_dur is the amount of time in seconds that we hold the specified next side choice in that generative state. if not given, we do not hold for any time.
      if isempty(hold_dur),
            hold_dur = -1;
      end
...
%% push_history
  case 'push_history'
    % We need to push more things. measured_hazard_rate?
      push_history(ThisGenState);
      push_histroy(ThisCorrectAnswer);
...
%% make_sounds
  case 'make_this_sound',
    ...
    % Need to do surgery here to make clicks with generative state
...
% I think this is the volume level of clicks, we can ignore
    ...
    % CHECK if PBupsSound is the sound of a single click, if so, we can ignore this
    ...
    % Im not sure if we need this, or if we need to change it
	ThisLeftRate.value = lrate;
	ThisRightRate.value = rrate;

%	bpt.gamma = value(ThisGamma); % I think we can delete this because gammas are not constant
    bpt.genState = value(ThisGenState);
    bpt.correctAnswer = value(ThisCorrectAnswer);
	bpt.left = data.left;
