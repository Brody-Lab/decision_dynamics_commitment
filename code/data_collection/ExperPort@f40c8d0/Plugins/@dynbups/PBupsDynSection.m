% Plugin for dynamic pbups
% Alex Piet, Ahmed El Hady, august 2015
% Modified from Bing
%
% [x, y] = PBupsDynSection(obj, action, tname, varargin)
%
% This plugin makes a window that manages the making of dynamic Poisson bups. The system has two states
% S1 and S2, which each have their own left and right click rates. The hazard rate dictates how we switch between
% states. Within a state, the poisson clicks are generated as independent poisson processes with their given click rates.
% 
% One complication is that the trials need to be generated backwards so we can control when we get a left or right trial. 
%
% PARAMETERS:
% -----------
%
...
% Alex Piet, modified for dynamic clicks task
%
% I added this variable list:
% gen_state, hazard_rate, measured_hazard_rate, ThisCorrectAnswer, ThisGenState


function [x, y] = PBupsDynSection(obj, action, varargin)
...
%% init    
  case 'init'
    ...
    next_row(y, 1.5);
	% % % % WE MAY WANT TO INCLUDE A TESTING SECTION FOR THE HAZARD RATE,
	% ETC.
	...
	next_row(y, 1.5);
    % % % This section computes the need (or lack there of) of anti-bias
    ...
    % % % We likely will want to add a section for our estimate of the rats
    % hazard rate
    ...
    NumeditParam(obj, 'ThisGenEnd', 1, x, y, 'position', [x+450 y 150 20], ...
        'TooltipString', 'What state the Generative variable ends in (1 = right, 0 = left)');
    NumeditParam(obj, 'ThisHazard', 2, x, y, 'position', [x+600 y 150 20], ...
        'TooltipString', 'Hazard Rate for state switches (Hz)');
    NumeditParam(obj, 'ThisCorrectAnswer', 1, x, y, 'position', [x+750 y 150 20], ...
        'TooltipString', 'Correct Answer for the trial 0 =left, 1 =right');
    NumeditParam(obj, 'ThisHazardBarrier', 0, x, y, 'position', [x+900 y 150 20], ...
        'TooltipString', '# of Seconds before end of trial when no state transitions can happen');
    
	next_row(y, 1);
    
    
	HeaderParam(obj, 'panel_title', 'Dynamic PBups plugin, v.0', x, y, 'position', [x y 700 20]);
    ...
    % % % WE WILL NEED TO INCLUDE SOMETHING LIKE THIS
    %SoloParamHandle(obj, 'ThisTrialAnswer','value', {});
    %SoloParamHandle(obj, 'hazardRate','value', {});
...
%% get_gen_state  
  case 'get_gen_state',
    x = value(ThisGenEnd); %#ok<NODEF>
    return;
    
%% DYN PROBABLY NO WORK
%% get_all_gen_State
  case 'get_all_gen_state'
    x = get_history(ThisGenEND); %#ok<NODEF>
    return;
   
%% DYN OK
%% get_correct_answer  
  case 'get_correct_answer',
    x = value(ThisCorrectAnswer); %#ok<NODEF>
    return;
    
%% DYN PROBABLY NO WORK    
%% get_all_correct_answer
  case 'get_all_correct_answer'
    x = get_history(ThisCorrectAnswer); %#ok<NODEF>
    return;
               
%% DYN, maybe OK, added hazardRate, mHazardRate   
%% get
  case 'get'
     switch varargin{1},
         case 'hazard',
             x = value(ThisHazard);  
         case 'mHazardRate',% doesnt work
             x = measured_hazard_rate;
         case 'nstimuli',
...
%% next_trial
  case 'next_trial'  
	  ...
	  side = varargin{1};
	  if isempty(side), % if we are not given which side the next trial is, we pick randomly
		  if rand(1) < 0.5,     side = 'l';
		  else                  side = 'r';
		  end;
	  end;
	  ...
	  LocalPrevSounds.value = varargin{3};
        ...
        % DYN, we need to modify all of these  
      ...
      % this code block generates the trial gammas, we need it to generate the generative state. 
	  if side == 'l',
      ...
      feval(mfilename, obj, 'make_this_sound'); % This function makes the clicks, given a fixed gamma, we need it to make clicks with changing gammas. 
	  feval(mfilename, obj, 'push_history'); %NOT sure what I need to modify here
      x = value(ThisCorrectAnswer);
      % break
%% push_history
  case 'push_history'
    % We need to push more things. measured_hazard_rate?
      % All redundant to ThisBupTimes
      %push_history(ThisGenEnd); % NOT SURE WHAT THIS DOES
      %push_history(ThisGamma); %alex add
      %push_history(ThisHazard); % NOT SURE WHAT THIS DOES
      %push_histroy(ThisCorrectAnswer); %alex add ??
...
%% make_sounds
  case 'make_this_sound',
	...
	% the sound made is as long as T
	[snd lrate rrate data genSwitchTimes genState corResp R] = make_dyn_pbup(value(total_rate), value(ThisGamma), srate, value(T),value(ThisHazardBarrier), value(ThisHazard),value(ThisGenEnd), ...
    ...
    % This is the volume level of clicks
    ...
    % Might be a good idea to change this to rate1, rate2. 
	ThisLeftRate.value = lrate;
	ThisRightRate.value = rrate;

    % Save Variables
	bpt.hazard = value(ThisHazard);
    bpt.hazardBarrier = value(ThisHazardBarrier);
    bpt.gamma = value(ThisGamma); 
    bpt.genSwitchTimes = genSwitchTimes;
    bpt.genEndState = value(ThisGenEnd);
    %bpt.genState = genState; % Will get rid of eventually. Useful for debugging
    bpt.evidenceRatio = R;
	bpt.left = data.left;
	bpt.right = data.right;
    bpt.correctAnswer = corResp;
    bpt.T = value(T);
    bpt.rate_1 = rrate;
    bpt.rate_2 = lrate;
	ThisBupTimes.value = bpt;
    ThisCorrectAnswer.value = double(corResp(end));


%% update_measured_hazard_rate
%% need to implement

%% Dyn, I think we can leave this as is
%% update_hitfrac
  case 'update_hitfrac',
...
%% DYN, I think we also dont need to change this
%% update_biashitfrac
...
%% We can get rid of this when we implement test_gen_state      
%% test_gamma
%   case 'test_gamma'
% 	srate = SoundManagerSection(obj, 'get_sample_rate');
%     [snd lrate rrate] = make_dyn_pbup(value(total_rate), value(test_gamma), srate, value(sduration), ...
%                      'bup_width', value(bup_width), 'first_bup_stereo', value(first_bup_stereo), ...
%                      'crosstalk', value(crosstalk), 'base_freq', value(base_freq), ...
...
% 	
% %% Change this so we have dynamic state
% %% test_play
%   case 'test_play',
%     srate = SoundManagerSection(obj, 'get_sample_rate');
%     [snd] = make_dyn_pbup(value(total_rate), value(test_gamma), srate, value(sduration), ...
%                      'bup_width', value(bup_width), 'first_bup_stereo', value(first_bup_stereo), ...
%                      'crosstalk', value(crosstalk), 'base_freq', value(base_freq), ...
...
%% DYN, we shouldnt need to change this    
%% T_resample
  case 'T_resample'
...
%% We should simplify this because we only have two gammas, make sure it returns gammas that reflect structure of task
%% gammas
  case 'gammas'
...
%% I dont know what this does. Something about displaying information, we probably need to update it
%% gamma_style
  case 'gamma_style'
...
%% DYN, WE DID NOT CHANGE ANYTHING. IF YOU ARE DOING OPTO, THINK CLEARLY
%% check_stim_channels
...
%% dyn,  I think this is display stuff, and we dont need to change it
%% hide, show_hide
  case 'hide',
    pbup_show.value = 0;
    feval(mfilename, obj, 'show_hide');
