...
% DG, June 2018
% added an option to generate trials with a given cick difference from a variety of gammas 
...
%% init    
  case 'init'
    ...
    SoloParamHandle(obj, 'pbup_fig', ...
        'value', double(figure('Position', [200 50 600 800], ...
	...
	NumeditParam(obj, 'T_min', 0.2, x, y, 'position', [x+100 y 100 20], ...
		'TooltipString', 'the minimum delay before stim start');
	NumeditParam(obj, 'T_max', 0.2, x, y, 'position', [x+200 y 100 20], ...
		'TooltipString', 'the maximum delay before stim start duration');
    NumeditParam(obj, 'tau', 0.2, x, y, 'position', [x+300 y 100 20], ...
        'TooltipString', 'mean of the exponential delay of the delay used to set the T');

	PushbuttonParam(obj, 'T_resample', x, y, 'position', [x+400 y 100 20], ...
		'label', 'Resample', ...
    ...
    next_row(y);
    ToggleParam(obj, 'delclickstask', 0, x, y, 'position', [x y 200 20], ...
        'OffString', 'Normal task', ...
        'OnString',  'Trials with fixed delta clicks', ...
        'TooltipString', 'this when on produces trials which have prespecified delta clicks');
    set_callback(delclickstask, {mfilename, 'delclickstask'});
    set_callback_on_load(delclickstask, 1);
    NumeditParam(obj, 'delta_clicks', 20, x, y, 'position', [x+200 y 150 20], ...
        'labelfraction', 0.4, ...
        'TooltipString', 'prespecified delta click values, must be positive');
    NumeditParam(obj, 'delclicks_prob', 1, x, y, 'position', [x+350 y 150 20], ...
        'labelfraction', 0.5, ...
        'TooltipString', 'fraction of trials with prespecified delta clicks');
    PushbuttonParam(obj, 'normalize_delprobs', x, y, 'position', [x+500 y 100 20], ...
        'label', 'normalize', ...
        'TooltipString', 'Normalize delclicks_prob');
    set_callback(normalize_delprobs, {mfilename, 'normalize_delprobs'});
    ...
    ToggleParam(obj, 'first_bup_stereo', 1, x, y, 'position', [x y 150 20], ...
    ...
    ToggleParam(obj, 'task_type', 1, x, y, 'position', [x+150 y 150 20], ...
	...
	NumeditParam(obj, 'total_rate', 40, x, y, 'position', [x+300 y 100 20], ...
        'TooltipString', 'the sum of left and right bup rates');
	
	NumeditParam(obj, 'crosstalk', 0, x, y, 'position', [x+400 y 100 20], ...
		'TooltipString', 'if >0, then is the amount the left clicks leak into the right channel, and vice versa.');    
    NumeditParam(obj, 'vol', 0.5, x, y, 'position', [x+500 y 100 20], ...
        'labelfraction', 0.3, ...
    ...
    DispParam(obj, 'ThisDeltaClick', 20, x, y, 'position', [x+450 y 150 20], ...
        'labelfraction', 0.6, ...
        'TooltipString', 'the delta click for this trial');
	next_row(y, 1);
	HeaderParam(obj, 'panel_title', 'PBups plugin, v.5', x, y, 'position', [x y 600 20]);
...
%% get delclicks value for the current trial
  case 'get_delclicks'
    x = value(ThisDeltaClick);

%% get what the value of delclicks task is currently
  case 'get_delclickstask'
    x = value(delclickstask);
...
%% next_trial
  case 'next_trial'  
      ...
      if delclickstask == 1
        T.value = 10;
         % normalize and pick delta clicks for the current trial  
        
        feval(mfilename, obj, 'normalize_delprobs');
        tempdelclick = randsample(1:length(delta_clicks),1 , true, value(delclicks_prob));  
        ThisDeltaClick.value = delta_clicks(tempdelclick);
    ...
    if delclickstask == 1
	   ...
	   while (flag == 0  & counter < 10)
	       ...
	       [snd lrate rrate data] = make_pbup(value(total_rate), value(ThisGamma), srate, max(1,value(T)), ...
                                       ...
                                       'ntones', value(ntones), 'bup_ramp', value(bup_ramp), 'generate_sound', 0, 'task_type',value(task_type)); %#ok<NODEF>

            % checking if the the delta clicks is within the permissible range
            bups = [data.left'; data.right'];
            ids = [-1*ones(length(data.left),1); ones(length(data.right),1)];
            [sortbups, sortid] = sort(bups);
            newids = ids(sortid);
            delta = cumsum(newids);
            a = find((delta == (value(ThisDeltaClick)) | delta == (-value(ThisDeltaClick))), 1);

            if isempty(a)
                flag = 0;
            elseif sortbups(a(1)) <= value(T_max) & sortbups(a(1)) >= value(T_min)
                flag =1;
                T.value = sortbups(a(1));

                data.left = data.left(data.left < value(T));
                data.right = data.right(data.right < value(T));
                data.lrate = lrate;
                data.rrate = rrate;
                delta(a(1))
                [snd lrate rrate data] = make_pbup(value(total_rate), value(ThisGamma), srate, max(1,value(T)), ...
                                       'bup_width', value(bup_width), 'first_bup_stereo', value(first_bup_stereo), ...
                                       'crosstalk', value(crosstalk), 'base_freq', value(base_freq), 'fixed_sound', data, ...
                                       'ntones', value(ntones), 'bup_ramp', value(bup_ramp), 'generate_sound', 1, 'task_type',value(task_type)); %#ok<NODEF>      
        ...
        if (counter >=10 & flag == 0)
            ...
            feval(mfilename, obj, 'next_trial', side, previous_sides, previous_sounds);   
        end
        ...
        [snd lrate rrate data] = make_pbup(value(total_rate), value(ThisGamma), srate, max(1,value(T)), ...
                                       'bup_width', value(bup_width), 'first_bup_stereo', value(first_bup_stereo), ...
                                       'crosstalk', value(crosstalk), 'base_freq', value(base_freq), ...
                                       'ntones', value(ntones), 'bup_ramp', value(bup_ramp), 'task_type',value(task_type)); %#ok<NODEF>
        ...
        % setting the value of ThisDeltaClick field
        leftbups = data.left(data.left < value(T));
        rightbups = data.right(data.right < value(T));
        ThisDeltaClick.value = abs(length(rightbups) - length(leftbups));
...
%% test_gamma
  case 'test_gamma'
    ...
    [snd lrate rrate] = make_pbup(value(total_rate), value(test_gamma), srate, value(sduration), ...
                     'bup_width', value(bup_width), 'first_bup_stereo', value(first_bup_stereo), ...
                     'crosstalk', value(crosstalk), 'base_freq', value(base_freq), ...
                     'ntones', value(ntones), 'bup_ramp', value(bup_ramp), 'generate_sound', 0, 'task_type',value(task_type));
...
%% test_play
  case 'test_play',
    ...
    [snd] = make_pbup(value(total_rate), value(test_gamma), srate, value(sduration), ...
                     'bup_width', value(bup_width), 'first_bup_stereo', value(first_bup_stereo), ...
                     'crosstalk', value(crosstalk), 'base_freq', value(base_freq), ...
                     'ntones', value(ntones), 'bup_ramp', value(bup_ramp), 'task_type',value(task_type)); 
...
%% T_resample
  case 'T_resample'
	...
	if rand(1) < p_probe,
        ...
        % This is all really weird and relies on NIC being set at 1.5 additionally
        % it picks the delay before stim start from an exponeential distribution
        % and then subtracts that value from 1.5 to set T for the current trial
        T.value = 1.5 - (value(T_min) - value(tau)*log(1 - rand(1)*(1 - exp(-(value(T_max) - value(T_min))/value(tau)))));
		% T.value = value(T_min)+rand(1)*(T_max-T_min);     if sampling from uniform distribution
	end;
...
%% normalize_pprobs
  case 'normalize_pprobs',
    ...
    %% normalize_delprobs
  case 'normalize_delprobs',
    p = value(delclicks_prob);
    p = p/sum(p);
    delclicks_prob.value = p;
...
%% delclickstask
  case 'delclickstask'
    if delclickstask == 1, % if we are doing fixed delta clicks trials
        enable(delta_clicks);
        enable(delclicks_prob);
        enable(normalize_delprobs);
        disable(T_gap);
        feval(mfilename, obj, 'normalize_delprobs');
    elseif delclickstask == 0,                 % if we are doing the normal task
        disable(delta_clicks);
        disable(delclicks_prob);
        disable(normalize_delprobs);
        enable(T_gap);
    end;
