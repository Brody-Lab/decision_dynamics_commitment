...
if double(file_path(end-4)) > 96
    %This ASV folder had a letter
    u2 = dir([file_path(1:end-5),'*.mat']);
else
...
if isempty(datafile_name)
    if double(file_path(end-4)) > 96
        %This ASV folder had a letter
        newfile = [file_path(1:end-5),ltr(nextltr),'.mat'];
    else
        newfile = [file_path(1:end-4),ltr(nextltr),'.mat'];
    end
else
    newfile = datafile_name;
end
save(newfile,'saved','saved_history','saved_autoset','fig_position');
disp(['Data saved to file: ',newfile]);

%This isn't exactly the same as the settings file generated by the saveload
%plug-in but it's as close as we can get without changing that code.
if isempty(settingsfile_name)
    sfile = convert_data_to_settings_file(newfile);
else
    sfile = settingsfile_name;
end

if isempty(session_commit) || session_commit == 0
    save(sfile,'saved','saved_autoset','fig_position');
    disp(['Settings saved to file: ',sfile]);
end

usebucket = bSettings('get','RIGS','use_bucket');
if usebucket == 1
    queststring = 'Do you want to upload the files to Bucket?';
    questtitle  = 'Bucket Upload';
    errorstring = 'Files FAILED to upload to Bucket';
    successstr  = 'Files successfully uploaded to Bucket';
else
...
if strcmp(answer3,'Yes')
    try
        commit_datafile_session(datafile,settingsfile);
    catch
