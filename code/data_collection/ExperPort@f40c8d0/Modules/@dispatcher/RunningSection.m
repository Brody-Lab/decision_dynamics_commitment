        ...
        if ~bSettings('compare','GENERAL','use_timers',0),
            ...
            while Running==1,
                ...
                try
                    ...
                    %Bpod can get stuck particularly in state 0. Let's
                    %check if that's happening
                    try
                        pe = value(parsed_events);
                        if ~isempty(pe) && isfield(pe,'states') && isfield(pe.states,value(CurrState))
                            if isnan(eval(['pe.states.',value(CurrState),'(end,1)']))
                                eval(['pe.states.',value(CurrState),'(end,1) = value(Time);']);
                                parsed_events.value = pe;
                            end
                            ...
                            if ~isempty(max_state_dur)
                                ...
                                if numel(state_pos) == 1
                                    ...
                                    if value(TimeCurrState) > max_currstate_dur
                                        currstate_dur_violated = 1;
                                    end
                                end
                            end

                            if (value(TimeCurrState) > 180 && value(Time) > value(TimeCurrState) &&...
                              (strcmp(value(CurrState),'state_0') || strcmp(value(CurrState),'check_next_trial_ready'))) ||...
                                (currstate_dur_violated == 1)

                                if runrats('session_ended') == 0
                                    %We have not clicked End Session on runrats
                                    ...
                                    %if value(Time) == value(TimeCurrState)
                                    %    keyboard
                                    %end
                                    ...
                                    chc{value(n_started_trials)} = [value(CurrState),' frozen at ',num2str(value(TimeCurrState)),'s'];
                    ...
                    %if rand(1)<0.02; x=1; disp(x(2)); end; %this forces errors for error handling debugging
                catch me1
