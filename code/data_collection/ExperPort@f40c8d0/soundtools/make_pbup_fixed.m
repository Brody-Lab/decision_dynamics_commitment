function [snd,lrate,rrate,data] = make_pbup_fixed(total_rate, gamma, srate, T, varargin)
%
% Makes Poisson bups
...
%   multiple of the sampling period).
    ...
    if task_type == 0 && numel(base_freq) > 1 
        ...
        if length(freq_multiply) > 1
            bupl = bupl*freq_multiply(1);
            bupr = bupr*freq_multiply(2);
        end
        bup  = bupl;
        ...
        if ~isempty(tones)
            ...
            if stereo_freq(1) ~= base_freq(1)
                sbup = singlebup(srate, 0,'ntones', ntones, 'width', bup_width, 'basefreq', stereo_freq(1), 'ramp', bup_ramp);
            else
                sbup = bup;
            end
        end
    ...
    if isempty(fixed_sound)
        ...
        if force_count == 1
            ...
            if ~is_fixed %classic pclicks instantiation
            ...
            else % WS edit: for fixed interval clicks
                tp_tot = linspace(1,n_bup_can_fit,total_rate); idxs = randperm(numel(tp_tot));
                tp1 = tp_tot(idxs(1:lrate)); tp2 = tp_tot(idxs(lrate:end));   
                tp1 = sortrows(tp1')'; tp2 = sortrows(tp2')';  
            end
        else
            ...
            lcount = length(tp1); rcount = length(tp2);
            tot_clicks = lcount + rcount;
            [~,idx_1,~] = intersect(sortrows([tp1 tp2]'),tp1);
            [~,idx_2,~] = intersect(sortrows([tp1 tp2]'),tp2);
            if (total_rate * T) < tot_clicks
                tp1 = tp1(idx_1 <= (total_rate * T));
                tp2 = tp2(idx_2 <= (total_rate * T));
                idx_1 = idx_1(idx_1 <= (total_rate * T));
                idx_2 = idx_2(idx_2 <= (total_rate * T));
            end
            if is_fixed %WS edit: for fixed interval clicks
                %rcount = round(rrate*real_T); lcount = round(lrate*real_T); %scale by how much time we have in this sample
                
                click_samp_rate = (1/total_rate)*srate; %click_samp_rate gets tp into the expected base for the rest of make_sound
                
                tp_tot = 1:click_samp_rate:tot_clicks*click_samp_rate; %create evenly spaced vector
                first_bup_t = (real_min_ISI + (1-real_min_ISI)*rand())*click_samp_rate; %randomly sample a first time and put in tp_tot units
                tp_tot = tp_tot + first_bup_t ; %shift tp_tot to *not* just start on the first sample
                
                if max(tp_tot) < (T * srate - click_samp_rate) %shift click train to end of T if there's space
                    space = (T * srate) - max(tp_tot);
                    tp_tot = tp_tot + space-first_bup_t;
                end
                
                %gets indices for tp1 and tp2
                if lcount < 1 %handles the case where there should be no l bups
                    tp1 = []; tp2 = tp_tot(idx_2);
                elseif rcount < 1 %handles the case where there should be no r bups
                    tp1 = tp_tot(idx_1); tp2 = [];
                else %otherwise takes first lcount indicies for left clicks, and rest for r
                    tp1 = tp_tot(idx_1); tp2 = tp_tot(idx_2); 
                end
            end
            tp1 = sortrows(tp1')'; tp2 = sortrows(tp2')';

            if is_sing
                k = randsample([tp1 tp2],1); %select one bup to keep
                tp1 = tp1(tp1==k);
                tp2 = tp2(tp2==k);
            end
        ...
        if first_bup_stereo
            ...
            bupwidth = bup_width*srate./1000./2;  % should  : bupwidth = bup_width*srate./1000./2
            %%
    ...
    if generate_sound
        ...
        if doing_frequency_version == 0
            if any(sbup ~= bup)
                for i = 2:length(tp1) % place left bups
                    if tp1(i)>w && tp1(i)+w<=size(snd,2)
                        snd(1,tp1(i)-w:tp1(i)+w) = snd(1,tp1(i)-w:tp1(i)+w)+bup;
                    end
                end
                for i = 2:length(tp2) % place right bups
                    if tp2(i)>w && tp2(i)+w<=size(snd,2)            
                        snd(2,tp2(i)-w:tp2(i)+w) = snd(2,tp2(i)-w:tp2(i)+w)+bup;
                    end
                end
                
                snd(1,tp1(1)-w:tp1(1)+w) = snd(1,tp1(1)-w:tp1(1)+w)+sbup;
                snd(2,tp2(1)-w:tp2(1)+w) = snd(2,tp2(1)-w:tp2(1)+w)+sbup;
