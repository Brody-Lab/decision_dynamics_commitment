...
function [obj] = PubVariableSize4(varargin)
...
%% INIT 
  case 'init'
    ...
    for i=1:9
        PushbuttonParam(obj,['Session',num2str(i)],0,0,'label',num2str(i),'position',[((MP(3)/13)*(i-1))+10 350 (MP(3)/13)-10 60]);
        ...
        SubheaderParam(obj,['SessionBox',num2str(i)],'',0,0,'position',[((MP(3)/13)*(i-1))+10 342 (MP(3)/13)-10 7]);
        set(get_ghandle(eval(['SessionBox',num2str(i)])),'BackgroundColor',[1 1 1]);
    end
    
    i = 10;
    PushbuttonParam(obj,'RedButton',0,0,'label','Start Manual Test','position',[((MP(3)/13)*(i-1))+10 350 330 60]);
    set(get_ghandle(eval('RedButton')),'Fontsize',22*F,'BackgroundColor',[1 0 0]);    
    ...
    for i=1:numel(RWL)
        ...
        for j=1:numel(RWL{i})
            ...
            if isempty(st) || isempty(ed) 
                ...
                if d < 0.95 && max(pbm) < value(MinWaterPerent)
    ...
    if value(IsRatRig) == 1; firstsnug = 301;
    else,                    firstsnug = 501;
    end
    ...
    brkcnt = 0;
    temp = (WRIGS(1)-firstsnug) - sum(brk < WRIGS(1)) + 1:(WRIGS(1)-firstsnug) - sum(brk < WRIGS(1)) + sum(~isnan(waterdio));
    for i = 1:numel(WRIGS)
        if sum(brk == WRIGS(i)) ~= 0
            brkcnt = brkcnt + 1;
        end
    end
    ratlistpos = min(temp):min(temp)+(value(SnugCount)-brkcnt)-1;
    ...
    for i = 1:value(SnugCount)
        ...
        elseif ~isnan(waterdio(i)) && ~isempty(activerats{i})
            ...
            PubVariableSize4(obj,'set_slot',{i,activerats{i}});
    ...
    for i = 1:value(SnugCount)
        if ~isempty(activerats{i})
            PubVariableSize4(obj,'get_rat_data',i);
        end
    end
    ...
    PubVariableSize4(obj,'manual_test');
  ...
  case 'prepare_next_trial'
    ...
    if n_done_trials > 0
        ...
        if sum(strcmp(s,'reset_state')) > 0 && ~isempty(parsed_events.states.reset_state)
            ...
            pause(1)
            PubVariableSize4(obj,'end_session');
            return;
        end
    ...
    if n_done_trials == 1
        ...
        if value(DoneReboot) == 0 %#ok<NODEF>
            if value(DoneTraining) == 0  %#ok<NODEF>
                ...
                PubVariableSize4(obj,'wait_for_training_end'); 
            end
            ...
            for i = 1:numel(wst)
                if isnan(wst(i)) && aw(i) == 1
                    ...
                    PubVariableSize4(obj,'update_bars',{i,'rig_water'});
            ...
            for i = 1:value(SnugCount)
                PubVariableSize4(obj,'update_bars',{i,''});
            end

            if all(dr <= 0)
                %RunningSection(dispatcher,'RunButtonCallback');
                PubVariableSize4(obj,'wait_for_reboot');
...
%% set_slot1_callback
    case 'set_slot1_callback'
        PubVariableSize4(obj,'set_slot',1);
        
%% set_slot2_callback
    case 'set_slot2_callback'
        PubVariableSize4(obj,'set_slot',2);
        
%% set_slot3_callback
    case 'set_slot3_callback'
        PubVariableSize4(obj,'set_slot',3);
        
%% set_slot4_callback
    case 'set_slot4_callback'
        PubVariableSize4(obj,'set_slot',4);
        
%% set_slot5_callback
    case 'set_slot5_callback'
        PubVariableSize4(obj,'set_slot',5);

%% set_slot6_callback
    case 'set_slot6_callback'
        PubVariableSize4(obj,'set_slot',6);
...
%% session1
  case 'Session1_callback'
      PubVariableSize4(obj,'update_session',1);  
        
%% session1
  case 'Session2_callback'
      PubVariableSize4(obj,'update_session',2);  
        
%% session1
  case 'Session3_callback'
      PubVariableSize4(obj,'update_session',3);  
        
%% session1
  case 'Session4_callback'
      PubVariableSize4(obj,'update_session',4);  
        
%% session1
  case 'Session5_callback'
      PubVariableSize4(obj,'update_session',5);  
        
%% session1
  case 'Session6_callback'
      PubVariableSize4(obj,'update_session',6);
        
%% session1
  case 'Session7_callback'
      PubVariableSize4(obj,'update_session',7);  
        
%% session1
  case 'Session8_callback'
      PubVariableSize4(obj,'update_session',8);  
        
%% session1
  case 'Session9_callback'
      PubVariableSize4(obj,'update_session',9);  
...
%% RedButton
  case 'RedButton_callback'
      ...
      if strcmp(curr_string,'Start Manual Test')
...
%% update_session
  ...
  for j = 1:value(SnugCount)
      ...
      PubVariableSize4(obj,'get_rat_data',j);
  end
...
%% end_session    
  ...
  case 'end_continued'
      ...
      if value(stopping_process_completed) %This is provided by dispatcher
          ...
          %This is where we should restart matlab with PubVariableSize4
          %running
          ...
          if value(CurrSess) == 8 || all(comp == 0)
...
for i = 1:numel(id)
    bdata('call mark_rigfixed("{Si}","{S}","{S}","{S}")',...
        id(i),...                              %maintenance_id
        '',...                                 %fixperson
        datestr(now,'yyyy-mm-dd HH:MM:SS'),... %fixdate
        '');                                   %fixnote
