...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    ...
    if      strcmp(value(Output),'Light Only')    == 1; do_out = right1led;
    elseif  strcmp(value(Output),'Water Only')    == 1; do_out = right1water;
    elseif  strcmp(value(Output),'Light & Water') == 1; do_out = right1led + right1water;
    end
    
    sma = StateMachineAssembler('full_trial_structure','use_happenings',1);
    
    sma = add_scheduled_wave(sma, 'name',     'pulse_wave',...
    ...
    sma = add_state(sma, 'name', 'wait_for_cin',...
                ...
                'input_to_statechange', {'Cin', 'pulsing'});
        
    sma = add_state(sma, 'name', 'pulsing', 'self_timer', Number_Pulses*(InterPulseInterval + Valve_Open_Time),...
        'output_actions', {'SchedWaveTrig', 'pulse_wave'},...
