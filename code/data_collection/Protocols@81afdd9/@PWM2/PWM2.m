% PWM2 protocol
%
% Written by Jorge Yanar & Jess Breda 2022. Successor to PWM, written
% by Emily Dennis & Athena Akrami.

function [obj] = PWM2(varargin)
...
switch action
    ...
    case 'init'
        ...
        set(value(myfig), 'Position', [400 45 825 700]);
        ...
        [x, y] = HistorySection(obj, 'init', x, y); y = y + 5;
        ...
        [x, y] = ShapingSection(obj, 'init', x, y); next_row(y, 1.1);
        ...
        % Debugging button, for stopping the protocol and examining SPHs
        ToggleParam(obj, 'debug_toggle', 0, x, y, 'position', [x y 80 20], ...
            'OnString', 'ENABLED', 'OffString', 'DISABLED');
        PushbuttonParam(obj, 'debug_button', x, y, 'position', [x+80 y 120 20], ...
            'label', 'ENTER DEBUGGING');
        disable(debug_button);
        set_callback(debug_toggle, {mfilename, 'debug_toggle_callback'});
        set_callback(debug_button, {mfilename, 'debug_button_callback'});
        next_row(y, 1.1);

        % Section for balancing motor & sound bias
        [x, y] = AntibiasSection(obj, 'init', x, y); next_row(y, 1.2);
        
        % Section for managing training stage & curriculum (inplace of SessionDefinition)
        [x, y] = TrainingSection(obj, 'init', x, y); next_row(y, 1.1);
        
        % Section for managing antibias on different stimulus pairs.
        %[x, y] = MultibiasSection(obj, 'init', x, y, [1 4 50 10]);
        
        
        [expmtr, rname] = SavingSection(obj, 'get_info'); 
        HeaderParam(obj, 'prot_title', ['PWM2: ' expmtr ', ' rname], 0, 683);
        ...
        PWM2(obj, 'prepare_next_trial');

        % Tell dispatcher the maximum amt of time we can spend in each state
    ...
    case 'prepare_next_trial'
        ...
        if isempty(varargin) || ~strcmp(varargin{1},'crashed')
            ...
            GetSoloFunctionArgs(obj); % History variables are global need this
            ...
            % TODO How to make AdLibGUI track water amounts for give, water multiplier pairs, etc?
            if n_done_trials > 0
                AdLibGUISection(obj, 'update_water_volume', side_history(end), reward_history(end));
            end        

            % Update Sides - is next trial a L or R?
            AntibiasSection(obj, 'update', SideSection(obj, 'get_left_prob'),...
                                           hit_history, side_history);
            ...
            % If we want to turn on the light guide only for low-hitrate pairs, check whether we're
            % going to show a low-hitrate pair, and set the light guide appropriately.
            ShapingSection(obj, 'set_lightguide_lowhitrate_pairs');
            ...
            case {'cpoke_fix', 'cpoke_nofix', 'cpoke_fix_decoupled'}
        ...
        if n_done_trials == 1
            [expmtr, rname]=SavingSection(obj, 'get_info');
            prot_title.value = ['PWM2 - on rig ' get_hostname ' : ' expmtr ', ' rname  '.  Started at ' datestr(now, 'HH:MM')];
    ...
    case 'debug_button_callback'
        % Enter debug mode!
        dbstop in RunningSection at 388
