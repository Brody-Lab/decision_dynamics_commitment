...
switch action
    ...
    case 'init',
        ...
        ToggleParam(obj, 'betaRampup', 0, x+180, y, 'position', [x+180, y, 15, 15], ...
            'label', '', ...
            'TooltipString', sprintf(['\nif ON (black), Beta ramps up linearly from trial 0 ' ...
                'to its desired indicated value at trial round(antibias_tau).\n' ...
                'If OFF (brown), Beta is the value indicated, no rampup.']))
        next_row(y);
        ToggleParam(obj, 'antibias_toggle', 1, x, y, 'position', [x y 200 20],...
            'OnString', 'Antibias ON', 'OffString', 'Antibias OFF', 'TooltipString',...
            'Turn Antibias on or off. Defaults to on.');
        next_row(y);
        ...
        % We define a local Beta value, for tracking the value of Beta when betaRampup is true.
        % If betaRampup is false, then LocalBeta always equals Beta.
        SoloParamHandle(obj, 'LocalBeta',       'value', [], 'save_with_settings', true);
        ...
        % Send out variables
        training_section_vars = {'antibias_toggle', 'Beta', 'betaRampup'};
        SoloFunctionAddVars('SideSection', 'ro_args', {'antibias_toggle'});
        ...
        SoloFunctionAddVars('TS_JB_cpoke_fix',   'rw_args', training_section_vars);
    ...
    case 'update_biashitfrac',
        ...
        if ~exist('n_done_trials', 'var')
            % we're not in dispatcher, probably doing some debugging 
            n_done_trials = 0; 
        end
        if betaRampup==false || round(value(BiasTau))==0 || n_done_trials >= round(value(BiasTau))
            % no Beta rampup, or already done with the rampup
            LocalBeta.value = value(Beta);
            choices = probabilistic_trial_selector([value(BiasLtHitFrac), value(BiasRtHitFrac)], ...
                    [LeftProb, 1-LeftProb], value(Beta));
        else
            % in the middle of Beta rampup
            LocalBeta.value = value(Beta)*n_done_trials/round(value(BiasTau));
            choices = probabilistic_trial_selector([value(BiasLtHitFrac), value(BiasRtHitFrac)], ...
                [LeftProb, 1-LeftProb], value(LocalBeta));        
        end
    ...
    case 'update_hitfrac'
        ...
        hit_hist = hit_hist(valid_trials); % exclude nans
        previous_sides = value(LocalPrevSides);
        previous_sides = previous_sides(valid_trials);
    ...
    case 'set'
        if length(varargin) ~= 2
            error('AntibiasSection:Invalid', '''set'' needs two extra params');
        end
        switch varargin{1}
        case 'antibias_tau'
            BiasTau.value = varargin{2};
        otherwise
            try
                eval([varargin{1} '.value = varargin{2}'])
            catch
                error('AntibiasSection:Invalid', 'Don''t know how to set %s', varargin{1});
            end
        end
