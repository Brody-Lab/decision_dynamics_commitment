...
% Edited to speed up the ramp to full trial duration April 2014 BBS
...
switch action,
    ...
    case 'init'
        x=varargin{1};
        y=varargin{2};
        NumeditParam(obj, 'StepSize', 250, x, y, 'TooltipString', 'Increase to NosPos on each completed trial');
        next_row(y);
        NumeditParam(obj, 'pressure', 0, x, y, 'TooltipString', 'Increase to NosPos on each completed trial');
        next_row(y);
        ...
        SoloFunctionAddVars('SMA1', 'ro_args', ...
            {'NosPos';'StepSize'; 'pressure'; 'StepNextTrial'; 'training_stage';'led_time'; 'bin_size';'side_lights' ; 'ignore_errors';...
            ...
            'PistonPressure'; 'Imaging';'WaterImmersion'; 'ObjWater_dur'; 'Vacuum_dur'});
    ...
    case 'next_trial',
        ...
        if n_done_trials > 0 && isempty(parsed_events.pokes.B) && ~was_viol && was_hit
            StepNextTrial.value=1;
            NosPos.value=Value(NosPos)+Value(StepSize);
            microswitch_counter.value=0;
        elseif n_done_trials > 0 && ~isempty(parsed_events.pokes.B) && ~was_viol && was_hit
        ...
        if n_done_trials>25 && antibias_wtr_mult==1
            ...
            if value(training_stage) ~=2
                right_hit = nanmean(hh(ps=='s'));
                left_hit = nanmean(hh(ps=='m'));
            else
        ...
        if use_training==1
            ...
            switch value(training_stage) %#ok<*NODEF>
                ...
                case 3 % Earliest stage of training.  Light chase and move center port
                    antibias_wtr_mult.value=0;
                    if value(microswitch_counter)>4
                ...
                case 1,  % Light chase and grow nose in center
                    ...
                    if ~was_viol  % if last trial was violation don't increment.
                        ...
                        if settling_time<0.25
                            settling_time.value=settling_time+0.3; %changed from 0.5 to 0.3 per trial
                            % settling_time.value=settling_time+0.01;
                        else
                            delay_time.value=delay_time+0.3;
                            % delay_time.value=delay_time+0.01; % changed from 0.5 to 0.3 per trial
                        end
                ...
                case 2, % start counting flashes
                    ...
                    %removed this next section because we are already
                    %ramping the animal up in the first section
                    ...
                    %   delay_time.value=0.04*n_done_trials;  %ramp up over the first 10 trials
                    %else
                    settling_time.value=1; %increased settling time for imaging
                    delay_time.value=0.5;
                    %end
                    
                    if trials_in_stage<1
                        nic_time.value=2*bin_size; %modified for more bins
                        if rand>0.5
                    ...
                    elseif trials_in_stage<500 %changed for F064
                        nic_time.value=4*bin_size; %modified for more bins
                        if rand>0.5
                    ...
                    elseif trials_in_stage<1500 %changed for F064
                        nic_time.value=6*bin_size; %modified for more bins
                        if rand>0.5
                            right_prob.value=0.9;
                            left_prob.value=0.1;
                        else
                    ...
                    elseif trials_in_stage<3000 %changed for F064
