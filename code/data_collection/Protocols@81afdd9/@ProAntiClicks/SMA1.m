...
switch action
    ...
    case 'next_trial',
        ...
        if (strcmp(value(ThisTask), 'Pro'))
            ...
            % for anti task
        elseif (strcmp(value(ThisTask), 'Anti'))
            if strcmp(value(ThisSide),'LEFT')
                correct_response='Rhi';
                error_response='Lhi';
                hit_dio=right1water;
        ...
        if(strcmp(value(ThisTask),'Pro'))
            ...
            wait_for_cpoke2='wait_for_cpoke_pro';
        elseif(strcmp(value(ThisTask),'Anti'))
            task_sound_id=task2_sound_id;
            helper_lights=value(helper_lights_anti);
            error_forgiveness=value(error_forgiveness_anti);
            wait_delay=value(wait_delay_anti);
            
            centerlight=center1led;
            centerlight2=center1led;
            
            wait_for_cpoke2='wait_for_cpoke_anti';
        ...
        sma = add_state(sma,'name','wait_for_cpoke',...
            'self_timer',min_time,...
            'input_to_statechange',{'Clo',wait_for_cpoke2}); %wait for poke out
        ...
        sma = add_state(sma,'name','wait_for_cpoke_pro',...
            'self_timer',value(wait_for_cpoke_timeout),...
            'output_actions',{'SoundOut',task_sound_id;'DOut', centerlight },...
            'input_to_statechange',{'Tup','timeout_state'; ...
            'Chi','nic_prestim'});
        ...
        sma = add_state(sma,'name','wait_for_cpoke_anti',...
            'self_timer',value(wait_for_cpoke_timeout),...
            'output_actions',{'SoundOut',task_sound_id;'DOut', centerlight },...
            'input_to_statechange',{'Tup','timeout_state'; ...
            'Chi','nic_prestim'});
        ...
        legal_cbreak= value(legal_cbreak);
        ...
        if error_forgiveness==1
            % you enter the first side (which is incorrect) wait state
            sma = add_state(sma,'name','wait_for_spoke','self_timer',value(wait_for_spoke_timeout),...
                ...
                error_response,'wait_for_spoke2';...
                'Tup','timeout_state'});
            
            % you enter the first side (which is incorrect) wait state
            sma = add_state(sma,'name','wait_for_spoke2','self_timer',value(wait_for_spoke_timeout),...
                'output_actions',{'DOut',sidelights},...
                'input_to_statechange',{correct_response,'delay_state';...
                error_response,'wait_for_spoke2';...
                'Tup','timeout_state'});
            
        else
            % otherwise you enter the hit or error state
            sma = add_state(sma,'name','wait_for_spoke','self_timer',value(wait_for_spoke_timeout),...
                'output_actions',{'DOut',sidelights},...
                'input_to_statechange',{correct_response,'hit_state';...
        ...
        % forgiveness reward with a delay
        % (i.e. if error_forgiveness == 1)
        sma = add_state(sma,'name','delay_state','self_timer', wait_delay,...
            'output_actions',{'SoundOut',-stimulus_sound_id;'DOut',hit_led; 'SoundOut',hit_sound_id},...
            'input_to_statechange',{'Tup','delayrew_state'});
        
        sma = add_state(sma,'name', 'delayrew_state','self_timer', value(reward_delay)+hit_valve_time+0.4,...
            'output_actions',{'SchedWaveTrig','direct_reward'},...
            'input_to_statechange',{'Tup','clean_up_state'});
        
        % we do the operations for the hit state
        sma = add_state(sma,'name','hit_state','self_timer',value(reward_delay)+hit_valve_time+0.4,...
        ...
        sma = add_state(sma,'name','timeout_state','self_timer',value(timeout_delay),...
            'output_actions',{'SoundOut',-stimulus_sound_id;'SoundOut',timeout_sound_id},...
            'input_to_statechange',{'Tup','clean_up_state'});
        ...
        dispatcher('send_assembler', sma, {'hit_state', 'error_state', 'delayrew_state'...
