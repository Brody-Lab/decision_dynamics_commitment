% protocol to assess probability distortion in a foraging task
% CMC Nov 2015

function [obj] = ProspectForaging(varargin)
...
switch action,
   ...
   case 'init'
      ...
      SoloParamHandle(obj, 'risky_pref_history', 'value', []);
      DeclareGlobals(obj, 'ro_args', {'hit_history'; 'risky_pref_history'});
     ...
     % [x, y] = HFWaterAdaptorSection(obj, 'init', x, y); 
      ...
      HeaderParam(obj, 'prot_title', ['ProspectForaging: ' expmtr ', ' rname], ...
         x, y, 'position', [10 figpos(4)-25, 800 20]);
      
      ProspectForaging(obj, 'prepare_next_trial');
   ...
   case 'trial_completed'
     ...
     %    AdLibGUISection(obj, 'set_first_trial_time_stamp');
      ...
      %x = 'l' or 'r'; y = 1 for a hit (if y ~= 1, the water won't be tallied for that trial).
      [x, y] = SidesSection(obj, 'get_hit_tally');
