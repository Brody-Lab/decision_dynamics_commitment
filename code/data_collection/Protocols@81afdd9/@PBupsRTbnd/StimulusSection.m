...
%% init    
  case 'init',
      ...
      ToggleParam(obj, 'rtbnd_task', 0, x, y, ...
            'OffString', 'non-reaction time _bound_ task', ...
            'OnString',  'reaction time bound task', ...
            'TooltipString', sprintf(['\n' ...
            'If reaction time bound task, then rat can indicate choice any time min_rt after stimulus onset.\n' ...
            'It is rewarded if the animal responds with the side which had greater than rtbnd clicks. \n' ...
            'If non-reaction time task, then stimulus duration is controlled by experimenter.\n']));
      set_callback(rtbnd_task,   {mfilename, 'rtbnd_task'});
      set_callback_on_load(rtbnd_task, 1);
      next_row(y);

      ToggleParam(obj, 'gamma_antibias', 0, x, y, ...
            'OffString', 'gamma antibias OFF', ...
            'OnString',  'gamma antibias ON', ...
            'TooltipString', sprintf(['\n' ...
            'If the animal violates a trial the next trial is of the same gamma. \n' ...
            'This is not quite probabilistic yet but that is the plan. \n']));
      next_row(y);

      NumeditParam(obj, 'gamAntiBiasProb', 0.5, x, y, ...
      'TooltipString', sprintf(['this variable sets the porbability with which the next trial will be of the same gamma'...
          '\n as previous when the gamma antibias is turned on']));
      next_row(y);


       NumeditParam(obj, 'rtBound', 10, x, y, ...
      'TooltipString', sprintf(['the stimulus must have atleast this many delta clicks for the trial to not be considered as a'...
          '\n violation trial']));
      disable(rtBound);
      next_row(y);
      ...
      SoloFunctionAddVars('SidesSection', 'ro_args', {'rt_task','rtbnd_task','rtBound'});
...
%% next_trial
  case 'next_trial',      
     ...
     if (n_done_trials > 1 & violation_history(end) == 1)
	     [this_sound this_T] = PBupsSection(obj, 'next_trial', set_side, previous_sides, previous_sounds, value(gamma_antibias), value(gamAntiBiasProb));
     else
       [this_sound this_T] = PBupsSection(obj, 'next_trial', set_side, previous_sides, previous_sounds, 0, 0);
     end
      ...
      if value(rtbnd_task)
          ...
          bups = [bupTimes.left'; bupTimes.right'];
          ids = [-1*ones(length(bupTimes.left),1); ones(length(bupTimes.right),1)];

          [sortbups, sortid] = sort(bups);
          newids = ids(sortid);
          delta = cumsum(newids);

            [a,b] = find(delta == (value(rtBound) - 1) | delta == (-value(rtBound)-1));

            min_rt.value = sortbups(a(1)); 
       end
