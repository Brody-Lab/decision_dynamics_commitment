...
switch action
	...
	case 'prepare_next_trial'
        ...
        if strcmp(block_switch_sound_type, 's_a')
            block_swtich_sound_id = SoundManagerSection(obj, 'get_sound_id', 'SaBlockSwitchSound');
        end
        ...
        pre_go_time = value(adjusted_pre_dur) + (2*value(stimulus_dur))+ value(delay_dur) + value(post_dur);
        ...
        if value(stimuli_on)
            ...
            timing_and_stim_waves_on = 'sa_sound_wave + sb_sound_wave + viol_off_wave + pre_go_wave + pre_go_cled_wave';
            % off waves triggered in violation state
            timing_and_stim_waves_off = '-sa_sound_wave - sb_sound_wave - viol_off_wave - pre_go_wave';
        else
            timing_and_stim_waves_on = 'viol_off_wave + pre_go_wave + pre_go_cled_wave';
        ...
        %% Block Switch Sound Waves
        % this is a sound wave that plays at the beginning of a new block
        % to indicate a transition occured
        if value(block_switch_sound)
            if strcmp(block_switch_sound_type, 's_a')
                sma = add_scheduled_wave(sma, 'name', 'block_switch_sound_wave',...
                                            'preamble', 0,...
                                            'sustain', 1,...
                                            'loop', value(block_switch_sound_dur),...
                                            'sound_trig', block_swtich_sound_id);
            end
        end
        ...
        %%% Block Switch Sound
        if value(block_switch_sound) && value(n_trials_in_block) == 0
           sma = add_state(sma, 'name', 'trigger_switch_sound',...
                             'self_timer', value(block_switch_sound_dur),...
                             'output_actions', {'SchedWaveTrig', 'block_switch_sound_wave'},...
        ...
        % on reward light (cant't do all of this AND play replay sounds)
        ...
        % delivery, but replay plays to indicate hit
        sma = add_state(sma, 'name', 'terror_water_delay',...
                             'self_timer', value(temp_error_water_delay),...
                             'output_actions', {'SchedWaveTrig', replay_sound_waves_on},...
