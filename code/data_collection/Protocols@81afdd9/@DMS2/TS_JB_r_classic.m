%% TS_JB_r_classic
% Training Section; Jess Breda; Rats ; Classic
% 
...
% sound rule.
%
% NOTE: this is very similar to TS_JB_m_classic upon intitial writing- it
% just takes into account longer trianing sessions & changes eos logic
%
% Stage 1: Poke Left
...
function varargout = TS_JB_r_classic(obj, action, varargin)
...
switch action,
    ...
    case 'get_update'
        ...
        case 5
            ...
            if value(n_trials_stage) == 0 % session init
                ...
                if value(n_days_stage) <= 1 % stage init 
                    ...
                    inter_trial_max.value = 15; %DB 20
        ...
        case 6
            ...
            %%% EOS logic
            % DB trials_end -> 10
            trials_end = min(value(blocks_end) + 100, 200);
            % ! could switch from frac_correct to last 50 trials?
        ...
        case 7
            ...
            if value(n_trials_stage) == 0 % session init
                ...
                if value(n_days_stage) <= 1 % stage init 
                    ...
                    inter_trial_max.value = 12; %DB 12
                    inter_trial_sample_std.value = 1; 
            ...
            if value(n_trials_stage) > 200 && value(frac_no_answer) < 0.1 && ...
                ...
                % current avg cpoke length, but no longer than required
                cpoke_durs = get_sphandle('name', 'cpoke_dur_history');
                avg_delay = mean(value(cpoke_durs{1}), 'omitnan') - 0.25;
                delay_dur.value = min(avg_delay, 1.2);
                TrainingSection(obj, 'increment_stage');
            end
        case 8
            %%
            ...
            %%% EOS logic
            if value(n_trials_stage) > 250 && value(frac_violations) < 0.20 && ...
                    value(frac_correct) > 0.85
                TrainingSection(obj, 'increment_stage');
            end 
        ...
        case 9
            ...
            if value(n_trials_stage) == 0 % session init
                ...
                if value(n_days_stage) <= 1 % stage init 
                    ...
                    stimulus_fixed_growth_rate.value = 0.003;
                    stimulus_fixed_growth_unit.value = 's';
                    stimulus_warm_up.value = 0;
                    delay_dur.value = 1; % allows for sounds to grow in (0.4 * 2s)
                    viol_off_growth.value = 'match_pre_go';

                    % inter trial dur is smaller in stage stage bc viol penalty
                    inter_trial_dur_type.value = 'sampled'; % gaussain
                    ...
                    % keep at light- water & light often leads to bad side
                    % biases
                    give_type_set.value = 'light';
                    ...
                    DistribInterface(obj, 'set', 'exp_delay', 'Max',   1.3);
                    DistribInterface(obj, 'set', 'exp_delay', 'Tau',   1.1);
            ...
            if strcmp(stimulus_growth, 'none')
                if value(n_trials_stage) == 1
                   volume_multiplier.value = value(volume_multiplier) - 0.15;
                elseif value(n_trials_stage) == 20 && value(frac_violations) < 0.4
                    volume_multiplier.value = value(volume_multiplier) + 0.15;
                end
            ...
            if (value(frac_violations) < 0.30) && (value(frac_correct) > 0.85) && ...
                    (value(volume_multiplier) == 0.85) && (value(stimulus_dur) >= 0.4)
                TrainingSection(obj, 'increment_stage');
            end
        ...
        case 10
            ...
            if value(n_trials_stage) == 0 % session init
                ...
                if value(n_days_stage) <= 1 % stage init 
                    ...
                    % set the stimulus to be an sb rule (3-3, 3-12)
                    presets.value = 'D-sb, sa-3';
                    StimulusSection(obj, 'set_preset');
                    
                    % initialize give frac (probabilistic) at full value
                    decrease_give_frac.value = 1;
                    give_type_set.value = 'light';
                    give_frac.value = 1;
    ...
    case 'get_eod_logic'
        ...
        case {5, 6, 7}
            % don't need animals fixating more than required and over-growing
            % themselves bc want to get to rule ASAP
            cpoke_durs = get_sphandle('name', 'cpoke_dur_history');
            avg_cpoke_dur = mean(value(cpoke_durs{1}), 'omitnan');
            if avg_cpoke_dur > 1.6 && ~strcmp(value(delay_growth), 'none')
                delay_growth.value = 'overnight_sampled';
            end

            if value(n_trials_stage) > 75 && strcmp(value(delay_growth), 'overnight_growth')
        ...
        case 9
            ...
            % NOTE this assumes desired stim length is 0.4 s and growth by
            % 0.15 overnight is good.
            if value(frac_violations) < 0.4
               if value(stimulus_min) < 0.4 
                   stimulus_min.value = min(0.4, (value(stimulus_min) + 0.1)); % TODO make this an SPH
               else
                   % turn off growth
                   delay_growth.value = 'none';
                   delay_dur.value = 0.2;
                   stimulus_growth.value = 'none';
                   volume_multiplier.value = min(0.85, (value(volume_multiplier) + 0.25)); 
               end
            end
            ...
            if strcmp(value(delay_growth), 'overnight_sampled')
                ...
                % note that because stimulus is growing, delay needs to
                % shrink otherwise fication with continue to grow
                min_exp = 1 - (value(stimulus_min) * 2);
                max_exp = 1.3;
                tau_exp = (min_exp + max_exp) / 2;

                DistribInterface(obj, 'set', 'exp_delay', 'Min',   min_exp);
                DistribInterface(obj, 'set', 'exp_delay', 'Max',   max_exp);
                DistribInterface(obj, 'set', 'exp_delay', 'Tau',   tau_exp);
        ...
        case 10
            % give frac handling done in ShapingSection
