...
switch action
    ...
    case 'init'
        ...
        %%%Pro-Anti presets
        ToggleParam(obj, 'pro_anti_on', 0, x, y, 'position', [x y 100 20],...
            'OnString', 'Pro-Anti ON', 'OffString', 'Pro-Anti OFF',...
            'TooltipString', 'Turn pro/anti blocks on and off'); 
        MenuParam(obj, 'pro_preset',...
            {'D-sb, sa-3','D-sa, sb-3', 'D-sb, sa-12','D-sa, sb-12','3-12 1/2 XOR', '12-3 1/2 XOR'},...
            1, x, y,'position', [x+100 y 100 20], 'label', 'Pro', 'labelfraction', 0.4,...
            'TooltipString', 'Whcih Stimulus pair preset to use as pro');
        MenuParam(obj, 'anti_preset',...
            {'D-sb, sa-12','D-sa, sb-12', 'D-sb, sa-3','D-sa, sb-3', '3-12 1/2 XOR', '12-3 1/2 XOR'},...
            1, x, y,'position', [x+200 y 100 20], 'label', 'Anti', 'labelfraction', 0.4,...
            'TooltipString', 'Whcih Stimulus pair preset to use as anti');
        set_callback(pro_anti_on, {mfilename, 'enable_pro_anti_callback'});
        DispParam(obj, 'pro_anti_block_type', 'NA', x, y, 'labelfraction', 0.6, 'label',...
            'p-a block', 'position', [x+300, y, 100, 20],...
            'TooltipString', 'Which type of pro-anti block is active (if any)');
        next_row(y);
        ...
        ToggleParam(obj, 'sound_vol_params_toggle', 0, x,y, 'position', [x+300 y 100 20],...
            'OnString', 'Sound Params Showing',...
            'OffString', 'Sound Params Hidden', 'TooltipString', 'Show/hide sound vol & params info');
        ...
        NumeditParam(obj, 'sb_extra', 0, x, y, 'label', 'Sb extra',...
            'labelfraction', 0.5, 'position', [x y 200 20],...
        ...
        set_callback(sb_extra, {mfilename, 'set_soundui_properties'}); next_row(y,1.1); 
        %%%  volume specific things
        NumeditParam(obj, 'volume_multiplier', 1, x, y, 'labelfraction', 0.5,...
            'label', 'Vol Mult', 'position', [x y 200 20]);next_row(y,1.1);
        ...
        [x, y] = SoundInterface(obj, 'add', 'SaSound', x, y, 'Duration', value(stimulus_dur));
        [x, y] = SoundInterface(obj, 'add', 'SaBlockSwitchSound', x+200, y-140); next_row(y, 0.5);
        [x, y] = SoundInterface(obj, 'add', 'SbSound', x-200, y, 'Duration', value(stimulus_dur));
        ...
        training_section_vars = {'stim_values', 'stimulus_type', 'rule', 'stimuli_on',...
            'presets', 'sb_extra', 'volume_multiplier', 'pro_anti_on', 'pro_anti_block_type'};
        ...
        for i = 1:numel(value(active_curricula))
            ...
            function_call = sprintf("SoloFunctionAddVars('TS_%s', 'rw_args', training_section_vars);", current_curricula );
            eval(function_call);
        end
        ...
        DeclareGlobals(obj, 'ro_args', ...
             ...
             'sb_extra', 'volume_multiplier',...
             'pro_anti_on', 'pro_preset', 'anti_preset', 'pro_anti_block_type'});
    ...
    case 'prepare_next_trial'
        ...
        % enable/disable pro-anti params handles if on/off
        StimulusSection(obj, 'enable_pro_anti_callback');
    ...
    %---------------------------------------------------------------%
    %          enable_pro_anti_callback                            %
    %---------------------------------------------------------------%      
    case 'enable_pro_anti_callback'
    ...
    case 'set_preset'
        switch value(presets)
            ...
            case '3-12 1/2 XOR' % for D-sb sa-3 or D-sa sb-12
                stim_table.value = {};
                switch value(stimulus_type)
                    case 'Frequency [Hz]'
            ...
            case '12-3 1/2 XOR' % for D-sb sa-12 or D-sa sb-3
                stim_table.value = {};
                switch value(stimulus_type)
                    case 'Frequency [Hz]'
                        Sa.value = 3000;  Sb.value = 3000; pprob.value = 0.25;   
    ...
    case 'play_sound'
        ...
        case 'Frequency [Hz]' 
            ...
            SoundInterface(obj, 'set', 'SbSound',... 
                                ...
                                'Loop', 0, 'Bal', 0);
            SoundInterface(obj, 'set', 'SaBlockSwitchSound',... 
                    'Style', 'Tone',...
                    'Vol', 0.001,...
                    'Freq1', stim_table{k,3},...
                    'Dur1', value(block_switch_sound_dur),...
                    'Loop', 0, 'Bal', 0);
        end
    ...
    case 'set_soundui_properties'
        ...
        switch value(stimulus_type)
            case 'Frequency [Hz]'
                ...
                % params), updating here because needs to match updates
                % made to sa/sb.
    ...
    case 'calculate_sound_volume'
        ...
        if sound_freq == 3000
            eval([sound_name '_vol.value = value(vol_3_khz) * value(volume_multiplier);']);
        elseif sound_freq == 12000
            eval([sound_name '_vol.value = value(vol_12_khz) * value(volume_multiplier);']);
        else
