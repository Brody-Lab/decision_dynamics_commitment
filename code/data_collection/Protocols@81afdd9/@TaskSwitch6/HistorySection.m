...
switch action,
    ...
    case 'init'
        ...
        SoloParamHandle(obj, 'opto_left_history', 'value',[]); 
        SoloParamHandle(obj, 'opto_right_history', 'value',[]); 
...
%         
    ...
    case 'next_trial',
        ...
        %is opto on?
        if(value(opto_active))
            opto_left_history.value=[value(opto_left_history) value(opto_left_power)];
            opto_right_history.value=[value(opto_right_history) value(opto_right_power)];        
        else
            opto_left_history.value=[value(opto_left_history) NaN];
            opto_right_history.value=[value(opto_right_history) NaN];
        end
        ...
        if(value(opto_active))
            ...
            vec_side=value(side_history);
            vec_res=value(result_history);
            vec_optoL=value(opto_left_history);
            vec_optoR=value(opto_right_history);
            
            nTrials_opto_left.value=length(find(vec_optoL>0 & vec_optoR==0));
            num_violations=length(find(vec_res==3 & vec_optoL>0 & vec_optoR==0));
            percent_violations_opto_left.value=round(100*num_violations/value(nTrials_opto_left))/100;
            left_correct_opto_left.value=round(100*nanmean(vec_hit(vec_side=='l' & vec_optoL>0 & vec_optoR==0)))/100;
            right_correct_opto_left.value=round(100*nanmean(vec_hit(vec_side=='r' & vec_optoL>0 & vec_optoR==0)))/100;
           
            nTrials_opto_right.value=length(find(vec_optoR>0 & vec_optoL==0));
            num_violations=length(find(vec_res==3 & vec_optoR>0 & vec_optoL==0));
            percent_violations_opto_right.value=round(100*num_violations/value(nTrials_opto_right))/100;
            left_correct_opto_right.value=round(100*nanmean(vec_hit(vec_side=='l' & vec_optoR>0 & vec_optoL==0)))/100;
            right_correct_opto_right.value=round(100*nanmean(vec_hit(vec_side=='r' & vec_optoR>0 & vec_optoL==0)))/100;
            
            nTrials_opto_bi.value=length(find(vec_optoR>0 & vec_optoL>0));
            num_violations=length(find(vec_res==3 & vec_optoR>0 & vec_optoL>0));
            percent_violations_opto_bi.value=round(100*num_violations/value(nTrials_opto_bi))/100;
            left_correct_opto_bi.value=round(100*nanmean(vec_hit(vec_side=='l' & vec_optoR>0 & vec_optoL>0)))/100;
            right_correct_opto_bi.value=round(100*nanmean(vec_hit(vec_side=='r' & vec_optoR>0 & vec_optoL>0)))/100;
            
            nTrials_opto_0.value=length(find(vec_optoR==0 & vec_optoL==0));
            num_violations=length(find(vec_res==3 & vec_optoR==0 & vec_optoL==0));
            percent_violations_opto_0.value=round(100*num_violations/value(nTrials_opto_0))/100;
            left_correct_opto_0.value=round(100*nanmean(vec_hit(vec_side=='l' & vec_optoR==0 & vec_optoL==0)))/100;
            right_correct_opto_0.value=round(100*nanmean(vec_hit(vec_side=='r' & vec_optoR==0 & vec_optoL==0)))/100;
            
            
        else
            
            
            nTrials_opto_left.value=0;
            percent_violations_opto_left.value=0;
            left_correct_opto_left.value=0;
            right_correct_opto_left.value=0;
           
            nTrials_opto_right.value=0;
            percent_violations_opto_right.value=0;
            left_correct_opto_right.value=0;
            right_correct_opto_right.value=0;
            
            nTrials_opto_bi.value=0;
            percent_violations_opto_bi.value=0;
            left_correct_opto_bi.value=0;
            right_correct_opto_bi.value=0;
            
            nTrials_opto_0.value=0;
            percent_violations_opto_0.value=0;
            left_correct_opto_0.value=0;
            right_correct_opto_0.value=0;
