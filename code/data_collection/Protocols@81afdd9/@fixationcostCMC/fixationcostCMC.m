% fixationcost can be used to asses the relative cost of headrestraint
% BBS May 2014

function [obj] = fixationcostCMC(varargin)

% Default object is of our own class (mfilename);
% we inherit only from Plugins

obj = class(struct, mfilename, saveload, water, ...
   pokesplot2, AdLibGUI, soundmanager, soundui, ...
...
switch action,
   ...
   case 'init'
      ...
      HeaderParam(obj, 'prot_title', ['fixationcostCMC: ' expmtr ', ' rname], ...
         x, y, 'position', [10 figpos(4)-25, 800 20]);
      
      fixationcostCMC(obj, 'prepare_next_trial');
