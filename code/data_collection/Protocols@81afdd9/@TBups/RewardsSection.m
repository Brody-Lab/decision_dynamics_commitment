% @TBups/RewardsSection.m
% Wynne, based off RewardsSection of PBups protocol
...
switch action
  case 'init',
    ...
    % MenuParam(obj, 'reward_type', {'diff_clicks';'endpoints_only'}, 1, x, y, ...
    %     'labelfraction', 0.4, ...
    %     'TooltipString', 'Reward based on generative alpha or ICI difference.'); next_row(y);
    % NumeditParam(obj, 'diff_click_threshold', 0.001, x, y, ...
    %     'TooltipString', sprintf(['if reward_type is diff_clicks, this is the time in s which clasifies a trial as irregularly timed'])); next_row(y);        
    ...
    DispParam(obj, 'nogive_hitfrac', 0, x, y, ...
        'TooltipString', 'average hit frac, ignoring give trials, only valid when give on'); next_row(y);
    ...
    SoloFunctionAddVars('TBups', 'rw_args',{'nTrials'});
  ...
  case 'next_trial',
    if n_done_trials > 0,
        if isfield(parsed_events.states, 'temp_error_state') && rows(parsed_events.states.temp_error_state) > 0,
            ...
            temp_err_history.value = [temp_err_history(:); 1];
            nRewarded.value = nRewarded + 1;
        elseif rows(parsed_events.states.hit_state) > 0 , 
            hit_history.value = [hit_history(:) ; 1]; 
            temp_err_history.value = [temp_err_history(:); 0];
            if rows(parsed_events.states.reward_state) > 0, nRewarded.value = nRewarded + 1; end; 
        elseif isfield(parsed_events.states, 'violation_state') && rows(parsed_events.states.violation_state) > 0,
            ...
            temp_err_history.value = [temp_err_history(:); NaN];
        else
            hit_history.value = [hit_history(:) ; 0];
            temp_err_history.value = [temp_err_history(:); 0];
        end;
        
        if isfield(parsed_events.states, 'probe_state') && rows(parsed_events.states.probe_state) > 0,
            probe_history.value = [probe_history(:) ; 1];          
        else
            probe_history.value = [probe_history(:) ; 0];
        end
        if isfield(parsed_events.states, 'reward_state') && rows(parsed_events.states.reward_state) > 0,
            reward_history.value = [reward_history(:) ; 1];
        else
            reward_history.value = [reward_history(:) ; 0];
        end;
        if isfield(parsed_events.states, 'post_tup_state') && rows(parsed_events.states.post_tup_state) > 0,
            tup_history.value = [tup_history(:) ; 1];
        else
            tup_history.value = [tup_history(:) ; 0];
        end;
        ...
        if (isfield(parsed_events.states, 'temp_violation') && rows(parsed_events.states.temp_violation>0)) ...
        ...
        if value(use_give) && value(stim_type) == "Sound Only" 
            if hit_history(end) == 1,
                no_give_hit_history.value = [no_give_hit_history(:) ; 1];
            else
                no_give_hit_history.value = [no_give_hit_history(:) ; 0];
            end;
        elseif(use_give) && value(stim_type) == "Light AND Sound"
            no_give_hit_history.value = [no_give_hit_history(:) ; nan];
        elseif(use_give==0)
            no_give_hit_history.value = [no_give_hit_history(:) ; nan];
        end;
        nogive_hitfrac.value = nanmean(no_give_hit_history(:));
    end;
