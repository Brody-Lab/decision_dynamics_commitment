...
switch action,
    ...
    case 'init'
        ...
        next_row(y);
        NumeditParam(obj, 'risky_prob', 0, x, y, 'TooltipString', 'Probability of a high reward');
        next_row(y);
        NumeditParam(obj, 'risky_prob_options', [0.1:.1:1], x, y, 'label', 'risky_prob_options');
        next_row(y);
        NumeditParam(obj, 'risk_hit', 1, x, y, 'TooltipString', 'Will this gamble pay off?');
        next_row(y);
        NumeditParam(obj, 'risky_volume', 35, x, y, 'TooltipString', 'Volume on side with flashes');
        next_row(y);
        NumeditParam(obj, 'safe_volumes', [3:7:45], x, y, 'label', 'safe_volumes');
        next_row(y);
        NumeditParam(obj, 'this_safe_volume', 10, x, y, 'label', 'this_safe_volume');
        next_row(y);
        ...
        SoloFunctionAddVars('SMA1', 'ro_args', ...
            ...
            'right_prob'; 'left_prob';'risky_prob';'risky_prob_options';'safe_side';...
            'this_safe_volume'; 'safe_volumes'; 'risky_volume'; 'nic_time'; 'wait_for_cpoke_timeout';...
            ...
            switch value(training_stage) %#ok<*NODEF>
                ...
                case 2, % start counting flashes
                   ...
                   % last_trial=previous_sides(end)=='r' || previous_sides(end)=='s';
                    ...
                    r = rand(1,length(value(safe_volumes)));
                    dice = rand(1,length(p));
                    risky_prob.value = value(risky_prob_options(find(dice==max(dice))));
                    risky_volume.value = value(risky_volume);
                    dice = rand(1, length(r));
                    this_safe_volume.value = value(safe_volumes(find(dice==max(dice))));
                    
                    if value(randomize_sides)==1
                        
                        if doantibias ==1
                            if max([value(left_bias) value(right_bias)]) >= .8; %bias is really bad.
                                risky_prob.value = min(value(risky_prob_options));
                                this_safe_volume.value = max(value(safe_volumes));
...
%                                 
%                                 % if doantibias ==1
%                                 if value(risky_prob)*value(risky_volume) < value(this_safe_volume);
%                                     if value(left_bias) < value(right_bias)
%                                         probmult.value = value(left_bias);
%                                     elseif value(left_bias) > value(right_bias);
%                                         probmult.value = value(right_bias);
%                                     elseif value(left_bias)==value(right_bias);
%                                         probmult.value = 0.5;
%                                     end
%                                 elseif value(risky_prob)*value(risky_volume) >= value(this_safe_volume);
%                                     if value(left_bias) < value(right_bias);
%                                         probmult.value = value(right_bias);
%                                     elseif value(left_bias) > value(right_bias);
%                                         probmult.value = value(left_bias);
...
%                                 end
                                ...
                                probmult.value = .5;
%                                 
                                if rand>value(probmult); %.5
                                    safe_side.value = 'l';
                                else
                                    safe_side.value = 'r';
                                end
                            end
                            %elseif antibias_wtr_mult ==0;
                        elseif doantibias ==0;
                            probmult.value = 0.5;
                            if rand>value(probmult); %.5
                                safe_side.value = 'l';
                        ...
                        %                             safe_side.value = 'r';
                        %                         end
                    elseif value(randomize_sides)==0
                        if value(blockcounter)==0 || value(blockcounter) > value(block_length);
                            if rand>.5
                                safe_side.value = 'l';
                                balance = -1;
                            else
    ...
    case 'make_and_send_summary'
        ...
        pd.this_safe_volume = cell2mat(get_history(this_safe_volume));
        ...
        pd.safe_side = pd.safe_side(1:nt);
        pd.this_safe_volume = pd.this_safe_volume(1:nt);
        pd.risky_prob = pd.risky_prob(1:nt);
        % pd.risky_prob_options = pd.risky_prob_options(1:nt);
        % pd.safe_volumes = pd.safe_volumes(1:nt);
