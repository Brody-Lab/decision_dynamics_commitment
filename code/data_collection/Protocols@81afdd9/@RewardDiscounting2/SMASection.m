...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    ...
    if strcmp(type,'Forced')
        ...
        leave_Lup = {'Lin','left_reward';  'Tup', 'error_state'}; 
        leave_Rup = {'Rin','right_reward'; 'Tup', 'error_state'};
    else
        if Decision_Time == 1
    ...
    if  Fix_Trial_Length    == 0 
        ...
        NRWtimeL = Left_PostReward;  if NRWtimeL <= 0; NRWtimeL = 0.001; end  
        NRWtimeR = Right_PostReward; if NRWtimeR <= 0; NRWtimeR = 0.001; end
        leave_after_Lreward = 'left_wait_after';
        leave_after_Rreward = 'right_wait_after';
    end
        
    Left_Window = Reward_Window * l_gap; 
    Right_Window = Reward_Window * r_gap;
    
    if     Left_Window < MinReward_Window; 
           Left_Window = MinReward_Window; 
    elseif Left_Window > MaxReward_Window; 
           Left_Window = MaxRewardWindow;
    end
    if     Right_Window < MinReward_Window; 
           Right_Window = MinReward_Window; 
    elseif Right_Window > MaxReward_Window; 
           Right_Window = MaxRewardWindow;
    end
    
    if  Left_Window == Inf;  Left_Window = 1e6; end
    if Right_Window == Inf; Right_Window = 1e6; end
    ...
    sma = add_scheduled_wave(sma, 'name', 'leftpost_wave',  'preamble', Left_PostReward); 
    sma = add_scheduled_wave(sma, 'name', 'rightpost_wave', 'preamble', Right_PostReward);
    
    if value(Left_PostReward) > 0 && Fix_Trial_Length == 0
        sma = add_state(sma, 'name', 'iti', 'self_timer', Left_PostReward,...
    ...
    if strcmp(type,'Forced')
        sma = add_state(sma, 'name', 'leftup','self_timer',Left_Window,...
            'output_actions', {'DOut', led},...
            'input_to_statechange', leave_Lup);

        sma = add_state(sma, 'name', 'rightup','self_timer',Right_Window,...
            'output_actions', {'DOut', led},...
    ...
    sma = add_state(sma, 'name', 'bothup','self_timer',max([Left_Window Right_Window]),...
        'output_actions', {'DOut', led},...
        'input_to_statechange', {'Tup','error_state';'Lin','left_reward'; 'Rin','right_reward'});
    ...
    if rand(1) <= L_Reward_Probability
        sma = add_state(sma, 'name', 'left_reward', 'self_timer', LtValve, ...
            'output_actions', {'DOut',left1water+left1led; 'SchedWaveTrig', 'leftpost_wave'},...
            'input_to_statechange', {'Tup','current_state+1'});
        ...
        sma = add_state(sma, 'name', 'left_reward','self_timer',0.01, ...
            'output_actions', {'DOut',left1led; 'SchedWaveTrig', 'leftpost_wave'},...
    ...
    if rand(1) <= R_Reward_Probability
        sma = add_state(sma, 'name', 'right_reward', 'self_timer', RtValve, ...
            'output_actions', {'DOut',right1water+right1led; 'SchedWaveTrig', 'rightpost_wave'},...
            'input_to_statechange', {'Tup','current_state+1'});
        ...
        sma = add_state(sma, 'name', 'right_reward','self_timer',0.01, ...
            'output_actions', {'DOut',right1led; 'SchedWaveTrig', 'rightpost_wave'},...
    ...
    sma = add_state(sma, 'name', 'no_reward_wait_ll','self_timer',NRWtimeL,...
        'output_actions', {'DOut',left1led},...
        'input_to_statechange', {'Tup','warndanger'; 'leftpost_wave_In','warndanger'});

    sma = add_state(sma, 'name', 'no_reward_wait_rr','self_timer',NRWtimeR,...
        'output_actions', {'DOut',right1led},...
        'input_to_statechange', {'Tup','warndanger'; 'rightpost_wave_In','warndanger'});
