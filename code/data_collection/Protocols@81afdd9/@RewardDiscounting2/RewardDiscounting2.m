...
%% INIT 
  case 'init'
    ...
    SoloParamHandle(obj, 'check_stability',   'value', 0 );
...
%     set_callback_on_load(after_load_callbacks, 1);
    ...
    next_column(x); y=5;
    MenuParam(obj, 'n_center_pokes', {'0', '1'}, 2, x, y, 'labelfraction', 0.7, 'position', [x y 140 20]); next_row(y);
    
    ToggleParam(obj, 'Start_on_Left', 0, x,y, 'OnString','Start_With_Left_Poke',...
        'OffString','No_Initial_Left_Poke','position', [x y 140 20]); next_row(y, 1.1);
    
    [x, y] = DistribInterface(obj, 'add', 'left_gap', x, y);  %next_row(y);
    [x, y] = DistribInterface(obj, 'add', 'right_gap', x, y); next_row(y,0.1);
    
    NumeditParam(obj, 'Reward_Window',    0.2, x, y, 'position', [x y 140 20],   'labelfraction', 0.75); next_row(y);
    NumeditParam(obj, 'MinReward_Window',   2, x, y, 'position', [x    y 70 20], 'labelfraction', 0.5, 'label','Min');
    NumeditParam(obj, 'MaxReward_Window', Inf, x, y, 'position', [x+70 y 70 20], 'labelfraction', 0.5, 'label','Max'); next_row(y);
    NumeditParam(obj, 'Left_PostReward',    0, x, y, 'position', [x y 140 20],   'labelfraction', 0.75); next_row(y);
    NumeditParam(obj, 'Right_PostReward',   0, x, y, 'position', [x y 140 20],   'labelfraction', 0.75); next_row(y);
    ...
    SoloFunctionAddVars('SMASection',  'ro_args', {'Reward_Window',...
                                                   'MinReward_Window',...
                                                   'MaxReward_Window',...
                                                   'Left_PostReward',...
                                                   'Right_PostReward',...
...
%% pre_saving_settings
  ...
  case 'pre_saving_settings'
        ...
        pre_left        = get_sphandle('name','left_gap_Min');
        pre_right       = get_sphandle('name','right_gap_Min');
        
        post_left       = get_sphandle('name','Left_PostReward');
        post_right      = get_sphandle('name','Right_PostReward');
        ...
        ID1 = get_sphandle('name','Indifference1');
        ID2 = get_sphandle('name','Indifference2');
        ID3 = get_sphandle('name','Indifference3');
        ID4 = get_sphandle('name','Indifference4');
        ID5 = get_sphandle('name','Indifference5');
        ID6 = get_sphandle('name','Indifference6');
        ID7 = get_sphandle('name','Indifference7');
        ID8 = get_sphandle('name','Indifference8');
        
        pd.pre_left        = cell2mat(get_history(pre_left{1}));
        pd.pre_right       = cell2mat(get_history(pre_right{1}));
        pd.post_left       = cell2mat(get_history(post_left{1}));
        pd.post_right      = cell2mat(get_history(post_right{1}));
