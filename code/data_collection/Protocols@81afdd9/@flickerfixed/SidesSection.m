...
switch action,
    ...
    case 'init'
        ...
        next_row(y);
         NumeditParam(obj, 'Max_NIC', 2, x,y,'label','Maximum NIC','TooltipString','Max NIC time');
        next_row(y);
    ...
    case 'get_current_side',
        x = value(previous_sides(end-1));
        y = value(hit_history(end));
            ...
            switch value(training_stage) %#ok<*NODEF>
                ...
                case 1,  % Light chase and grow nose in center
                    ...
                    if ~was_viol  % if last trial was violation don't increment.
                        ...
                        delay_time.value=delay_time+0.005;
                        
                        
                        allow_nic_breaks.value=0; %#ok<*STRNU>
                        
                        
                        if delay_time>0.25 % increased this time to make the transition easier
                            training_stage.value=2;
                            trials_in_stage.value=0;
                            delay_time.value=0;
                            bin_size.value=0.250;
                            nic_time.value=value(bin_size);
                        else
                            side_lights.value=3;
                        end
                    end
                ...
                case 2, % start counting flashes
                    ...
                    if ~was_viol
                        ...
                        if value(nic_time) <= value(Max_NIC);
                             nic_time.value=value(nic_time)+0.001;
                        end
                        if trials_in_stage>750
                        if value(settling_time)<1
                            settling_time.value=value(settling_time)+0.001;
                        end
                        
                        if value(delay_time)<0.5
                            delay_time.value=value(delay_time)+0.001;
                        end
                        end
                    end
                    
                    if trials_in_stage<250
                        %side_light.value=3;
                        if rand>0.5
                            right_prob.value=1;
                            left_prob.value=0;
                    ...
                    elseif trials_in_stage<1500
                        ...
                        led_time.value=0.03;
                    else   %if trials_in_stage<3000             %On Dec 30th 2014 we changed the probability values to mirror those       
                        if rand>0.5                         %that were used in the original flicker fixed.
