...
switch action
  case 'init',
    ...
    DispParam(obj, 'n_right_pokes', 0, x, y, 'labelfraction',0.45); next_row(y);
    DispParam(obj, 'n_left_pokes', 0, x, y, 'labelfraction',0.45); next_row(y);
    DispParam(obj, 'n_center_pokes', 0, x, y, 'labelfraction',0.45); next_row(y);
    DispParam(obj, 'hit_streak', 0, x, y, 'labelfraction',0.45, ...
        'TooltipString', 'number of continuous hit trials (2 hit strials -> 1 hit_streak)');
    next_row(y);
    
    DispParam(obj, 'last60_hitfrac_byDiff', 0, x, y, 'label','last 60', 'labelfraction', 0.45,...
        'TooltipString', 'average hit frac in the last 30 trials ignoring violations'); next_row(y);
    DispParam(obj, 'last30_hitfrac_byDiff', 0, x, y, 'label','last 30', 'labelfraction', 0.45,...
        'TooltipString', 'average hit frac in the last 30 trials ignoring violations'); next_row(y);
    DispParam(obj, 'last15_hitfrac_byDiff', 0, x, y, 'label','last 15', 'labelfraction',0.45,...
        'TooltipString', 'average hit frac in the last 10 trials ignoring violations'); next_row(y);
    DispParam(obj, 'mean_hitfrac_byDiff', 0, x, y, 'label','hitfrac_byDiff', 'labelfraction',0.45,...
        'TooltipString', 'average hit frac, ignoring violation trials'); next_row(y);
    
    
    DispParam(obj, 'last60_hitfrac', 0, x, y, 'labelfraction',0.45,...
        'TooltipString', 'average hit frac in the last 60 trials ignoring violations'); next_row(y);
    DispParam(obj, 'last30_hitfrac', 0, x, y, 'labelfraction',0.45,...
        'TooltipString', 'average hit frac in the last 30 trials ignoring violations'); next_row(y);
    DispParam(obj, 'last15_hitfrac', 0, x, y, 'labelfraction',0.45,...
        'TooltipString', 'average hit frac in the last 10 trials ignoring violations'); next_row(y);
    DispParam(obj, 'mean_hitfrac', 0, x, y, 'labelfraction',0.45,...
        'TooltipString', 'average hit frac, ignoring violation trials'); next_row(y);
    DispParam(obj, 'cpoke_violations', 0, x, y, 'labelfraction',0.45, ...
        'TooltipString', 'number of cpoke violations (where the rat did not complete cpokes within wait_for_cpoke_grace)'); next_row(y);
    DispParam(obj, 'nTrials',  0, x, y, 'labelfraction',0.45); next_row(y);
    DispParam(obj, 'nRewarded', 0, x, y, 'labelfraction',0.45,...
        'TooltipString', 'number of trials for which water was dispensed'); next_row(y);
    DispParam(obj, 'nCorrect', 0, x, y, 'labelfraction',0.45,...
        'TooltipString', 'number of trials where the response was correct'); next_row(y);
    
    ToggleParam(obj, 'direct_delivery', 0, x, y, ...
        'OffString', 'Deliver water on correct poke', ...
        'OnString',  'Deliver water on either poke ', ...
        'TooltipString', sprintf(['\n' ...
        'If ''deliver water on poke'', reward is given only when the rat pokes the correct poke.\n' ...
        'If ''direct delivery'', water is given out at the correct side after the rat pokes either poke\n']));
    set_callback(direct_delivery, {'StateMatrixSection', 'next_trial'});
    next_row(y);
  ...
  case 'next_trial',
    if n_done_trials > 0,
        ...
        % Update previous hitfracs by difficulty
        [tilde, diffHistory] = SidesSection(obj,'get_previous_sides_and_sounds');
        diffHistory = abs(diffHistory);
        % Get the number of difficulties from PBupsSection
        numDiffs = PBupsSection(obj,'get','nstimuli') / 2;
        % Calculate the recent accuracy by difficulty, put it up
        last15 = zeros(1,numDiffs); last30 = zeros(1,numDiffs); last60 = zeros(1,numDiffs);
        for diff_i = 1:numDiffs
            history_by_diff = hit_history(diffHistory == diff_i);
            all(diff_i) = mean(history_by_diff);
            if length(history_by_diff) >= 15, last15(diff_i) = mean(history_by_diff((end-14):end)); end;
            if length(history_by_diff) >= 30, last30(diff_i) = mean(history_by_diff((end-29):end)); end;
            if length(history_by_diff) >= 60, last60(diff_i) = mean(history_by_diff((end-59):end)); end;
        end
        mean_hitfrac_byDiff.value = all;
        last15_hitfrac_byDiff.value = last15;
        last30_hitfrac_byDiff.value = last30;
        last60_hitfrac_byDiff.value = last60;
...
%% get direct_delivery
    case 'get_direct_delivery'
        x = value(direct_delivery);
