...
switch action
    ...
    case 'next_trial',
        ...
        value_task = ValueSection(obj, 'get_feedback_type');  % Are we using the value scheme, or running traditional PBups?
        ...
        right_side_valvetime = RightWValveTime*right_wtr_mult;
        left_side_valvetime = LeftWValveTime*left_wtr_mult;
        %% set up responses
        
     
        
        % I added the 'if ~value_task' conditionals here --KJM
        if SidesSection(obj, 'get_current_side') == 'l',                  % Left is the correct response
            
            % set up the rewards for the value task
            if strcmp(value(reward_timing), 'from spoke'),
                LeftResponseState = 'correct_reward';
                RightResponseState = 'incorrect_reward';
            elseif strcmp(value(reward_timing), 'from cpoke'),
                LeftResponseState = 'hit_state';
                RightResponseState = 'err_value';
            else
            ...
            incorrect_side_water = right1water;
            correct_side_valvetime = left_side_valvetime;
            incorrect_side_valvetime = right_side_valvetime;
            
        else   % Right is the correct response
            
            
            if strcmp(value(reward_timing), 'from spoke'),
                LeftResponseState = 'incorrect_reward';
                RightResponseState = 'correct_reward';
            elseif strcmp(value(reward_timing), 'from cpoke'),
                LeftResponseState = 'err_value';
            ...
            incorrect_side_water = left1water;
            correct_side_valvetime = right_side_valvetime;
            incorrect_side_valvetime = left_side_valvetime;
        end;
        
        % There's a strange problem that happens when valvetime = 0;  Check
        % for it here and try to fix it.  We'll do this by saying if
        % valvetime is zero, break the reference to the valve so we
        % couldn't turn it on even if we wanted to...
        if incorrect_side_valvetime == 0
            incorrect_side_water = 0;
        end
        if correct_side_valvetime == 0;
            correct_side_water = 0;
        end
        
        % I added the correct_ and incorrect_ rewards here --KJM
        ...
        sma = add_scheduled_wave(sma, 'name', 'correct_reward', 'preamble', reward_delay(1), ...
            'sustain', correct_side_valvetime, 'DOut', correct_side_water);
        % If in direct delivery training mode, we should spit out water at
        % the correct side after a side poke, even if the poke was on the
        % wrong side.
        direct_delivery = RewardsSection(obj,'get_direct_delivery');
        if direct_delivery
            sma = add_scheduled_wave(sma, 'name', 'incorrect_reward', 'preamble', reward_delay(1), ...
                'sustain', correct_side_valvetime, 'DOut', correct_side_water);
        else
            sma = add_scheduled_wave(sma, 'name', 'incorrect_reward', 'preamble', reward_delay(1), ...
                'sustain', incorrect_side_valvetime, 'DOut', incorrect_side_water);
        end
        ...
        waves_on  = '+sound_on + min_rt_wave + laser_stim_on';
        waves_off = '-sound_on - min_rt_wave';
        waves_off_cleanup = '-sound_on - min_rt_wave - laser_stim_on';
        ...
        sma = add_state(sma, 'name', 'legal_cbreak', 'self_timer', 1000, ...
            'output_actions', {'DOut',          cpoke_led}, ...
        ...
        sma = add_state(sma, 'self_timer', legal_cbreak_rt, ...
            'output_actions', {'DOut',          cpoke_led}, ...
        ...
        % I altered these to trigger right_reward and left_reward instead
        % of just correct_side_reward --KJM
        sma = add_state(sma, 'name', 'correct_reward', 'self_timer', locked_snd_dur, ...
            'output_actions', {'SchedWaveTrig', ['correct_reward' swaves_to_add]; ...
            'SoundOut', locked_snd}, ...
            'input_to_statechange', {'Tup', 'hit_state'});
        sma = add_state(sma, 'name', 'incorrect_reward', 'self_timer', locked_snd_dur, ...
            'output_actions', {'SchedWaveTrig', ['incorrect_reward' swaves_to_add];
            'SoundOut', locked_snd}, ...
            'input_to_statechange', {'Tup', 'err_value'});
     
        %% hit states
        if SoftDrinkTime == 1,
            post_hit_state_in_corr  = 'soft_drink_state';
            post_hit_state_out_corr = 'soft_drink_state+1';
            post_hit_state_in_incorr  = 'err_soft_drink_state';
            post_hit_state_out_incorr = 'err_soft_drink_state+1';
        else
            post_hit_state_in_corr  = 'drink_state';
            post_hit_state_out_corr = 'drink_state';
            post_hit_state_in_incorr  = 'drink_state';
            post_hit_state_out_incorr = 'drink_state';
        end;
        
        switch SidesSection(obj, 'get_current_side'),
            case 'l', InAction_corr = 'Lin'; OutAction_corr = 'Lout'; OtherSidePokeIn_corr = 'Rin'; InAction_incorr = 'Rin'; OutAction_incorr = 'Rout'; OtherSidePokeIn_incorr = 'Lin';
            case 'r', InAction_corr = 'Rin'; OutAction_corr = 'Rout'; OtherSidePokeIn_corr = 'Lin'; InAction_incorr = 'Lin'; OutAction_incorr = 'Lout'; OtherSidePokeIn_incorr = 'Rin';
            otherwise
        ...
        sma = add_state(sma, 'input_to_statechange', ...
            {'hitstatetimer_In', post_hit_state_in_corr ; ...
            OutAction_corr, 'current_state+1'});
        sma = add_state(sma, 'input_to_statechange', ...
            {'hitstatetimer_In', post_hit_state_out_corr; ...
            InAction_corr, 'current_state-1'});
        
        % Added err_value in the same vein as the regular hit_state ~KJM
        
        sma = add_state(sma, 'name', 'err_value', 'self_timer', min_time, ...
            'output_actions', {'SchedWaveTrig', 'hitstatetimer'}, ...
            'input_to_statechange', {'Tup', 'current_state+2'});
        sma = add_state(sma, 'self_timer', min_time, ...
            'output_actions', {'SchedWaveTrig', 'hitstatetimer'}, ...
        ...
        % port:
        sma = add_state(sma, 'input_to_statechange', ...
            {'hitstatetimer_In', post_hit_state_in_incorr ; ...
            OutAction_incorr, 'current_state+1'});
        sma = add_state(sma, 'input_to_statechange', ...
            {'hitstatetimer_In', post_hit_state_out_incorr; ...
            InAction_incorr, 'current_state-1'});
        ...
        sma = add_state(sma, 'output_actions', {'SchedWaveTrig', '-sd_grace'}, ...
            ...
            OutAction_corr,           'current_state+1', ...
            'Cin',                    post_softdt, ...
            OtherSidePokeIn_corr,     post_softdt, ...
            });
        sma = add_state(sma, 'self_timer', min_time, 'output_actions', {'SchedWaveTrig', 'sd_grace'}, ...
            'input_to_statechange', {'Tup',                    'current_state+1' ; });
        sma = add_state(sma, ...
            'input_to_statechange', {'softdrinktimer_In', post_softdt, ...
            InAction_corr,            'current_state-2', ...
            'Cin',                    post_softdt, ...
            OtherSidePokeIn_corr,     post_softdt, ...
            'sd_grace_In',            post_softdt});
        
        % Added softdrink_incorr states for drinking in the incorrect port ~KJM
        
        % Entry to soft_drink_state_in:
        sma = add_state(sma, 'name', 'err_soft_drink_state', ...
            'self_timer', min_time, ...
            'output_actions', {'SchedWaveTrig', 'softdrinktimer'}, ...
            'input_to_statechange', {'Tup', 'current_state+2'});
        ...
        sma = add_state(sma, 'output_actions', {'SchedWaveTrig', '-sd_grace'}, ...
            ...
            OutAction_incorr,                'current_state+1', ...
            'Cin',                           post_softdt, ...
            OtherSidePokeIn_incorr,          post_softdt, ...
            });
        sma = add_state(sma, 'self_timer', min_time, 'output_actions', {'SchedWaveTrig', 'sd_grace'}, ...
            'input_to_statechange', {'Tup',                    'current_state+1' ; });
        sma = add_state(sma, ...
            'input_to_statechange', {'softdrinktimer_In', post_softdt, ...
            InAction_incorr,            'current_state-2', ...
            'Cin',                      post_softdt, ...
            OtherSidePokeIn_incorr,     post_softdt, ...
            'sd_grace_In',              post_softdt});
        ...
        dispatcher('send_assembler', sma, {'hit_state', 'err_value', 'error_state', 'violation_state'});
