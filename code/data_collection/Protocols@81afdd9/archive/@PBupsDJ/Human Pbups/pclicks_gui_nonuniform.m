...
gamma_pool = [-1.15 -0.5 -0.07 0.07 0.5 1.15];
signs = sign(gamma_pool);
target_prob = [0.9 0.7 0.6 0.6 0.75 0.9];
fixed_pool = 0;
pprob = ones(size(gamma_pool));
adapt_step = -0.04;
sample_range = [0.1 0.7];
sample_list = linspace(sample_range(1),sample_range(2),7);
...
%%  Run 1 trial
    function begin_trial()
        ...
        s = next_trial(hit_history, side_history, pprob, antibias_beta, antibias_tau)        
        ...
        bupsdata{rows_j(bupsdata)+1,1} = data;
...
%% stopsave function
    function stop_save()
		...
		gamma_history = gamma_history(1:length(hit_history));
		sample_history = sample_history(1:length(hit_history));
		observer = observer(1:length(hit_history));
		side_history = side_history(1:length(hit_history));
		stereo_history = stereo_history(1:length(hit_history));
        ...
        switch state,
            case 'ITI',
                if strcmp(evnt.Key, 'p')
                    ...
                    state = 'instruct';
                elseif strcmp(evnt.Key,'escape'), stop_save(), close(f);    
                end
			case 'Stim',
				...
				if abs(response) > 0,
					...
					elseif side == response,
                        ...
                        sample_misses(s0,samp0) = sample_hits(s0,samp0)+1;
						punish_state;
					end;
				end;
