% PBupsBen protocol with headfixation
% BBS, DECEMBER 2011

function [obj] = PBupsBen(varargin)
...
obj = class(struct, mfilename, saveload, water, ...
    ...
    distribui, poissonbups2, comments, sqlsummary);
...
%% init
  case 'init'
    ...
    next_row(y);
    % These values control the duration of headfixation and the immersion
    % fluid
    ...
    HeaderParam(obj, 'prot_title', ['PBupsBen: ' expmtr ', ' rname], ...
...
%% prepare next trial
  case 'prepare_next_trial'
    ...
    if n_done_trials==1
        [expmtr, rname]=SavingSection(obj, 'get_info');
        prot_title.value=['PBupsBen - on rig ' get_hostname ' : ' expmtr ', ' rname  '.  Started at ' datestr(now, 'HH:MM')];
    end
