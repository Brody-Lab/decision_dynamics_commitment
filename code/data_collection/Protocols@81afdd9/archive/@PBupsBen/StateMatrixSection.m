% @PBupsBen/StateMatrixSection
% BWB, March 2009
% additional input lines BBS, January 2013
...
switch action
  case 'init', 
     x=210; y=380;
    NumeditParam(obj, 'NosePos', 2000, x, y, 'TooltipString', 'Number of Steps for the Nose Poke stage')
         x=210; y=400;
...
happSpec = cell2struct(hs, fnames, 2);
     sma = StateMachineAssembler('full_trial_structure', 'use_happenings', 1, 'default_happSpec', happSpec, 'n_input_lines', 6); %

     min_time= 2.5E-4;  % This is less than the minumum time allowed for a state transition.
    
    %center1water        = Settings('get', 'DIOLINES', 'center1water');  %12V TTL
    left1water          = Settings('get', 'DIOLINES', 'left1water');    %12V TTL
    right1water         = Settings('get', 'DIOLINES', 'right1water');   %12V TTL
    center1led          = Settings('get', 'DIOLINES', 'center1led');    %5V TTL
    left1led            = Settings('get', 'DIOLINES', 'left1led');      %5V TTL
    right1led           = Settings('get', 'DIOLINES', 'right1led');     %5V TTL
    pistons1            = Settings('get', 'DIOLINES', 'A12');      %12V TTL
    pistons2            = Settings('get', 'DIOLINES', 'B12');      %12V TTL
    CW                  = Settings('get', 'DIOLINES', 'A5');       %5V TTL 
    CCW                 = Settings('get', 'DIOLINES', 'B5');          %5V TTL

 
    if n_done_trials==0
            %--------------Stage Commands-------------------------------------
    sma = add_scheduled_wave(sma, 'name', 'forward', 'preamble', 0, 'sustain', .001,'DOut', CW, 'loop', -1 );
    sma = add_scheduled_wave(sma, 'name', 'backward', 'preamble', 0, 'sustain', .001,'DOut', CCW, 'loop', 5000);
    
        %-------------- Trial Structure & Behavioral Input ----------------------------------------   
    ...
    sma = add_state(sma, 'name', 'Stop', 'self_timer', 0.001, ...
     ...
     sma = add_state(sma, 'name', 'MoveBack', 'self_timer', 15, ...
        'output_actions', {'SchedWaveTrig','+backward'}, ...
        'input_to_statechange', {'Tup', 'current_state+1'});
       
   dispatcher('send_assembler', sma, 'MoveBack');
    state_machine=dispatcher('get_state_machine');
    send(sma, state_machine, 'use_default_input_lines', 'sm = SetInputEvents(sm, 12, ''ai'')'); % ************  Sixth INPUT LINE NEEDS THIS: the number should be twice the number of
    n_done_trials=1; 
    end
...
%% set up reward quantity and timing
    ...
    LeftWValveTime = WaterValvesSection(obj,'get_left_time');
    CenterWValveTime = WaterValvesSection(obj,'get_center_time');
    RightWValveTime = WaterValvesSection(obj,'get_right_time');
...
%% set up correct responses
    if SidesSection(obj, 'get_current_side') == 'l',
        ...
        switch value(side_lights),
          ...
          otherwise
            warning('PBupsBen:UnknownSetting', 'Don''t know what to do with the side_lights setting');
        end;
        correct_side_led   = left1led; %#ok<NASGU>
        correct_side_water = left1water;
        correct_side_valvetime = LeftWValveTime*left_wtr_mult;
    else
        if strcmp(value(reward_timing), 'from spoke'),
        ...
        correct_side_valvetime = RightWValveTime*right_wtr_mult;
    end;
...
%% set up headfixation, stimuli and scanning  

                             
        sma = add_scheduled_wave(sma, 'name', 'pistons1', ...
                                    'preamble', 0, ...
                                    'sustain',2, ...
                                    'DOut', pistons1);
                          
        sma = add_scheduled_wave(sma, 'name', 'pistons2', ... 
                                    'preamble', 0.4, ...
                                    'sustain', 2, ...
                                    'DOut', pistons2);
                                  
        sma = add_scheduled_wave(sma, 'name', 'center_water', ...
                                      'preamble', 0 ,...
                                      'sustain', Value(CenterWValveTime)*0.5,...
                                      'refraction', 1,...
                                      'loop', 3,...
                                      'DOut', center1water); 
	...
	waves_on  = '+sound_on +min_rt_wave +pistons1 +pistons2';
	waves_off = '-sound_on  -min_rt_wave -pistons1 -pistons2';
    waves_off_cleanup = '-sound_on -min_rt_wave -pistons1 -pistons2';
...
'cpoke_timer_lo'}, ...
 ...
 'wave_low'}, ...
       'inputNumber',          {get_wavenumber(sma, 'cpoke_timer'),
get_wavenumber(sma, 'cpoke_timer')}));
...
%% reward states
	...
	sma = add_state(sma, 'name', 'left_reward', 'self_timer', locked_snd_dur, ...
		'output_actions', {'SchedWaveTrig', ['direct_reward' swaves_to_add]; ...
		                   'SoundOut', locked_snd}, ...
...
%% clean up                        
    ...
    state_machine=dispatcher('get_state_machine');
    send(sma, state_machine, 'use_default_input_lines', 'sm = SetInputEvents(sm, 12, ''ai'')');
%% reinit
  case 'reinit',
