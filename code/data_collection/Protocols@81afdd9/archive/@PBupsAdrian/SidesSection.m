...
%% make_and_send_summary
  case 'make_and_send_summary',
    ...
    pd.reward_delay = cell2mat(rd);      pd.reward_delay = pd.reward_delay(1:length(hit_history));
...
%% psych_summary
  case 'psych_summary',
    ...
    sizeDiff=length(peh)-length(x.sides);
    if sizeDiff~=0
        ...
        if sizeDiff>0     % peh longer than pd    
            if all( (hitsFromPeh(1:end-sizeDiff)==x.hits | isnan(x.hits) ) & violationsFromPeh(1:end-sizeDiff)==x.violations)
                states = states(1:end-sizeDiff);
            elseif all( ( isnan(x.hits) | hitsFromPeh(1+sizeDiff:end)==x.hits ) & violationsFromPeh(1+sizeDiff:end)==x.violations)
                states = states(1+sizeDiff:end); 
            else
                error('Size mismatch between peh and pd which could not be resolved.'); 
            end
        else     % pd longer than peh     
            sizeDiff=-sizeDiff;
            if all( (isnan(x.hits(1:end-sizeDiff)) | hitsFromPeh==x.hits(1:end-sizeDiff)) & violationsFromPeh==x.violations(1:end-sizeDiff)  )
                pdInds2Keep = 1 : (length(x.hits)-sizeDiff);
            elseif all ( (isnan(x.hits(1+sizeDiff:end)) | hitsFromPeh==x.hits(1+sizeDiff:end) ) & violationsFromPeh==x.violations(1+sizeDiff:end) )
                pdInds2Keep = (1 + sizeDiff) : length(x.hits);
            else
                error('Size mismatch between peh and pd which could not be resolved.'); 
            end  
            x.hits = x.hits(pdInds2Keep);
            x.violations = x.violations(pdInds2Keep);
            x.samples = x.samples(pdInds2Keep);
            x.sides = x.sides(pdInds2Keep); 
            x.bupsdata = x.bupsdata(pdInds2Keep);  
            x.choices=x.choices(pdInds2Keep);
