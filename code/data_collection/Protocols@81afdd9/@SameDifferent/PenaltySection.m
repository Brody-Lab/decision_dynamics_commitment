...
%% init
  case 'init',
    ...
    SoloParamHandle(obj, 'mypfig', 'saveable', 0, 'value', ...
        figure('position', [409   316   420   658], ...
            'MenuBar', 'none',  ...
            'NumberTitle', 'off', ...
    ...
    SoundInterface(obj, 'set', 'TempPun', 'Style', 'WhiteNoise', 'Vol', 0.01, 'Dur1', 0.1, ...
      'Loop', 0, 'Bal', 0);
    [x, y] = PunishInterface(obj, 'add', 'RTempPun', x, y); %#ok<NASGU>
    PunishInterface(obj, 'set', 'RTempPun', 'SoundsPanel', 0, 'Duration', 1.5, 'Reinit', 1, ...
      'ReinitGrace', 0.2, 'ReinitPenalty', 0);
    SoundInterface(obj, 'set', 'RTempPun_InitSnd', 'Style', 'WhiteNoise', 'Vol', 0.005, ...
      'Dur1', 0.1);
    SoundInterface(obj, 'set', 'RTempPun_OngoingSnd', 'Style', 'WhiteNoise', 'Vol', 0.001, ...
      'Dur1', 1);
    PunishInterface(obj, 'disable', 'RTempPun');
    ...
    next_row(y);
    NumeditParam(obj, 'wait_for_cpoke_grace', 2, x, y, ...
          'labelfraction', 0.7, ...
          'TooltipString', sprintf(['\nthe duration (sec) at each ''wait_for_cpoke'' state (excepting wait_for_cpoke1) ' ...
                                    '\nthe rat has to make the next poke, otherwise the trial times out and is considered an error.']));
    next_row(y);
    NumeditParam(obj, 'wait_for_spoke_grace', 10, x, y, ...
          'labelfraction', 0.7, ...
          'TooltipString', sprintf(['\nthe duration (sec) at ''wait_for_spoke'' state' ...
                                    '\nthe rat has to make the next poke, otherwise the trial times out and is considered an error.']));
    next_row(y);
    NumeditParam(obj, 'violation_punishment', 30, x, y, 'position', [x y 160 20], ...
          'labelfraction', 0.7, ...
          'TooltipString', 'time out (in sec) upon either spoke or cpoke violation');
	ToggleParam(obj, 'repeat_trial_after_violation', 1, x, y, 'position', [x+160 y 40 20], ...
		  'OffString', 'New', ...
		  'OnString',  'Rep', ...
		  'TooltipString', sprintf(['\nIf ON, then the trial following a violation trial is identical to the previous;' ...
                                    '\nin other words, the exact same stimulus is played.' ...
                                    '\nIf OFF, then the trial following a violation trial is computed as normal,' ...
                                    '\nso the training string in sessiondefinition will be evaluated and the ' ...
                                    '\nnew trial type determined anew.']));
    next_row(y, 2);
    ...
    ToggleParam(obj, 'PunishSidePokesinWFCO', 0, x, y, ...
      ...
      'TooltipString', sprintf(['\n***This setting only relevant in "have wait for center nose out" mode.***\n' ...
    ...
    ToggleParam(obj, 'WaitForCenterNoseOut', 0, x, y, ...
      ...
      'TooltipString', sprintf(['\n***This setting only relevant in "one poke w/legal cbreaks" mode.***\n' ...
    ...
    SoloFunctionAddVars('StateMatrixSection', 'ro_args', ...
          ...
          'wait_for_cpoke1_tout' ; 'wait_for_cpoke_grace'; 'wait_for_spoke_grace'; ...
          'miss_ITI'; 'violation_punishment' ; 'TempPunChoice' ; 'LEDtemp_pun' ; ...
          'PunishSidePokesinWFCO' ; 'WaitForCenterNoseOut'});
    
	SoloFunctionAddVars('RewardsSection', 'ro_args', ...
		  {'repeat_trial_after_violation'});
  ...
  case 'TempPunChoice',
    switch(value(TempPunChoice)),
      case 'reinit-able temp pun',
        PunishInterface(obj, 'enable', 'RTempPun');
        SoundInterface(obj, 'disable_all', 'TempPun');

      case 'non-reinit temp pun',
        PunishInterface(obj, 'disable', 'RTempPun');
        PunishInterface(obj, 'set', 'RTempPun', 'SoundsPanel', 0);
        SoundInterface(obj, 'enable_all', 'TempPun');
