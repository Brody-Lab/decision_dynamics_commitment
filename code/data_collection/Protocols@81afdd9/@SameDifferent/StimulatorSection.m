...
% ----------------------------------------------------------------
  ...
  case 'init', 
      ...
      ToggleParam(obj, 'StimShow', 0, x, y, 'OnString', 'Stim showing', ...
        'OffString', 'Stim hidden', 'TooltipString', 'Show/Hide stim panel'); 
      set_callback(StimShow, {mfilename, 'show_hide'}); %#ok<NODEF> (Defined just above)
      next_row(y);
...
% -----------------------------------------------------------------------     
  ...
  case 'hide',
    StimShow.value = 0; set(value(myfig), 'Visible', 'off');

  case 'show',
    StimShow.value = 1; set(value(myfig), 'Visible', 'on');

  case 'show_hide',
    if StimShow == 1, set(value(myfig), 'Visible', 'on'); %#ok<NODEF> (defined by GetSoloFunctionArgs)
...
case 'get_send_summary_info',
		upto=x;
        clear x;
		sl=get_history(StimLines);
        stim_duration=get_history(PulseWidth);
        stim_delay=get_history(StartDelay);
        stim_state=get_history(StimStates);
        sl=cell2mat(sl);
        stim_duration=cell2mat(stim_duration);
        stim_delay=cell2mat(stim_delay);
        stim_state=cell2mat(stim_state);
        stim_hist=StimulatorSection(SameDifferent,'get','stimulator_history');

%         vsl=zeros(numel(sl),1);
%         for tx=1:numel(sl)
%             tsl=sscanf(sl(tx),'stim%d');
%             if ~isempty(tsl)
%                 vsl(tx)=tsl;
%             end
%         end
        ...
        x.stimline=sl(1:upto);
        x.stimhist=stim_hist(1:upto);
        x.stimdur=stim_duration(1:upto);
        x.stimdel=stim_delay(1:upto);
        x.stimstate=stim_state(1:upto);
        x.stimhist=x.stimhist(:);
        x.stimdur=x.stimdur(:);
        x.stimdel=x.stimdel(:);
        x.stimstate=x.stimstate(:);
        varargout{1}=x;
