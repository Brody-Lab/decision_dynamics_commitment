...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    ...
    sma = add_happening_spec(sma, struct(...
        'name',                 {'bad_nic_wave_hi',                   'bad_nic_wave_lo',                   'Clo',      'Chi'       }, ...
        'detectorFunctionName', {'wave_high',                         'wave_low',                          'line_low', 'line_high',}, ...
        'inputNumber',          {get_wavenumber(sma, 'bad_nic_wave'), get_wavenumber(sma, 'bad_nic_wave'), 1,          1           }));
    ...
    if KeepNoseCenter == 0
        if StimTime == 1
            ...
            sma = add_state(sma, 'name', 'nicstim', 'self_timer',0.001,...
                'output_actions',{'DOut',post_cue; 'SoundOut', SoundManagerSection(obj, 'get_sound_id', 'StimSound')},...
                'input_to_statechange', {'Tup','csgap'});
        end
    else 
        ...
        if StimTime == 1 %stim comes after NIC context period
            sma = add_state(sma, 'name', 'context', 'self_timer', 0.0001,...
              'output_actions', {'DOut',post_cue; 'SchedWaveTrig', 'noseincenter_wave'; 'SoundOut', SoundManagerSection(obj, 'get_sound_id', context_sound)},...
              'input_to_statechange', {'Tup', 'current_state+1'});
        else %stim comes during NIC, no context sound
            sma = add_state(sma, 'name', 'nicstim', 'self_timer', 0.0001,...
              'output_actions', {'DOut',post_cue; 'SchedWaveTrig', 'noseincenter_wave + prestim_wave'},...
              'input_to_statechange', {'Tup', 'current_state+1'});
        end
      
        sma = add_state(sma, ...  % NOSE IN
          'output_actions', {'DOut',post_cue; 'SchedWaveTrig', '-legalcbreak_wave'} ,...  
          'input_to_statechange', {'noseincenter_wave_In',after_nic_comp    ;...
        ...
        if ExitNICTime == 0                       
            sma = add_state(sma, ...  % NOSE OUT, NIC time up, must Cin before legalcbreak
              ...
              'input_to_statechange', {'Cin',                  after_nic_comp ;...
                                       'legalcbreak_wave_In',  'reinit_cpoke'});
        ...
        sma = add_state(sma, 'name', 'reinit_cpoke', 'self_timer', 0.0001,...
          'output_actions', {'DOut',post_cue; 'SchedWaveTrig', '-noseincenter_wave-memoryzap_wave'} ,...
          'input_to_statechange', {'Tup', 'current_state+1'});
        sma = add_state(sma, 'self_timer', 0.0001,...
          'output_actions', {'DOut',post_cue; 'SoundOut', -SoundManagerSection(obj, 'get_sound_id', context_sound); 'SchedWaveTrig', 'bad_nic_wave'},...
          'input_to_statechange', {'Tup', 'current_state+1'});
        sma = add_state(sma, 'self_timer', 0.0001,...
          'output_actions', {'DOut',post_cue; 'SoundOut', -SoundManagerSection(obj, 'get_sound_id', 'StimSound')},...
          'input_to_statechange', {'Tup', 'current_state+1'});
