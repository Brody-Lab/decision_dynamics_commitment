...
% ----------------------------------------------------------------
  ...
  case 'init', 
      ...
      states_colors = SMASection(obj, 'get_state_colors');
      states = fieldnames(states_colors);
...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
      ...
      stimline = bSettings('get','DIOLINES',value(StimLine));
      ...
      if value(ShuffleValues) == 1
          ...
          if length(unique([length(sd) length(sf) length(pw) length(np)])) > 1
          ...
          sd = sd(temp); sf = sf(temp); pw = pw(temp); np = np(temp);
          ss = ss(ceil(rand(1) * length(ss)));
          sl = sl(ceil(rand(1) * length(sl)));
      end
      ...
      if rand(1) <= value(StimProb)
          ...
          if strcmp(value(StimState),'none') == 0
              if sd ~= 0
                  sma = add_stimulus(sma,'stimulator_wave_pause',value(StimState));
              else
                  sma = add_stimulus(sma,'stimulator_wave',value(StimState));
              end
