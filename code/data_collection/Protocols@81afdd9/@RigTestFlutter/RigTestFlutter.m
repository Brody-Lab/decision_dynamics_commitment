...
function [obj] = RigTestFlutter(varargin)
...
switch action,
  ...
  case 'init'
    ...
    RigTestFlutter(obj,'prepare_next_trial');
  ...
  case 'prepare_next_trial'
      ...
      sma = add_scheduled_wave(sma, 'name', 'trial_wave',    'preamble', 10000, 'sustain', 1);
      sma = add_scheduled_wave(sma, 'name', 'subtrial_wave', 'preamble', 4,   'sustain', 1);
      
      ontime = 0.0005 + (0.0001 * n_done_trials);
      sma = add_scheduled_wave(sma, 'name', 'servo_wave','preamble', 0.020-ontime,'sustain', ontime, 'loop',100,'DOut',bSettings('get','DIOLINES','bnc5'));
%       ontime = 1;
%       offtime= 2;
%       sma = add_scheduled_wave(sma, 'name', 'servo_wave','preamble', offtime,'sustain', ontime, 'loop',10,'DOut',bSettings('get','DIOLINES','bnc5'));
      %if ontime > 0.0025
      %    keyboard;
      %end
      
      sma = add_happening_spec(sma, struct(...
        'name',                 {'trial_wave_hi',                 'trial_wave_lo',                   'subtrial_wave_hi',              'subtrial_wave_lo'}, ...
        'detectorFunctionName', {'wave_high',                     'wave_low',                        'wave_high',                     'wave_low'}, ...
        'inputNumber',          {get_wavenumber(sma,'trial_wave'),get_wavenumber(sma,'trial_wave'),get_wavenumber(sma, 'trial_wave'),get_wavenumber(sma, 'trial_wave')}));
      
      sma = add_state(sma, 'name', 'first_state', 'self_timer',0.01,...
          'output_actions',{'SchedWaveTrig', 'trial_wave'},...
          'input_to_statechange', {'Tup', 'state_a'});
      
      sma = add_state(sma, 'name', 'state_a', 'self_timer',10,...
          'output_actions',{'SchedWaveTrig', 'servo_wave'},...
          'input_to_statechange', {'Tup', 'final_state'; 'trial_wave_hi', 'final_state'});
      
      %sma = add_state(sma, 'name', 'state_b', 'self_timer',0.0001,...
      %    'input_to_statechange', {'Tup', 'state_c'; 'trial_wave_hi', 'final_state'; 'subtrial_wave_hi','state_d'});
      %
      %sma = add_state(sma, 'name', 'state_c', 'self_timer',0.0001,...
      %    'input_to_statechange', {'Tup', 'state_b'; 'trial_wave_hi', 'final_state'; 'subtrial_wave_hi','state_d'});
      %
      %sma = add_state(sma, 'name', 'state_d', 'self_timer',20.3,...
      %    'input_to_statechange', {'Tup', 'state_a'; 'trial_wave_hi', 'final_state'});
