...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    ...
    LL = PsychSection(obj,'get_lastleft');
    FR = PsychSection(obj,'get_firstright');
      
    posterior = AntibiasSection(obj, 'get_posterior_probs');
    pleft = sum(posterior(1:LL));
    ...
    if n_done_trials > value(MaxWithout)
        ...
        for t = 1:10
            ...
            if wo >= value(MaxWithout) && ~isempty(find(g == t,1)) %#ok<EFIND>
                forcetrial.value = t; disp(t)
                if t <= LL; ThisTrial.value = 'LEFT';
    ...
    if value(Probe_Trials) == 1 
        if rand(1) < value(Percent_Probe_Trials)
            if all(isnan(value(Probe_Delays)))
                forcetrial.value = ceil(rand(1) * 8) + 1;
            else
                PDelays = value(Probe_Delays);
                Locked_VarGap2_Offset.value = PDelays(ceil(rand(1) * length(PDelays)));
            end
        else
            Locked_VarGap2_Offset.value = 0;
        end
    end
    ...
    if isempty(value(forcetrial))
        ...
        if value(forcetrial) <= LL; ThisTrial.value = 'LEFT';
        else                        ThisTrial.value = 'RIGHT';
        end
    end
    ...
    if isequal(ThisTrial, 'LEFT'),
      sides_history.value = [oldsides 'l']; 
    else
      sides_history.value = [oldsides 'r']; 
    end;
...
% -----------------------------------------------------------------------
  ...
  case 'update_plot',
    ...
    if length(ph) < n_done_trials
        temp = value(sides_history); %#ok<NODEF>
        temp = temp(1:end-1);
    ...
    for t = 1:10
        ...
        plot(ax, blacks(ph(blacks)==t), ones(length(blacks(ph(blacks)==t)),1) * yvals(t), 'k.');
    end
