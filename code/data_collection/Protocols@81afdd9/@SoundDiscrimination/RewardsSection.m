...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    if n_done_trials >= 1,
        ...
        elseif size(parsed_events.states.prereward,1) >= 1
        ...
        if isfield(parsed_events.states,'reinit_cpoke')
            if sum(isnan(value(hit_history))) > 0
                if isnan(hit_history(end)); cpoketry.value = [cpoketry(:) ; 0]; %#ok<NODEF>
                else                        cpoketry.value = [cpoketry(:) ; 1]; %#ok<NODEF>
                end
                Average_Cpoke_Trys.value   = nan;
                Percent_Single_Cpoke.value = mean(value(cpoketry));
                    
            else
                cpoketry.value = [cpoketry(:) ; size(parsed_events.states.reinit_cpoke,1)]; %#ok<NODEF>
                Average_Cpoke_Trys.value   = mean(value(cpoketry)) + 1;
