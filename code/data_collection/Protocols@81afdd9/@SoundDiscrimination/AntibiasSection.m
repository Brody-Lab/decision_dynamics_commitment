...
switch action
  ...
  case 'update_biashitfrac',     % ------- CASE UPDATE_BIASHITFRAC -------------
    ...
    pth            = PsychSection(obj,'get_psychtypehist');
    ph             = ph( 1:length(l_hit_history));
    %ns             = PsychSection(obj,'get_numpsych');
    LL             = PsychSection(obj,'get_lastleft');   if isempty(LL); LL = 5; end
    FR             = PsychSection(obj,'get_firstright'); if isempty(FR); FR = 6; end    
    pt             = PsychSection(obj,'get_psychtype');
    
    if     isequal(pt,'Duration');  pt = 1;
    elseif isequal(pt,'Frequency'); pt = 2;
    else                            pt = 3;
    end
    
    if length(pth) >= length(l_hit_history); 
        pth = pth(1:length(l_hit_history));
        l_hit_history(pth ~= pt) = [];
        ph(           pth ~= pt) = [];
    else
        pth = ones(size(l_hit_history)) * pt;
    end
    
    n_done_type_trials = sum(pth == pt);
...
%     end
    ...
    PriProb         = zeros(1,10);
    PriProb(1:LL)   = (1/LL) * LeftProb;
    PriProb(FR:end) = (1/(10-FR+1)) * RightProb;
    
    if n_done_type_trials < BiasTau
        
        pad = int16(BiasTau - n_done_type_trials);
        ...
        for k = 1:10
            ...
            else              BiasHitFrac(k) = nansum(l_hit_history(khit).*kernel(khit))/sum(kernel(khit));
            end;
        end;
...
%         if isempty(ur) && ~isempty(ul), BiasRtHitFrac.value = value(BiasLtHitFrac); end;
    ...
    LtProb.value = sum(choices(1:LL));
    RtProb.value = sum(choices(FR:10));
  ...
  case 'update_hitfrac',     % ------- CASE UPDATE_HITFRAC -------------
    ...
    if length(l_hit_history)>0, 
      ...
      kernel = kernel(end:-1:1);
      HitFrac.value = nansum(l_hit_history .* kernel)/sum(kernel);
    
      sh = l_sides_history(1:length(l_hit_history))';
      u = find(sh == 'l');
      if isempty(u), LtHitFrac.value = NaN;
      else           LtHitFrac.value = nansum(l_hit_history(u) .* kernel(u))/sum(kernel(u));
      end;
        
      u = find(sh == 'r');
      if isempty(u), RtHitFrac.value = NaN;
      else           RtHitFrac.value = nansum(l_hit_history(u) .* kernel(u))/sum(kernel(u));
      end;
    end;
