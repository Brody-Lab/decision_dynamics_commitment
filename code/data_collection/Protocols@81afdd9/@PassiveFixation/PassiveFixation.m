% protocol to get the transgenics to passively fixate in the center port.
% cmc 11/23/15

function [obj] = PassiveFixation(varargin)
...
switch action,
   ...
   case 'init'
      ...
      %   ----------------------
      x = 5; y = 50; maxy=50;     % Initial position on main GUI window
     ...
     % [x, y] = HFWaterAdaptorSection(obj, 'init', x, y); 
      ...
      HeaderParam(obj, 'prot_title', ['PassiveFixation: ' expmtr ', ' rname], ...
         x, y, 'position', [10 figpos(4)-25, 800 20]);
      
      PassiveFixation(obj, 'prepare_next_trial');
