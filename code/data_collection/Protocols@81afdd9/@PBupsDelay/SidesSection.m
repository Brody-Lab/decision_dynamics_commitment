...
%% init
  case 'init',
    ...
    SoloParamHandle(obj, 'previous_sdelays', 'value', 0); % delay before stimstart
    ...
    ToggleParam(obj, 'LeftRightOnly', 0, x, y, 'position', [pos(3)-720 pos(4)-180 100 20], ...
...
%% update_last_trial_info
  case 'update_last_trial_info'
	if n_done_trials == 0,
		...
		if violation_history(n_done_trials) == 1,
			...
			if strcmp(specs.stim_timing, 'During Cpoke Only'),
                memory_duration = 0;
                delay_duration = specs.sound_delay;
                ...
                memory_duration = specs.memory_delay;
                delay_duration = specs.sound_delay;
    ...
    %             else
        ...
        previous_sdelays.value = [previous_sdelays(:); delay_duration]; %#ok<NODEF>
...
%% make_and_send_summary
  case 'make_and_send_summary',
    ...
    pd.sdelays    = value(previous_sdelays);    pd.sdelays = pd.sdelays(1:length(hit_history));
