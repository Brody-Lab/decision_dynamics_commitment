...
%% init    
  case 'init',
      ...
      MenuParam(obj, 'center_light', {'off', 'on', 'off at first cpoke', 'on at first cpoke'}, 2, ...
    ...
    DispParam(obj, 'memory_gap', '0', x, y, 'position', [x y 200 20], ...
      'TooltipString', 'displays the duration of delay after stimulus ends for the current trial'); 
    next_row(y);


	  DistribInterface(obj, 'add', 'memory_gap_dist', x+25, y, 'Style',...
      'exponential', 'Tau', 0, 'Min', 0, 'Max', 0,...
	  ...
	  next_row(y);
       ...
       DispParam(obj, 'stim_start_delay', '0', x, y, 'position', [x y 200 20], ...
      'TooltipString', 'displays stim_start_delay for the current trial'); 
    next_row(y);

      DistribInterface(obj, 'add', 'stim_start_delay_dist', x+25, y, 'Style', ...
          'uniform', 'Min', 0, 'Max', 0.3, ...
...
%% next_trial
  case 'next_trial',      
      ...
      if rand(1) < value(prob_mem_gap) && strcmp(value(stim_timing),'Fixed T After Start')
          memory_gap.value = DistribInterface(obj, 'get_new_sample', 'memory_gap_dist');
      else
          memory_gap.value = 0;
      end
      
      % setting stim_start_delay based on specified parameters
      stim_start_delay.value = DistribInterface(obj, 'get_new_sample', 'stim_start_delay_dist');
      
      % recomputing NIC time
      nose_in_center.value = max(0, value(stim_start_delay) + this_T+ value(memory_gap));
   
       % sets up the stimulator, uses the other three arguments only if one of the codes are used 
    PBupsSection(obj, 'next_trial_stimulator', value(stim_start_delay), this_T, value(memory_gap));
	  ...
	  specs.stim_timing = value(stim_timing);
    specs.memory_delay = value(memory_gap);
