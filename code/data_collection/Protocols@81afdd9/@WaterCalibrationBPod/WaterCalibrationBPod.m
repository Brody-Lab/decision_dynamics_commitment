%WaterCalibrationBPod is a protocol desgined to calibrate water delivery to
%behavior boxes.  
%
%Chuck Kopec - October 2011

function [obj] = WaterCalibrationBPod(varargin)
...
switch action
    ...
    case 'init'        
        ...
        if ~isnan(bSettings('get','INACTIVEDIOLINES',['left',doside,'water'])) ||...
            isnan(bSettings('get',        'DIOLINES',['left',doside,'water']))
        ...
        if ~isnan(bSettings('get','INACTIVEDIOLINES',['center',doside,'water'])) ||...
            isnan(bSettings('get',        'DIOLINES',['center',doside,'water']))
        ...
        if ~isnan(bSettings('get','INACTIVEDIOLINES',['right',doside,'water'])) ||...
            isnan(bSettings('get',        'DIOLINES',['right',doside,'water']))
        ...
        MenuParam(obj,'User', names, 1,0,0,'position',[310 405 130 35],'labelfraction',0.02);
        ...
        %Going to use this to keep the program in a loop until the user
        %clicks enter, trying to eliminate the need to start and stop
        %dispatcher each cycle
        SoloParamHandle(obj,'EnterClicked','value',0);
        
        %These variables will be used to inform the user of how much time
        %is remaining in the cycle since we no longer can count states
        SoloParamHandle(obj,'StartTime','value',0);
        SoloParamHandle(obj,'TotalDuration','value',0);
        ...
        WaterCalibrationBPod(obj,'update_table');
...
%         cd(currdir);
        ...
        WaterCalibrationBPod(obj,'update_estimate');
        ...
        WaterCalibrationBPod(obj,'send_empty_state_machine');
    ...
    case 'Flush_callback'
        ...
        if all([value(LeftCup),value(CenterCup),value(RightCup)] == 0) %#ok<NODEF>
            WaterCalibrationBPod(obj,'empty_cup_weights');
        end
    ...
    case 'Enter_callback'
        ...
        rigid = str2num(value(RigID)); %#ok<ST2NM>
    ...
    case 'update_preference_values'
        ...
        %file in the WaterCalibrationBPod directory
...
%         end
        ...
        WaterCalibrationBPod(obj,'disable_buttons');
        dispatcher('send_assembler', sma, 'complete');
        
        StartTime.value = now;
        TotalDuration.value = (value(LeftCurrent)+value(CenterCurrent)+value(RightCurrent)+(value(InterPulseInt)*(sum(valves))))*value(PulseNumber);
    ...
    case 'update'
        ...
        if value(Running) == 1 %#ok<NODEF>
            rem_tot = (value(TotalDuration)/60) - ((now - value(StartTime)) * 24 * 60);
            
            rem_min = floor(rem_tot);
            rem_sec = round((rem_tot - rem_min) * 60);
            
            StatusString.value = ['Estimated Time Remaining ',sprintf('%02d',rem_min),':',sprintf('%02d',rem_sec),...
                '.  Please wait...'];
    ...
    case 'prepare_next_trial'
        ...
        WaterCalibrationBPod(obj,'enable_buttons');
        ...
        EnterClicked.value = 0;
        while value(EnterClicked) == 0
            pause(1);
        end
    ...
    case 'update_estimate'
        ...
        for i=1:length(badID); bdata('call invalidate_calibration_point("{Si}")',badID(i)); end
        ...
        for i=1:length(valvenames)
            ...
            if sum(strcmp(dts,datestr(now,29))) > 0
                ...
                for j=1:length(badID); bdata('call invalidate_calibration_point("{Si}")',badID(j)); end
            end
        end
        ...
        if any(completed(:) == 0)
            for i = 1:length(valvenames)
                ...
                if ESTIMATE(i) == 0
                    ...
                    if p(1) <= 0
                        StatusString.value = ['ERROR: WaterCalibrationBPod has estimated a negative slope.',...
            ...
            %we're done.
            WaterCalibrationBPod(obj,'invalidate_all_nontarget_entries');
        end
        ...
        if any(ESTIMATE > 0.5) && dataold == 0 && isearly == 1
            StatusString.value = ['ERROR: WaterCalibrationBPod has estimated a valve open time greater then 0.5s.',...
    ...
    case 'invalidate_all_nontarget_entries'
        ...
        bad = ID(good == 0);
        for i = 1:length(bad); bdata('call invalidate_calibration_point("{Si}")',bad(i)); end 
    ...
    case 'Close_callback'
        ...
        try
            ...
            WaterCalibrationBPod(obj,'close');
        end
        dispatcher('close');
