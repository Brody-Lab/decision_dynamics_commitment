% @PBupsDyn/SideSection
% Alex Piet, Ahmed El Hady, August 2015
% Modified from @PBups by Bing
%
...
function [x, y] = SidesSectionDyn(obj, action, x, y)
...
%% init
  case 'init',
    ...
    newaxes = axes;
    SoloParamHandle(obj, 'myaxes', 'saveable', 0, 'value', double(newaxes));
    set(value(myaxes), 'Units', 'pixels');
    set(value(myaxes), 'Position', [90 pos(4)-140 pos(3)-130 100]);
    set(value(myaxes), 'YTick', [0 1], 'YLim', [-0.5 1.5]);       
...
%% next_trial
  case 'next_trial',
	...
	if LR_Beta > 0,
		lr_pprob = PBupsDynSection(obj, 'get', 'pprobs');
	...
	this_side = '';
	% Check for MaxSame rules if it applies. MaxSame prevents a larger number of "left" or "right" trials in a row
    ...
    % We want to reward based on the discounted click difference, not the generative gamma
	[this_sound this_T] = StimulusSectionDyn(obj, 'next_trial', this_side);
 	ThisSound.value = this_sound; %now only stores correct answer. 
    trial_ans = PBupsDynSection(obj,'get_correct_answer');
    if trial_ans == 0
...
%% update_trial_info
  case 'update_trial_info'
    ...
    if n_done_trials == 0,
        ...
        previous_stims.value   = PBupsDynSection(obj, 'get_ThisStim');
    elseif n_done_trials > 0,
        ...
        previous_stims.value   = [previous_stims(:); PBupsDynSection(obj, 'get_ThisStim')];
	end
  ...
  case 'update_last_trial_info'
	if n_done_trials == 0,
		...
		if violation_history(n_done_trials) == 1,
			...
			specs = StimulusSectionDyn(obj, 'get_stimulus_specs');
...
% Save optimal accumulator, and generative state
  case 'make_and_send_summary',
    ...
    pd.bupsdata   = PBupsDynSection(obj, 'get_all_bup_times');
    pd.stimdata   = PBupsDynSection(obj, 'get_all_stimulator_specs');
...
%% update_plot
  case 'update_plot',
	...
	sides = PBupsDynSection(obj, 'get', 'all_sides');
	sides = cellstr(sides');  % turns it into a cell array of characters where each row is a trial type
	nleft = PBupsDynSection(obj, 'get', 'nleft');
	nright = PBupsDynSection(obj, 'get', 'nright');
    ...
    set(get(value(myaxes), 'YLabel'), 'String', 'STATES');
    drawnow;
