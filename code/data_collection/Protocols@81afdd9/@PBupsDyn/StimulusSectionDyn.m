% @PBupsDyn/StimulusSectionDyn.m
% Alex Piet, Ahmed El Hady, August 2015
% Modified from PBups by Bing, February 2009
...
function [x, y] = StimulusSectionDyn(obj, action, varargin)
...
%% init    
  case 'init',
      ...
      SoloParamHandle(obj, 'nic_end_sound_fig', 'value', ...
        double(figure('Position', [cfpos(1)+487 cfpos(2)+289 218 170], 'Name', 'nic_end_sound', ...
      ...
      NumeditParam(obj, 'nic_match_stim', 0, x, y, ...
          'TooltipString', sprintf(['If 0, then NIC and stim_dur are controlled independently.'...
                    '\n  If 1, then NIC is always pinned to stim_dur, with no stim_start_delay, or memory gap']));
      next_row(y);          
      [x, y] = PBupsDynSection(obj, 'init', x, y);
      ...
      SoloFunctionAddVars('StateMatrixSectionDyn', 'ro_args', ...
      ...
      SoloFunctionAddVars('SidesSectionDyn', 'ro_args', {'rt_task'});
...
%% next_trial
  case 'next_trial',      
	  ...
	  [previous_sides previous_sounds] = SidesSectionDyn(obj, 'get_previous_sides_and_sounds');
	  [this_sound this_T] = PBupsDynSection(obj, 'next_trial', set_side, previous_sides, previous_sounds);
      ...
      if rand(1) < value(prob_mem_gap) && strcmp(value(stim_timing),'Fixed T After Start')
          ...
          disp('WARNING: memory gap may interefere with optimal accumulation');
      else
          this_memory_gap = 0;
      end
      ...
      % stimulus duration
      if value(rt_task)
          disp('WARNING: rt_task is not set up for dynamic clicks');
          stim_start_delay.value = DistribInterface(obj, 'get_new_sample', 'rt_stim_delay');
      elseif value(nic_match_stim) == 1
          stim_start_delay.value = 0; %%%%% 10/2017 fix ---------------------------------------------------
      else
      ...
      elseif value(nic_match_stim) == 1
          nose_in_center.value = this_T; %%%%% 10/2017 fix ---------------------------------------------------
      end 
	  
	  x = this_sound; % CORRECT ANSWER 0 = left, 1 = right
	  y = this_T;
	  ...
	  switch value(stim_timing),
		  case 'Fixed T After Start',
              ...
              disp('Warning, Fixed T after Start may not work with dynamic clicks')
...
%% get_stimulus_specs
  case 'get_stimulus_specs'
      ...
      y = PBupsDynSection(obj, 'get_stimulator_specs');
