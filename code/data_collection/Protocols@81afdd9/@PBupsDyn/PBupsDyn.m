% Dynamic PBups protocol
% Alex Piet, Ahmed El Hady, August 2015
%
% Modified from PBups BWB, February 2009


function [obj] = PBupsDyn(varargin)

% Default object is of our own class (mfilename); 
% we inherit only from Plugins

%% Call new pclicks plugin instead of poissonbups2
obj = class(struct, mfilename, saveload, water, wateradaptor, ...
    pokesplot, sessionmodel, soundmanager, soundui, punishui, ...
    distribui, dynbups, comments, sqlsummary);
...
%% init
  case 'init'
    ...
    SoloFunctionAddVars('RewardsSectionDyn', 'rw_args', {'hit_history'; 'violation_history'});
    ...
    [x, y] = SidesSectionDyn(obj, 'init', x, y);
    
    maxy = max(y, maxy); next_column(x); y=5;
    
    % COLUMN 2
    [x, y] = RewardsSectionDyn(obj, 'init', x, y);
    
    maxy = max(y, maxy); next_column(x); y=5;

    % COLUMN 3
    [x, y] = StimulusSectionDyn(obj, 'init', x, y);
        

    maxy = max(y, maxy); next_column(x); y=5;
        
    % COLUMN 4
    [x, y] = PenaltySectionDyn(obj, 'init', x, y);
    ...
    StateMatrixSectionDyn(obj, 'init');
...
%% prepare next trial
  case 'prepare_next_trial'
    ...
    RewardsSectionDyn(obj, 'next_trial');
	
	% do sample duration accounting on the just finished trial
	SidesSectionDyn(obj, 'update_last_trial_info');

	SessionDefinition(obj, 'next_trial');
 	SidesSectionDyn(obj, 'next_trial');
	SidesSectionDyn(obj, 'update_plot');
	SoundManagerSection(obj, 'send_not_yet_uploaded_sounds');
	StateMatrixSectionDyn(obj, 'next_trial');
...
%% update
  case 'update'
    ...
    RewardsSectionDyn(obj, 'update');

    
%% close
  case 'close'
    PenaltySectionDyn(obj, 'close');
    StimulusSectionDyn(obj, 'close');
    PokesPlotSection(obj, 'close');
    WaterAdaptorSection(obj, 'close');
    RewardsSectionDyn(obj, 'close');
...
%% end_session
  case 'end_session'
     ...
     StimulusSectionDyn(obj, 'end_session');  
...
%% pre_saving_settings
  case 'pre_saving_settings'
    ...
    SidesSectionDyn(obj, 'make_and_send_summary');
    sendtrial(obj);
