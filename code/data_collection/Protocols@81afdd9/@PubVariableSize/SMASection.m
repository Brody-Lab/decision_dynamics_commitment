...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    ...
    for i = 1:numel(line_names)
        hapnames{end+1} = [line_names(i),'hi']; %#ok<AGROW>
        poknames{end+1} = [line_names(i),'in']; %#ok<AGROW>
        
        names{    end+1} = [line_names(i),'hi']; %#ok<AGROW>
        functions{end+1} = 'line_hi'; %#ok<AGROW>
    ...
    %names     = {'trial_wave_hi','cycle_wave_hi','lhi',      'chi',      'rhi',      'lin',    'cin',    'rin',    'Xhi',      'Xin'};
    %functions = {'wave_high',    'wave_high',    'line_high','line_high','line_high','line_in','line_in','line_in','line_high','line_in'};
    %numbers   = {twn,            cwn,            4,          5,          6,          4,        5,        6,        7,          7};
...
%% manual_test               
  case 'manual_test'
    ...
    line_names = value(LineNames);
    line_names(find(isnan(waterdio)==1)) = ''; %#ok<FNDSB>
