...
switch action,
    ...
    case 'init'
        ...
        ToggleParam(obj, 'error_forgiveness_dir', 0, x, y, ...
            'OffString', 'Forgive OFF', ...
            'OnString',  'Forgive ON', ...
            'TooltipString', 'If on (black), start new trial upon wrong side choice; if off (brown), can choose again',...
            'position', [x y 100 20]);
        NumeditParam(obj, 'wait_delay_dir', 3, x,y,'label','delay','position', [x+100 y 100 20]);next_row(y);
        
        
        ToggleParam(obj, 'helper_lights_dir', 0, x, y, ...
            'OffString', 'Helper lights off', ...
            'OnString',  'Helper lights on', ...
            'TooltipString', 'If on (black), LED lights help indicate what to do; if off (brown), no helper LED lights');
        next_row(y);
...
%             x, y, 'TooltipString', 'Maximum number of times the same side (L or R) can appear');next_row(y);       
        ...
        ToggleParam(obj, 'incoh_boost_toggle', 0, x,y,...
            'OnString', 'Incoh Boost ON','OffString', 'Incoh Boost OFF',...
            'TooltipString', sprintf('If on incoh boost is on'),'position', [x y 100 20]);
        DispParam(obj, 'incoh_prob', 0.5, x, y, 'labelfraction', 0.6, 'position', [x+100 y 100 20]);next_row(y);
        ...
        NumeditParam(obj, 'wait_for_spoke_timeout', 10, x,y,'label','Timeout for spoke','TooltipString','Time after NIC to wait for a side poke');
        next_row(y);
        ...
        SoloFunctionAddVars('SMA1', 'ro_args',{'settling_time';...
            ...
            'error_forgiveness_dir';'error_forgiveness_freq';...
            'wait_delay_dir'; 'wait_delay_freq'; 'error_delay';...
    ...
    case 'next_trial',
        ...
        if(strcmp(value(ThisTask),'Direction'))
            ...
            if(value(nTrials_task)<=value(initial_pure_trials) && value(task_switch_auto)==1)
                crosstalk_freq=1;
            else
        ...
        val=value(correct_incoh_last10);
        if(value(incoh_boost_toggle) && ~isnan(val))
            
            %map %hit incoh onto %incoh on new trial
            b=6*(val*2-1);
            c=(1./(1+exp(-b)));
            val2=.9;
            c=val2-c*(val2-0.5);
            ...
            incoh_prob.value=0.5;
        end
        
        
        
        %%% decide whether the current trial is incoherent
        if(rand(1) > value(incoh_prob)) % not incoherent
        ...
        %%% if stimuli aren't "pure", the trial is not really incoherent
        if(crosstalk_freq>0.0001 || crosstalk_dir>0.0001 )            
            incoherent_trial.value=0;
        end
    ...
    case 'get_delay_incoh'
        ...
        flag2=(strcmp(value(ThisTask),'Direction') && value(dir_modulation_only)==0) ||...
               (strcmp(value(ThisTask),'Frequency') && value(freq_modulation_only)==0);
