...
function [obj] = ProAnti2(varargin)
...
switch action,
    ...
    case 'init'
        ...
        rand('twister', rem(now*1000,1)*10000);
        ...
        % any loading of data / settings. We can use that to do any updates we may want.
        % Note that Prepare_next_trial is run after loading of settings when
        % choosing between after_load_callbacks and after_load_data
        SoloParamHandle(obj, 'after_load_callbacks', 'value', [], 'save_with_settings', 1);
        set_callback(after_load_callbacks, {mfilename, 'after_load_callbacks'});
        set_callback_on_load(after_load_callbacks, 1);
        
        % We use the following to generate a call that will occur after
        % any loading of data (not settings). We can use that to do any updates we may want.
        SoloParamHandle(obj, 'after_load_data', 'value', []);
        set_callback(after_load_data, {mfilename, 'after_load_data'});
        set_callback_on_load(after_load_data, 1);
        ...
        HeaderParam(obj, 'prot_title', ['ProAnti2: ' expmtr ', ' rname] , ...
    ...
    case 'prepare_next_trial'
        ...
        if n_done_trials==1
            ...
            [expmtr, rname]=SavingSection(obj, 'get_info');
            prot_title.value=[mfilename ': ' expmtr ', ' rname  '.  Started at ' datestr(now, 'HH:MM')];
    ...
    case 'pre_saving_settings',
        ...
        %Runs after loading of settings or data.
        %Note that Prepare_next_trial is run after loading of settings.
        
    case 'after_load_callbacks',
        if SoftPokeStayInterface(obj, 'get', 'soft_drink_time_aftertemperror', 'Grace') == 2.401,
            newduration = SoftPokeStayInterface(obj, 'get', 'soft_drink_time', 'Duration');
            newgrace    = SoftPokeStayInterface(obj, 'get', 'soft_drink_time', 'Grace');
            SoftPokeStayInterface(obj, 'set', 'soft_drink_time_aftertemperror', 'Duration', newduration, 'Grace', newgrace);
        end;
        
        %% CASE AFTER_LOAD_DATA
        %Runs only after loading of data.
