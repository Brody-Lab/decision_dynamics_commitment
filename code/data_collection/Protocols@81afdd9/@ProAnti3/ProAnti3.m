...
function [obj] = ProAnti3(varargin)
...
%% INIT 
  case 'init'
    ...
    set(value(myfig), 'Position', [303 40 850 700]);
    ...
    %CRASH HANDLING: Some local variables
    ...
    NumeditParam(obj, 'Anti_R_Perf', 0, x, y, 'position',[x+100 y  100 20], 'labelfraction', 0.6); next_row(y);
    
    NumeditParam(obj, 'Average_Cpoke_Trys',   0, x, y, 'labelfraction', 0.6); next_row(y);
    NumeditParam(obj, 'Percent_Single_Cpoke', 0, x, y, 'labelfraction', 0.6); next_row(y);
    ...
    SoloParamHandle(obj, 'SoundPanel', 'value', double(figure('Position', [ 50 40 210 700], ...
    ...
    [nx, ny] = SoundInterface(obj, 'add', 'GoSound',        nx, ny); %added 231026 Chuck
    SoundInterface(obj,'set','GoSound','Dur1',0); %default so SMA default is as before
    ...
    ToggleParam(obj, 'LockContextNIC',0,x,y,                           'OnString','Lock_Context_to_NIC','OffString','Context_NIC_Independent'); next_row(y);
    ...
    NumeditParam(obj, 'GoodNICRewMult',1,x,y,                            'labelfraction', 0.5); next_row(y);
    ...
    NumeditParam(obj, 'MemoryZap',   0,x,y,'labelfraction',0.6,                        'position',[x+100 y 100 20]); next_row(y);
    
    ToggleParam(obj, 'Temperror',    0,x,y,'OnString', 'Temperror ON', 'OffString', 'Temperror OFF'); next_row(y);
    ...
    [x, y] = SoundInterface(obj, 'add', 'StimSound', x, y); next_row(y,0);
    [x, y] = PsychSection(  obj, 'init',             x, y); next_row(y,0);
    [x, y] = MaskSection(obj,    'init',             x, y); next_row(y,0);
    ...
    [x, y] = PunishInterface(obj, 'add', 'error_state', x, y);    next_row(y,1);
    PunishInterface(obj, 'set', 'error_state', 'SoundsPanel', 0);
    
    [x, y] = AntibiasSection(  obj, 'init', x, y); next_row(y,1);
    [x, y] = SidesSection(     obj, 'init', x, y); next_row(y,1);
    [x, y] = RewardsSection(   obj, 'init', x, y); 
    
    ToggleParam(obj, 'StimOnFree',0,x,y,'OnString', 'Stim On Free', 'OffString', 'Stim on ALL'); next_row(y,0);
...
%% prepare_next_trial    
  ...
  case 'prepare_next_trial'
    ...
    disp([numel(hit_history(:)), numel(sides_history(:)), numel(psych_history(:)),...
          numel(loc_history(:)), numel(mem_history(:)), numel(free_history(:)),...
          numel(cpoketry(:))]);
    
    %CRASH HANDLING: see if we're coming in from a crash caught by dispatcher
    if isempty(varargin) || ~strcmp(varargin{1},'crashed')
        %CRASH HANDLING: We have not crashed, prepare things as normal, 
        %this is your normal prepare_next_trial code

        SessionDefinition(obj, 'next_trial');
        RewardsSection( obj, 'prepare_next_trial');

        sides = value(sides_history);
        ...
        %CRASH HANDLING: store sma and prepare_next_trial_states as
        %soloparams and increment n_prepped_trials
        SMA.value = sma;
    ...
    elseif strcmp(varargin{1},'crashed')
        %CRASH HANDLING: We're here because dispatcher caught an error and 
        %crashed, let's check if we've already prepped the next trial yet 
        ...
        if value(n_prepped_trials) < value(n_started_trials)
            ...
            %CRASH HANDLING: append all non-gui soloparams that your normal
            %code above would have incremented
            hit_history.value         = [value(hit_history);       nan];
            sides_history.value      = [value(sides_history),      'l']; %can't be a nan
            psych_history.value      = [value(psych_history),      nan];
            loc_history.value        = [value(loc_history),        'p']; %can't be a nan
            mem_history.value        = [value(mem_history),        nan];
            free_history.value       = [value(free_history),       'r']; %can't be a nan
            cpoketry.value           = [value(cpoketry);           nan];
            stimulator_history.value = [value(stimulator_history), nan];
            mask_history.value       = [value(mask_history),       nan];
            

            %CRASH HANDLING: increment this counters
            n_prepped_trials.value = value(n_prepped_trials) + 1;
            
        else
            %CRASH HANDLING: we have prepped the next trial, when we send
            %sma to dispatcher the histories will be pushed so to keep them
            %the correct length we need to first pop them back one
            dispatcher('pop_history');
        end
    end
            
    %CRASH HANDLING: now send info to dispatcher that was stored in soloparams
    dispatcher('send_assembler', value(SMA), value(PrepareNextTrialStates));        
            
    %CRASH HANDLING: from here down is your regular code that happens after
    %the sma is pushed to dispatcher
    ...
    %CRASH HANDLING: make sure these counters start out in sync
    if n_done_trials == 0
...
%% pre_saving_settings
  ...
  case 'pre_saving_settings'
      ...
      try 
        ...
        pd.violations = pd.cpt > 0 | isnan(pd.cpt);
        ...
        pd.AntR_perf = value(Anti_R_Perf); %#ok<NODEF>
...
%% close    
  ...
  %---------------------------------------------------------------
  case 'close'
    try AdLibGUISection(obj, 'close'); end
