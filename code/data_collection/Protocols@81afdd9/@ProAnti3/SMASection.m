...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    ...
    if (strcmp(ThisTrial_Stim,'LIGHT') && (value(n_center_pokes) == 0 || NICDur == 0.001)) || value(LightAndSound) == 1
                                  sidecuesustain = stimlight;
    elseif strcmp(value(PostCueType),'Both')
                                  sidecuesustain = left1led + right1led;
    elseif strcmp(value(PostCueType),'Random')
        if rand(1) < 0.5;         sidecuesustain = left1led;
        else                      sidecuesustain = right1led;
        end
    elseif strcmp(value(PostCueType),'Opposite')
        if stimlight == left1led; sidecuesustain = right1led;
        else                      sidecuesustain = left1led;
        end
    else
                                  sidecuesustain = 0;
    end
    ...
    if memoryzap <= 0; memoryzap = 0.001; end
    
    nicdur = NICDur;
    if NICDur <= 0; nicdur = 0.001; end
    
    legalcbrk = LegalCBrk;
    if LegalCBrk <= 0; legalcbrk = 0.001; end
    
    stimdelay = StimDelay;
    if StimDelay <= 0; stimdelay = 0.001; end
    
    endnic = endNIC;
    if endNIC <= 0; endnic = 0.001; end
    
    minilcb = miniLCB;
    if miniLCB <= 0; miniLCB = 0.001; end
    ...
    if KeepNoseCenter == 0
        if StimTime == 1
            ...
            if strcmp(ThisTrial_Free,'FREE')
                ...
                sma = add_state(sma, 'name', 'nicstim', 'self_timer',0.001,...
        ...
        if StimTime == 1 %stim comes after NIC context period
            sma = add_state(sma, 'name', 'context', 'self_timer', 0.001,...
        ...
        sma = add_state(sma, 'self_timer', 0.001,... %(0) NOSE IN start good NIC period
            'output_actions', {'DOut',post_cue_in; 'SchedWaveTrig', '-endNIC_wave-miniLCB_wave'},...
            'input_to_statechange', {'Tup', 'current_state+1'});
        
        sma = add_state(sma, 'self_timer', 0.001,... %(A) NOSE IN start good NIC period
        ...
        sma = add_state(sma, 'self_timer',0.001,... %(H)
            'output_actions', {'DOut',post_cue_in;... 
        ...
        %This is where we can add a go cue sound trigger state
        go_dur = SoundInterface(obj,'get','GoSound','Dur1');
        if go_dur > 0
            sma = add_state(sma,'name','gostate','self_timer',go_dur,...
                'output_actions', {'DOut',post_cue_in;... 
                                   'SoundOut', SoundManagerSection(obj, 'get_sound_id', 'GoSound')},...
                'input_to_statechange', {'Tup',after_nic_comp});
        else
            sma = add_state(sma,'self_timer',0.001,... %nothing state
                'output_actions', {'DOut',post_cue_in},...
                'input_to_statechange', {'Tup',after_nic_comp});
        end
        ...
        if strcmp(ThisTrial_Free,'FREE')
            ...
            sma = add_state(sma, 'name', 'stimon_nosein', 'self_timer', 0.001,...
                'output_actions', {'DOut',     post_cue_in;... 
                                   'SchedWaveTrig', 'memoryzap_wave'},...
                'input_to_statechange', {'Tup', 'current_state-8'});
            sma = add_state(sma, 'name', 'stimon_noseout','self_timer', 0.001,...
                'output_actions', {'DOut',     post_cue_out;...
                                   'SchedWaveTrig', 'memoryzap_wave'},...
                'input_to_statechange', {'Tup', 'current_state-6'});
        else
            %Regular trials have a stim sound
            sma = add_state(sma, 'name', 'stimon_nosein', 'self_timer', 0.001,...
                'output_actions', {'DOut',     post_cue_in;... 
        ...
        %NIC failure, reinit
        sma = add_state(sma, 'name', 'reinit_cpoke', 'self_timer', 0.01,...
        ...
        if NICViolation == 0
            sma = add_state(sma, 'self_timer', SoundInterface(obj,'get','Reinit','Dur1'),...
                ...
                'input_to_statechange', {'Tup', 'wait_for_cin'});
            %sma = add_state(sma,...
            %    'output_actions', {'DOut', cue},...  
            %    'input_to_statechange', {'Cin', do_after_waitcin; 'Chi', do_after_waitcin});
        else
    ...
    if value('LightAndSound') == 0
        ...
        sma = add_state(sma, 'name', 'stim_on',...
            'output_actions',{'DOut',stimlight; 'SchedWaveTrig','stim_wave'; 'SoundOut', SoundManagerSection(obj, 'get_sound_id', 'StimSound')},...
            'input_to_statechange', {'stim_wave_In','stim_off'; correct,'son_reward_chooser'; wrong,on_error});
    ...
    sma = add_state(sma, 'name', 'wait_for_spoke',...
        'output_actions',       {'DOut', sidecuesustain},...
    ...
    %added extra final state which is now the only state that goes to
    %check_next_trial_ready and gave it a 10ms timer so Bpod has time
