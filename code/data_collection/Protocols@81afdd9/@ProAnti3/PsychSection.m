...
% ----------------------------------------------------------------
  ...
  case 'init', 
      NumeditParam(obj, 'ClickFrequency', 40,x,y,'labelfraction',0.7); next_row(y);
      MenuParam(obj,'PsychType',{'Balance';'Frequency';'Gamma'},1,x,y,'position',[x     y 130 20],'labelfraction',0.5);
...
% -----------------------------------------------------------------------
  ...
  case 'CalcMidSpread',
      ...
      elseif strcmp(value(PsychType),'Frequency')
          Spread.value   = value(Right_End) / value(Left_End); %#ok<NODEF>
...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    ...
    if strcmp(PsychType,'Gamma')
        gamma = psychvals(value(ThisTrial));
        cf = value(ClickFrequency);
        f2 = (cf*2.71828^gamma) / (1 + 2.71828^gamma);
        f1 = cf - f2;

        SoundInterface(obj, 'set', 'StimSound', 'Freq1', f1);
        SoundInterface(obj, 'set', 'StimSound', 'Freq2', f2);
        SoundInterface(obj, 'set', 'StimSound', 'Bal',   0);
        
    elseif strcmp(PsychType,'Balance')    
