...
% ----------------------------------------------------------------
  ...
  case 'init', 
      %NumeditParam(obj,'LCB_onmask', 0,x,y,'position',[x     y 100 20],'labelfraction',0.6);
      %NumeditParam(obj,'LCB_nomask', 0,x,y,'position',[x+100 y 100 20],'labelfraction',0.6); 
      ...
      DeclareGlobals(obj, 'rw_args', {'mask_history'});
...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
      ...
      if sh(end) == 1
          %We're zapping on this trial, let's match the mask to the stim
          sd = StimulatorSection(obj,'get','SD');
          mf = StimulatorSection(obj,'get','SF');
          pw = StimulatorSection(obj,'get','PW');
          np = StimulatorSection(obj,'get','NP');
          ms = StimulatorSection(obj,'get','StimState');
          
          pms = get(get_ghandle(MaskState),'String'); %#ok<NODEF>
          ms = find(strcmp(pms,ms),1,'first');
          ml = ml(1);
