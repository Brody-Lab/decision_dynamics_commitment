...
switch action,
    ...
    case 'init'
        x=varargin{1};
        y=varargin{2};
        NumeditParam(obj,'single_flash',0,x,y,'label','Single Flash');
        next_row(y);
        ...
        next_row(y);
        NumeditParam(obj, 'stereo_flash', 1, x,y,'label','Stereo Flash','TooltipString','Prob. of first flash being a stereo flash.');
        next_row(y);
        ...
        next_row(y);
        NumeditParam(obj, 'high_rew_trial', 1, x, y, ...
            'TooltipString', 'Is this trial a high reward trial'); next_row(y);
        NumeditParam(obj, 'high_rew_prob', 0, x, y, ...
            'TooltipString', 'Probability of a high reward'); next_row(y);
        NumeditParam(obj, 'high_rew_mult', 5, x, y, ...
            'TooltipString', 'How much to increase reward on high reward trial'); next_row(y);
        ...
        SoloFunctionAddVars('SMA1', 'ro_args', ...
            {'led_time'; 'bin_size';'side_lights' ; 'ignore_errors';...
            'right_prob'; 'left_prob'; 'stereo_flash';'high_rew_mult';'high_rew_trial';...
            'nic_time'; 'wait_for_cpoke_timeout'; 'allow_nic_breaks' ; ...
            'settling_time'; 'delay_time';'single_flash'; ...
            'wait_for_spoke_timeout' ; 'reward_delay';'reward_exponent';'left_wtr_mult';...
            'error_iti';'violation_iti';'right_wtr_mult';'antibias_wtr_mult';'imaging'});
        ...
        SoloParamHandle(obj, 'stereo_flash_history',      'value', []);
    ...
    case 'forced_stereo',
        ...
        stereo_flash_history.value=[stereo_flash_history(:); varargin{1}];
    ...
    case 'next_trial',
        ...
        %% Do the anti-bias
        %if the rat favors one side, reduce the reward volume on that side
        %and increase reward volume on the other.
        ...
        if n_done_trials>25 && antibias_wtr_mult==1 && value(training_stage)==2
            ...
            if abs(right_hit-left_hit)<0.12 || isnan(right_hit) || isnan(left_hit)
        ...
        high_rew_trial.value=rand<high_rew_prob;
            ...
            switch value(training_stage) %#ok<*NODEF>
                ...
                case 1,  % Earliest stage of training.  Light chase and grow nose in center
                    ...
                    if ~was_viol  % if last trial was violation don't increment.
                        ...
                        if delay_time>2 %transition to next training stage
                ...
                case 2, % start counting flashes
                    bin_size.value=0.25;
                    nic_time.value=3*bin_size+floor(rand*7)*bin_size;
                    settling_time.value=0.15+rand/5;
                    delay_time.value=0.05+rand/3;
                    led_time.value=0.01;
                    high_rew_prob.value=0.010;
                    side_lights.value=1;
                    reward_exponent.value=1;
                    last_trial=previous_sides(end)=='r' || previous_sides(end)=='s';
                    
                    % We will only use the variable this_trial during
                    % training, i.e., when the rat has done <1500 trials in
                    % training stage 2. After that, the side with the higher
                    % generative probability for each trial is chosen
                    % independently. JCE thinks this makes training easier for the rat.
                    if rand<0.35
                    ...
                    if trials_in_stage<900
                        ...
                        led_time.value=0.05;
                        if trials_in_stage<600
                            side_lights.value=3;
                        else
                            side_lights.value=1;
                        end
                        
                    elseif trials_in_stage<1500
                        ...
                        Ps=[0.3 0.7;
                            ...
                            0.8 0.2;];
                        tt=randi(8);
                        left_prob.value=Ps(tt,1);
                        right_prob.value=Ps(tt,2);
                        single_flash.value=rand<0.08;
                        if single_flash==1
                            if rand<0.5
                                left_prob.value=1;
                            else
                            ...
                            right_prob.value=1-left_prob;
                            nic_time.value=bin_size+0;
                        end
                        led_time.value=0.01;
                    end
            end
    ...
    case 'get_water_mult'
        if high_rew_trial==0
            ...
            x=left_wtr_mult*high_rew_mult;
            y=right_wtr_mult*high_rew_mult;
        end
    ...
    case 'make_and_send_summary'
        ...
        pd.side_lights=cell2mat(get_history(side_lights));
        pd.stereo_flash=value(stereo_flash_history);
        pd.high_rew_trial=value(get_history(high_rew_trial));
        ...
        pd.stereo_flash =  pd.stereo_flash(1:nt);
        pd.high_rew_trial = pd.high_rew_trial(1:nt);
