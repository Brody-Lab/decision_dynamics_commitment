...
switch action
    ...
    case 'init',
        ...
        % catch trials
        NumeditParam(obj, 'CatchFreq', .15, x, y, ...
            'position', [x y 100 20], ...
            'labelfraction', .6, ...
            'TooltipString', 'frequency with which catch trials (reward omissions) occur');
        NumeditParam(obj, 'MaxCatch', 1, x, y, ...,
            'position', [x+100 y 100 20], ...
            'labelfraction', .7, ...
            'TooltipString', 'Maximum number of catch trials that can happen in a row');next_row(y);
        DispParam(obj, 'ThisCatch', 0, x, y);  next_row(y);
        
        DispParam(obj, 'ThisSound', 1, x, y, ...
            'TooltipString', 'The id of the stimulus being presented for this trial');
        next_row(y);
        
        DispParam(obj, 'ThisTrial', 'RIGHT', x, y, 'position', [x y 200 20]); next_row(y);
        ...
        SoloParamHandle(obj, 'previous_catch', 'value', 0); % reward omission trial
        SoloParamHandle(obj, 'previous_rew_delay', 'value', 0);
        SoloParamHandle(obj, 'previous_err_delay', 'value', 0);
    ...
    case 'next_trial',
        ...
        % sample new wait delays
        PenaltySection(obj, 'sample_WaitDelays');
        ...
        if ~strcmpi(value(MaxSame), 'Inf') && MaxSame <= n_started_trials,
            % if there've been MaxSame same trials in a row, force change
        ...
        ThisCatch.value = 0;
        if n_done_trials>1,
            noticed_catch_trials = value(previous_catch(1:length(hit_history))) .* hit_history; % indicator for trials where the rat made the correct choice but the reward was omitted
            if sum(noticed_catch_trials(max(1,end-value(MaxCatch)+1):end)) < value(MaxCatch), % check that we haven't had too many consecutive noticed catch trials
                if rand(1) < value(CatchFreq),
                    ThisCatch.value = 1;
                end
            end
        end
    ...
    case 'update_trial_info'
        ...
        [hit_wait_delay, err_wait_delay]  = PenaltySection(obj, 'get_WaitDelays');
        
        if n_done_trials == 0,
            previous_sides.value   = s;
            previous_catch.value   = value(ThisCatch);
            ...
            previous_rew_delay.value =  hit_wait_delay;
            previous_err_delay.value = err_wait_delay;
        elseif n_done_trials > 0,
            previous_sides.value   = [previous_sides(:); s];
            previous_catch.value   = [previous_catch(:); value(ThisCatch)];
            ...
            previous_rew_delay.value = [previous_rew_delay(:); hit_wait_delay];
            previous_err_delay.value = [previous_err_delay(:); err_wait_delay];
        end
    ...
    case 'update_last_trial_info'
        if n_done_trials < 2,
            ...
            if violation_history(n_done_trials-1) == 1,
                % on violation trials, there's no sensible way to determine the
                % actual sample duration
                sample_duration = NaN;
                memory_duration = NaN;
            else
                % fill in info for prev trial
                specs = StimulusSection(obj, 'get_stimulus_specs');
                if strcmp(specs.StimTiming, 'During Cpoke Only'),
                    memory_duration = 0;
                    if isfield(parsed_events_history{end}.states, 'cpoke1') && rows(parsed_events_history{end}.states.cpoke1) > 0,
                        sample_duration = diff(parsed_events_history{end}.states.cpoke1(1,:)) - specs.sound_delay;
                    else
                        sample_duration = NaN;
                    end;
                else
                    ...
                    if isfield(parsed_events_history{end}.states, 'cpoke1') && rows(parsed_events_history{end}.states.cpoke1) > 0,
                        memory_duration = diff(parsed_events_history{end}.states.cpoke1(1,:)) - specs.sound_delay - sample_duration;
                    else
                        memory_duration = NaN;
                    end;
                end;
    ...
    case 'make_and_send_summary',
        pd.hits        = value(hit_history(1:n_completed_trials));
        pd.wait_time   = value(wait_time_history(1:n_completed_trials));
        pd.violations  = value(violation_history(1:n_completed_trials));
        pd.rewards     = value(rew_history(1:n_completed_trials));
        pd.sounds      = previous_sounds(1:n_completed_trials);
        pd.sides       = previous_sides(1:n_completed_trials);
        pd.rt_task     = previous_rt_task(1:n_completed_trials);
        pd.stims       = previous_stims(1:n_completed_trials);
        pd.catch       = previous_catch(1:n_completed_trials);
        pd.rew_delay   = previous_rew_delay(1:n_completed_trials);
        pd.err_delay   = previous_err_delay(1:n_completed_trials);
        
        %
        pd.samples     = [value(previous_samples); nan];
        pd.samples     = pd.samples(1:n_completed_trials);
        pd.memory_gaps = [value(previous_memory_gaps); nan];
        pd.memory_gaps = pd.memory_gaps(1:n_completed_trials);
        
        pd.bupsdata   = PBupsSection(obj, 'get_all_bup_times');
        pd.stimdata   = PBupsSection(obj, 'get_all_stimulator_specs');
        pd.bupsdata   = pd.bupsdata(1:n_completed_trials);
        pd.stimdata   = pd.stimdata(1:n_completed_trials);
        
        nl=nan(size(pd.hits));
        nr=nan(size(pd.hits));
        gamma=nan(size(pd.hits));
        
        for tx=1:numel(pd.hits)
            nl(tx)=sum(pd.bupsdata{tx}.left<pd.samples(tx));
            nr(tx)=sum(pd.bupsdata{tx}.right<pd.samples(tx));
            gamma(tx)=pd.bupsdata{tx}.gamma;
        end
