% PBups_WT protocol
% A fork of Pbups that introduces a reward delay during which rats must wait to be rewarded.
% TBM, Sept 2015

function [obj] = PBupsWT(varargin)
...
obj = class(struct, mfilename, saveload, water, wateradaptor, ...
    ...
    distribui, poissonbupsWT, comments, sqlsummary);
...
switch action,
    ...
    case 'init'
        ...
        set(value(myfig), 'Position', [100 100 850 800]);
        ...
        %   History of rewarded trials
        %   (useful for when rat is correct, but fails to wait for reward):
        SoloParamHandle(obj, 'rew_history', 'value', []);
        DeclareGlobals(obj, 'ro_args', 'rew_history');
        
        %   History of rewarded trials
        %   (useful for when rat is correct, but fails to wait for reward):
        SoloParamHandle(obj, 'wait_time_history', 'value', []);
        DeclareGlobals(obj, 'ro_args', 'wait_time_history');
        
        
        %   Let RewardsSection, the part that parses what happened at the end of
        %   a trial, write to hit_history:
        SoloFunctionAddVars('RewardsSection', 'rw_args', {'hit_history'; 'violation_history'; 'rew_history'; 'wait_time_history'});
        ...
        PBupsSection(obj, 'set_prep_before')
        ...
        HeaderParam(obj, 'prot_title', ['PBupsWT: ' expmtr ', ' rname], ...
    ...
    case 'end_session'
        ...
        StimulusSection(obj, 'end_session');   % nothing actually happens when this case is triggered
        RewardsSection(obj, 'end_session');
