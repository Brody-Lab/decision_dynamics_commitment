% @PBupsWT/PenaltySection.m
% Tyler , Sept 2015
% developed from PBups PenaltySection
...
switch action
    ...
    case 'init',
        ...
        next_row(y);
        DistribInterface(obj, 'add', 'HitWaitDelay', x, y, 'Style', ...
            'exponential', 'Tau', 1.5, 'Min', .5, 'Max', 8);next_row(y,5.1);
        DistribInterface(obj, 'add', 'ErrorWaitDelay', x, y, 'Style', ...
            'exponential', 'Tau', 1.5, 'Min', .5, 'Max', 8);next_row(y,5.1);
        NumeditParam(obj, 'LegalWaitBreak', .01, x, y, ...
            'TooltipString', sprintf(['The maximum allowable time (in sec) between leaving'...
            '\the side poke and returning. Longer excursions away from'...
            '\the correct side poke will lead to reward omission']));next_row(y);
        ToggleParam(obj, 'TerminateWaitBreak', 1, x, y, ...
            'OnString', 'Terminate Wait Break ON', 'OffString', 'Terminate Wait Break OFF', ...
            'TooltipString', sprintf(['\nTerminate trial if rat leaves the waiting port for too long '])); next_row(y);
         ToggleParam(obj,'SignalWaitViol', 1, x , y,...
            'OnString', 'signal wait viol ON', 'OffString', 'signal wait viol OFF', ...
            'TooltipString', sprintf(['\nwhether to keep light on and play sound to indicate '...
            '\nthat the legal wait break period has been violated']));
        next_row(y);
        ToggleParam(obj, 'WaitCpokeReset', 1, x, y, ...
            'OnString', 'Wait Cpoke Reset ON', 'OffString', 'Wait Cpoke Reset OFF', ...
            'TooltipString', sprintf(['\nRestarts trial if rat makes cpoke after wait state'])); next_row(y,2);
        ...
        SoloParamHandle(obj, 'mypfig', 'saveable', 0, 'value', ...
            figure('position', [409   316   420   650], ...
            'MenuBar', 'none',  ...
            'NumberTitle', 'off', ...
...
%             'TooltipString', 'if black, punish pokes during ITI with Violation3Sound');
        ...
        ToggleParam(obj, 'SoftDrinkTime', 10, x, y, 'position', [x y 120 20], ...
        ...
        next_row(y);
        NumeditParam(obj, 'SoftDrinkGrace', 2, x, y, 'position', [x y 100 20], ...
            'TooltipString', 'After this amount of time has passed without licks, we assume he is done drinking and go on');
        NumeditParam(obj, 'SoftDrinkCap', 10, x, y, 'position', [x+100 y 100 20], ...
        ...
        NumeditParam(obj, 'WarnDur',   1, x, y, 'labelfraction', 0.6, 'TooltipString', 'Warning sound duration in secs', 'position', [x+80 y 60 20]);
        NumeditParam(obj, 'DangerDur', 2, x, y, 'labelfraction', 0.6, 'TooltipString', sprintf('\nDuration of post-drink period where poking is punished'), 'position', [x+140 y 60 20]); next_row(y);
        ...
        % punishment for reinitiating waiting after a wait violation
        [x, y] = PunishInterface(obj, 'add', 'WaitReinitPun', x, y);
        SoundInterface(obj, 'set', 'WaitReinitPun_InitSnd', 'Style', ...
            'WhiteNoise', 'Vol', 0.01, 'Dur1', 0.06,'Loop', 0, 'Bal', 0);
        SoundInterface(obj, 'set', 'WaitReinitPun_OngoingSnd', 'Style', ...
            'WhiteNoise', 'Vol', 0.0, 'Dur1', 0.0,'Loop', 0, 'Bal', 0);
        PunishInterface(obj, 'set', 'WaitReinitPun', 'SoundsPanel', 0, 'Duration',.06);
        ...
        ToggleParam(obj, 'TempTOPun', 0, x, y, ...
        ...
        ToggleParam(obj, 'PunishSidePokesinWFCO', 0, x, y, ...
            'OffString', 'ignore side pokes during WFCO', ...
            'OnString',  'punish side pokes during WFCO', ...
            'TooltipString', sprintf(['\n' ...
            'If black, side pokes during WFCO are punished with white noise\n' ...
            'If brown, side pokes are ignored.'])); next_row(y);
        ...
        SoloFunctionAddVars('StateMatrixSection', 'ro_args', ...
            ...
            'PunishSideChoice'; 'TempTOPun';... %'PunishITIPokes'; 
            'SoftDrinkTime'; 'WD_in_sdt' ; ...
            'SoftDrinkGrace'; 'SoftDrinkCap'; ...
            'wait_for_cpoke1_tout' ;  ...
            'TempPunChoice' ; 'LEDtemp_pun' ; ...
            'WaitForCenterNoseOut'; 'PunishSidePokesinWFCO';
            'WaitCpokeReset'; 'TerminateWaitBreak'; 'LegalWaitBreak';'SignalWaitViol'});
    ...
    case 'get_ITI',
        ...
        x = DistribInterface(obj, 'get_current_sample', 'ITI')
        
    case 'sample_WaitDelays'
        DistribInterface(obj, 'sample', 'HitWaitDelay');
        x = DistribInterface(obj, 'get_current_sample', 'HitWaitDelay')
        DistribInterface(obj, 'sample', 'ErrorWaitDelay');
        y = DistribInterface(obj, 'get_current_sample', 'ErrorWaitDelay')
    
    case 'get_WaitDelays'
        [unused_var, ThisCatch]       = SidesSection(obj, 'get_current_side');
        if ~value(ThisCatch),
            x = DistribInterface(obj, 'get_current_sample', 'HitWaitDelay');
            y = DistribInterface(obj, 'get_current_sample', 'ErrorWaitDelay');
        else
            x = DistribInterface(obj, 'get', 'HitWaitDelay', 'Max');
            y = DistribInterface(obj, 'get', 'ErrorWaitDelay', 'Max');
        end
