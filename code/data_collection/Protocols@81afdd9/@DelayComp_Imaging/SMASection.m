...
%                             'Lout', 'violation_state'});
                        ...
                        sma = add_state(sma,'name','cpslotonly','self_timer', SettlingIn_time+0.00001, ...
                            'output_actions', {'SchedWaveTrig', 'center_poke +settling_period +stimA1+stimA2 +pistons +talktoscanimage +go +add_obj_water +remove_obj_water'}, ...
                            'input_to_statechange', {'Tup', 'cp_legal_cbreak_period_slot', ...
                            'Blo','violation_state', ...
                            'Dlo', 'release',...
                            'center_poke_Out', 'wait_for_collecting_reward_slot', ...
        ...
        if value(water_objective==1)
            sma = add_state(sma, 'name', 'release', 'self_timer', 0.001, 'output_actions',...
                {'SchedWaveTrig', '-pistons-add_obj_water-remove_obj_water'},'input_to_statechange',...
                {'Tup', 'violation_state'});
        else
            sma = add_state(sma, 'name', 'release', 'self_timer', 0.001, 'output_actions',...
                {'SchedWaveTrig', '-pistons'},'input_to_statechange',...
                {'Tup', 'violation_state'});
        end
        ...
        if stimuli_on ==0
            ...
            if value(water_objective)==1
                sma = add_state(sma,'name','violation_state','self_timer',viol_snd_duration,...
                    'output_actions',{'SchedWaveTrig', '-center_poke-stimA1-stimA2-pistons-go-add_obj_water-remove_obj_water', ...
                    'SoundOut',viol_sound_id},...
                    'input_to_statechange', {'Tup', 'current_state+1'});
            else
                sma = add_state(sma,'name','violation_state','self_timer',viol_snd_duration,...
