...
% This version is for imaging in the Tank lab
% Christine Constantinople, June 2014

function [obj] = DelayComp_Imaging(varargin)
...
switch action,
   ...
   case 'init'
    ...
    %SoloParamHandle(obj, 'deploy_practicepistons', 'value', 0);
    ...
    DeclareGlobals(obj, 'ro_args', {'thisside','ProbMaxPressure', 'StepSize', 'pair_history'});
        %'deploy_practicepistons','ProbMaxPressure', 'StepSize'});
    SoloFunctionAddVars(obj, 'StimulusSection', 'rw_args', 'pair_history');
    ...
    next_row(y, 1);
    NumeditParam(obj, 'water_objective', 0, x, y, 'TooltipString', 'If 1, water collar will apply and remove liquid to image with immersion objective');
    next_row(y, 1);
    ...
    next_row(y, 1);
    NumeditParam(obj, 'deploy_practicepistons', 0, x, y, 'TooltipString', 'if 1 deploys the pistons during reward');
    next_row(y, 1);
    NumeditParam(obj, 'StepSize', 300, x, y, 'TooltipString', 'StepSize');
    next_row(y, 1);
    NumeditParam(obj, 'NosPos', 0, x, y, 'TooltipString', 'Lick Tube Position')
    next_row(y, 1);
    NumeditParam(obj, 'microswitch_counter', 0, x, y,'label','microswitch_counter','TooltipString','microswitch_counter');
    next_row(y, 1);
    NumeditParam(obj, 'use_training_on', 1, x, y,'label','use_training_on','TooltipString','Enable Automated Training');
    next_row(y, 1);
    NumeditParam(obj, 'trials_in_stage_on', 1, x, y, 'label', 'trials_in_stage','TooltipString', 'if use_training_on, trials_in_stage');
    next_row(y, 1);
    ...
    SoloFunctionAddVars('SMASection', 'ro_args', ...
            ...
            'pressure';'deploy_practicepistons'; 'water_objective'});
    SoloFunctionAddVars('SMASection', 'rw_args', {'StepSize'});
    SoloFunctionAddVars('SideSection', 'rw_args', {'trials_in_stage_on'; 'microswitch_counter'; 'StepSize'});
    SoloFunctionAddVars('SideSection', 'ro_args', {'use_training_on'});
    SoloFunctionAddVars('StimulusSection', 'ro_args', {'trials_in_stage_on'});
   ...
   case 'prepare_next_trial'
	   try
		   ...
		   if have_i_crashed_yet==0, %#ok<*NODEF>
			   ...
			   save('../Protocols/@DelayComp_Imaging/crash_report');
		   end;
	   end;
   ...
   case 'trial_completed'
	   try
           ...
           SideSection(obj, 'trial_completed');
