function [obj] = ProAntiHessam2(varargin)

obj = class(struct, mfilename, pokesplot, saveload, water, ...
    soundmanager, soundui, sessionmodel, comments, sqlsummary, antibias);
...
switch action,
  ...
  case 'init'
    ...
    set(value(myfig), 'Position', [75 75 863 380]);
    ...
    %fprintf('x = %.2f, y = %.2f frog\n', x, y);
    [x, y] = SoundsSection(obj, 'init', x, y);
    
    %fprintf('x = %.2f, y = %.2f apple\n', x, y);
    ...
    HeaderParam(obj, 'prot_title', [mfilename ': ' expmtr ', ' rname], ...
        x, y, 'position', [10 figpos(4)-25, 900 20]);
    ...
    SoloParamHandle(obj, 'prev_target', 'value', 0);
    SoloParamHandle(obj, 'prev_context', 'value', 0);
    SoloParamHandle(obj, 'target_history', 'value', []);
    SoloParamHandle(obj, 'context_history', 'value', []);

    
    DeclareGlobals(obj, 'ro_args', {'completed_cfix', 'cfix_history', 'prev_correct', ...
        'correct_history', 'prev_target', 'prev_context', 'target_history', 'context_history'});
  ...
  case 'prepare_next_trial'
    ...
    % AntiBiasing:
    target_probs = sum(TrialSection(obj, 'get_probs'), 1);
    subset = ~isnan(correct_history(:)); %#ok<NODEF>
    AntibiasSection(obj, 'update', target_probs(1), correct_history(subset), target_history(subset)); %#ok<NODEF>
    TrialSection(obj, 'set_probs', AntiBiasSection(obj, 'get_posterior_probs')');
    
    TrialSection(obj, 'prepare_next_trial');
    TimeVarsSection(obj, 'prepare_next_trial');
  ...
  case 'trial_completed'
    ...
    [expmtr, rname]=SavingSection(obj, 'get_info');
    prot_title.value=[mfilename ' on rig ' get_hostname ' : ' expmtr ', ' ...
        rname  '.  Started at ' datestr(now, 'HH:MM')]; %#ok<STRNU>
  ...
  case 'close'
    ...
    case 'pre_saving_settings'
        ...
        sendsummary(obj, 'hits', correct_history(:)', 'sides', target_history(:)'); %#ok<NODEF>
