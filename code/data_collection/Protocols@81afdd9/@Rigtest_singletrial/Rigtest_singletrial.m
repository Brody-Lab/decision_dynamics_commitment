...
switch action,
  ...
  case 'init'
    ...
    % ---------------------------------------------------------------------
    % Thomas Luo 2019-01-23
    hp_max_V = bSettings('get', 'GENERAL', 'HEADPHONE_MAX_V');
    if ~isnan(hp_max_V)
         L22_max_V = sqrt(2) * sqrt(0.6) *10^(20/20); % Lynx L22 sound card
         max_vol = hp_max_V/L22_max_V;
         snd(snd > max_vol) = max_vol;
    end
    % ---------------------------------------------------------------------
...
%         'Balance',1); next_row(y);
  ...
  case 'prepare_next_trial'
    ...
    force_left_duration   = bSettings('get','RUNRATS','manual_test_left_dur');
    force_center_duration = bSettings('get','RUNRATS','manual_test_center_dur');
    force_right_duration  = bSettings('get','RUNRATS','manual_test_right_dur');
    
    skip_left_port   = bSettings('get','INACTIVEINPUTLINES','L');
    skip_center_port = bSettings('get','INACTIVEINPUTLINES','C');
    skip_right_port  = bSettings('get','INACTIVEINPUTLINES','R');
    
    
    sma = StateMachineAssembler('full_trial_structure','use_happenings',1);
    
    if isnan(skip_left_port)
        sma = add_state(sma, 'name','first_dummy','self_timer',0.1, ...
            'input_to_statechange', {'Tup','wait_for_left'});
    else
        sma = add_state(sma, 'name','first_dummy','self_timer',0.1, ...
            'input_to_statechange', {'Tup','second_dummy'});
    end
    ...
    if isnan(force_center_duration)
        sma = add_state(sma, 'name', 'wfl_in_center','self_timer',5, ...
            'output_actions', {'DOut', center1water+left1led, 'SoundOut',center_sound}, ...
            'input_to_statechange', {'Cout', 'wait_for_left'; 'Tup','wait_for_left'});
    else
        sma = add_state(sma, 'name', 'wfl_in_center','self_timer',force_center_duration, ...
            'output_actions', {'DOut', center1water+left1led, 'SoundOut',center_sound}, ...
            'input_to_statechange', {'Tup','wait_for_left'});
    end
        
    if isnan(force_right_duration)
        sma = add_state(sma, 'name', 'wfl_in_right','self_timer',5, ...
            'output_actions', {'DOut', right1water+left1led, 'SoundOut',right_sound}, ...
            'input_to_statechange', {'Rout', 'wait_for_left'; 'Tup','wait_for_left'});
    else
        sma = add_state(sma, 'name', 'wfl_in_right','self_timer',force_right_duration, ...
            'output_actions', {'DOut', right1water+left1led, 'SoundOut',right_sound}, ...
            'input_to_statechange', {'Tup','wait_for_left'});
    end
    
    if isnan(force_left_duration)
        sma = add_state(sma, 'name', 'in_left','self_timer',5, ...
            'output_actions', {'DOut', left1water, 'SoundOut',left_sound}, ...
            'input_to_statechange', {'Lout', 'second_dummy'; 'Tup','second_dummy'});
    else
        sma = add_state(sma, 'name', 'in_left','self_timer',force_left_duration, ...
            'output_actions', {'DOut', left1water, 'SoundOut',left_sound}, ...
            'input_to_statechange', {'Tup','second_dummy'});
    end
    
    if isnan(skip_center_port)
        sma = add_state(sma, 'name','second_dummy','self_timer',0.1, ...
            'input_to_statechange', {'Tup','wait_for_center'});
    else
        sma = add_state(sma, 'name','second_dummy','self_timer',0.1, ...
            'input_to_statechange', {'Tup','third_dummy'});
    end
    ...
    if isnan(force_left_duration)
        sma = add_state(sma, 'name', 'wfc_in_left','self_timer',5, ...
            'output_actions', {'DOut', left1water+center1led, 'SoundOut',left_sound}, ...
            'input_to_statechange', {'Lout', 'wait_for_center'; 'Tup','wait_for_center'});
    else
        sma = add_state(sma, 'name', 'wfc_in_left','self_timer',force_left_duration, ...
            'output_actions', {'DOut', left1water+center1led, 'SoundOut',left_sound}, ...
            'input_to_statechange', {'Tup','wait_for_center'});
    end
    
    if isnan(force_right_duration)    
        sma = add_state(sma, 'name', 'wfc_in_right','self_timer',5, ...
            'output_actions', {'DOut', right1water+center1led, 'SoundOut',right_sound}, ...
            'input_to_statechange', {'Rout', 'wait_for_center'; 'Tup','wait_for_center'});
    else
        sma = add_state(sma, 'name', 'wfc_in_right','self_timer',force_right_duration, ...
            'output_actions', {'DOut', right1water+center1led, 'SoundOut',right_sound}, ...
            'input_to_statechange', {'Tup','wait_for_center'});
    end

    if isnan(force_center_duration)
        sma = add_state(sma, 'name', 'in_center','self_timer',5, ...
            'output_actions', {'DOut', center1water, 'SoundOut',center_sound}, ...
            'input_to_statechange', {'Cout', 'third_dummy'; 'Tup','third_dummy'});
    else
        sma = add_state(sma, 'name', 'in_center','self_timer',force_center_duration, ...
            'output_actions', {'DOut', center1water, 'SoundOut',center_sound}, ...
            'input_to_statechange', {'Tup','third_dummy'});
    end
    
    if isnan(skip_right_port)
        sma = add_state(sma, 'name','third_dummy','self_timer',0.1, ...
            'input_to_statechange', {'Tup','wait_for_right'});
    else
        sma = add_state(sma, 'name','third_dummy','self_timer',0.1, ...
    ...
    if isnan(force_center_duration)
        sma = add_state(sma, 'name', 'wfr_in_center','self_timer',5, ...
            'output_actions', {'DOut', center1water+right1led, 'SoundOut',center_sound}, ...
            'input_to_statechange', {'Cout', 'wait_for_right'; 'Tup','wait_for_right'});
    else
        sma = add_state(sma, 'name', 'wfr_in_center','self_timer',force_center_duration, ...
            'output_actions', {'DOut', center1water+right1led, 'SoundOut',center_sound}, ...
            'input_to_statechange', {'Tup','wait_for_right'});
    end
        
    if isnan(force_left_duration)
        sma = add_state(sma, 'name', 'wfr_in_left','self_timer',5, ...
            'output_actions', {'DOut', left1water+right1led, 'SoundOut',left_sound}, ...
            'input_to_statechange', {'Lout', 'wait_for_right'; 'Tup','wait_for_right'});
    else
        sma = add_state(sma, 'name', 'wfr_in_left','self_timer',force_left_duration, ...
            'output_actions', {'DOut', left1water+right1led, 'SoundOut',left_sound}, ...
            'input_to_statechange', {'Tup','wait_for_right'});
    end
    
    if isnan(force_right_duration) 
        sma = add_state(sma, 'name', 'in_right','self_timer',5, ...
            'output_actions', {'DOut', right1water, 'SoundOut',right_sound}, ...
            'input_to_statechange', {'Rout', 'final_state'; 'Tup','final_state'});
    else
        sma = add_state(sma, 'name', 'in_right','self_timer',force_right_duration, ...
            'output_actions', {'DOut', right1water, 'SoundOut',right_sound}, ...
            'input_to_statechange', {'Tup','final_state'});
    end
