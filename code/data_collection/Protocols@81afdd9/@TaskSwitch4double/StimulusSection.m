...
switch action,
    ...
    case 'init'
        ...
        NumeditParam(obj, 'timeout_delay', 4, x,y,'label','Timeout delay','TooltipString','Delay after timeout at spoke, follows new trial');
        next_row(y);
        ...
        NumeditParam(obj, 'nic_delay', 8, x,y,'label','NIC violation delay','TooltipString','Delay after NIC violation, follows new trial');
        next_row(y);
        ...
        SoloParamHandle(obj, 'myaxeshistory0', 'saveable', 0,'value', axes);
...
%         vec_left_zap=value(left_laser_history);
%         vec_left_zap=vec_left_zap(1:length(vec_hit));
%         vec_right_zap=value(right_laser_history);
%         vec_right_zap=vec_right_zap(1:length(vec_hit));        
%         %discard zapped trials
%         vec_sides(vec_right_zap)=NaN;
%         vec_sides(vec_left_zap)=NaN; 
%         
        ...
        WaterL.value=p(1)*2;
        WaterR.value=p(2)*2;
        DelayL.value=max(p(1)*8,4);
        DelayR.value=max(p(2)*8,4);
...
%         %discard zapped trials
%         vec_quad(vec_right_zap)=NaN;
%         vec_quad(vec_left_zap)=NaN;    
        ...
        Delay1Dir.value=max(p(1)*16,4);
        Delay2Dir.value=max(p(2)*16,4);
        Delay3Dir.value=max(p(3)*16,4);
        Delay4Dir.value=max(p(4)*16,4);
        ...
        Delay1Freq.value=max(p(1)*16,4);
        Delay2Freq.value=max(p(2)*16,4);
        Delay3Freq.value=max(p(3)*16,4);
        Delay4Freq.value=max(p(4)*16,4);
