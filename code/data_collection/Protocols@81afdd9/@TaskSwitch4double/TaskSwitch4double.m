% TaskSwitch4double protocol
% Marino Pagan, April 2017

function [obj] = TaskSwitch4double(varargin)

% Default object is of our own class (mfilename);
% we inherit only from Plugins

obj = class(struct, mfilename, saveload, water, AdLibGUI, ...
...
switch action,
    ...
    case 'init'
        ...
        set(value(myfig), 'Position', [600 90 850 720]);       
...
%         next_row(y,-0.4);                
        ...
        HeaderParam(obj, 'prot_title', ['TaskSwitch4double: ' expmtr ', ' rname], ...
        x, y, 'position', [10 figpos(4)-20, 807 20]);

    
        %   OK, start preparing the first trial
        TaskSwitch4double(obj, 'prepare_next_trial');
...
%             AdLibGUISection(obj, 'set_first_trial_time_stamp');
            [expmtr, rname]=SavingSection(obj, 'get_info');
            prot_title.value=['TaskSwitch4double - on rig ' get_hostname ' : ' expmtr ', ' rname  '.  Started at ' datestr(now, 'HH:MM')];
        end
...
%         
        StimulationSection(obj, 'trial_completed'); 
