...
%% init    
  case 'init'
    ...
    %this saves the position on the main PBups screen so we can return there
...
previous = value(previous_sides); 
previous = previous(1:length(hit_history));
hits = value(hit_history);
ph = previous.*hits;
rewardedlefts = sum(find(ph=='l'));
rewardedrights=sum(find(ph=='r'));

[Left_volume,Right_volume]=WaterValvesSection(obj,  'get_water_volumes');
wateramnt = (value(rewardedlefts)*value(Left_volume)) + (value(rewardedrights)*value(Right_volume));
