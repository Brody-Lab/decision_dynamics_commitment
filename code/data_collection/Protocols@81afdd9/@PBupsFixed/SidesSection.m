...
%% init
  case 'init',
    ...
    SoloFunctionAddVars('PBupsFixed', 'rw_args', {'previous_sides',...
                                                 'previous_sounds',...
                                                 'previous_samples',...
...
%% make_and_send_summary
  case 'make_and_send_summary',
    ...
    for tx=1:min([numel(pd.hits),numel(pd.bupsdata),numel(pd.samples)]) %lengths can be different if it's a live save or from a reconstituted ASV file -Chuck 
