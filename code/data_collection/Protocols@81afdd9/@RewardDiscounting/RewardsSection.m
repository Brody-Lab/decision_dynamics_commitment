...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    if n_done_trials >= 1,
        ...
        if     isfield(parsed_events.states,'no_reward_wait_l') && size(parsed_events.states.reward_right,1) == 0
            nonreward_history.value = [nonreward_history(:) ; 'l']; %#ok<NODEF>
        elseif isfield(parsed_events.states,'no_reward_wait_r') && size(parsed_events.states.reward_left,1)  == 0
            nonreward_history.value = [nonreward_history(:) ; 'r']; %#ok<NODEF>
        elseif isfield(parsed_events.states,'no_reward_wait')
            if size(parsed_events.states.reward_left,1) > 0
                nonreward_history.value = [nonreward_history(:) ; 'l']; %#ok<NODEF>
            else
                nonreward_history.value = [nonreward_history(:) ; 'r'];  %#ok<NODEF>
            end
