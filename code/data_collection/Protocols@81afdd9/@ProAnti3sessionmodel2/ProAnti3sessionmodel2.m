...
function [obj] = ProAnti3sessionmodel2(varargin)

obj = class(struct, mfilename, saveload, water, antibias, pokesplot, ...
  soundmanager, soundui ,sessionmodel2, distribui, punishui, warnDanger, ...
...
%% INIT 
  case 'init'
    ...
    ToggleParam(obj, 'CenterSustain',0,x,y,'OnString','CenterCue_Sustain','OffString','CenterCue_PreOnly'); next_row(y);
    ...
    [x, y] = SoundInterface(obj,   'add', 'Reinit',    x, y);
    ...
    NumeditParam(obj, 'LegalCBrk',   0,x,y,'position',[x+100 y 100 20],'labelfraction', 0.6); next_row(y, 1.3); 
    
    NumeditParam(obj, 'ContextDur',        1,x,y,'labelfraction',0.5); next_row(y); 
    NumeditParam(obj, 'CtoSgap',           0,x,y,'labelfraction',0.5); next_row(y); 
    NumeditParam(obj, 'StimDur',           1,x,y,'labelfraction',0.5); next_row(y);
    NumeditParam(obj, 'StimRewardOverlap', 1,x,y,'labelfraction',0.5); next_row(y,1.3);
    ...
    [x, y] = SoundInterface(obj,   'add', 'StimSound', x, y); 
    
    
      
    [x, y] = WarnDangerInterface(obj, 'add', 'warndanger', x, y);  next_row(y, 1.3); 
    WarnDangerInterface(obj, 'set', 'warndanger', 'WarnDur',   3);
    WarnDangerInterface(obj, 'set', 'warndanger', 'DangerDur', 0);
    
    
    [x, y] = PunishInterface(obj, 'add', 'error_state', x, y);    next_row(y, 1.3);
    ...
    [x, y] = RewardsSection( obj, 'init', x, y); next_row(y, 1.3);
	
	SessionDefinition(obj, 'refresh_lbxParameters');
    
    SoloFunctionAddVars('SidesSection',   'rw_args', {'sides_history', 'loc_history'});
    SoloFunctionAddVars('RewardsSection', 'rw_args', {'hit_history', 'cpoketry','Average_Cpoke_Trys', 'Percent_Single_Cpoke'});
...
%% prepare_next_trial    
  ...
  case 'prepare_next_trial'
    ...
    sh = value(sides_history); sh = sh(1:length(hh));
    lh = value(loc_history);   lh = lh(1:length(hh));
    
    
    if n_done_trials >= 1
        Total_Trial_Count.value = value(Total_Trial_Count) + 1; %#ok<NODEF>
        if ~isempty(sh) && ~isempty(lh)
            ...
            elseif sh(end) == 'r' && lh(end) == 'a' ; Anti_R_Cnt.value = value(Anti_R_Cnt) + 1;  %#ok<NODEF>
             ...
             Pro_L_Perf.value = mean(hh(sh == 'l' & lh == 'p'));
            Anti_L_Perf.value = mean(hh(sh == 'l' & lh == 'a'));
             Pro_R_Perf.value = mean(hh(sh == 'r' & lh == 'p'));
            Anti_R_Perf.value = mean(hh(sh == 'r' & lh == 'a'));
