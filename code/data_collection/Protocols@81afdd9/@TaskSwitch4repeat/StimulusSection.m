...
switch action,
    ...
    case 'init'
        ...
        for i=1:7
            for j=1:7            
                [aaa{i,j}, bbb{i,j}] = make_pbup_mixed3(value(total_rate),...
                    gammas(i)/2,gammas(j)/2, srate, value(ThisDuration));                
            end
        end
        sndz.value=aaa;
        dataz.value=bbb;
...
%             value(bup_ramp),'vol_low',value(vol_low_freq),'vol_hi',value(vol_hi_freq));
        ...
        i=find(gammas==value(ThisGamma_dir));
        j=find(gammas==value(ThisGamma_freq));
        aaa=value(sndz);
        bbb=value(dataz);
...
%             for j=1:6                
%                 [aaa{i,j}, bbb{i,j}] = make_pbup_mixed3(value(total_rate),...
%                     gammas(i),gammas(j), srate, value(ThisDuration), ...
%                     'bup_width',value(bup_width),'crosstalk_dir', crosstalk_dir,...
%                     'crosstalk_freq', crosstalk_freq,'freq_vec',freq_vec,'bup_ramp',...
%                     value(bup_ramp),'vol_low',value(vol_low_freq),'vol_hi',value(vol_hi_freq));                
%             end
