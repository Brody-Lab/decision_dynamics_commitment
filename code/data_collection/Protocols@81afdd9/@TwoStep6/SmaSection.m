...
switch action
    ...
    case 'next_trial',
       ...
       % inlines = struct('C',1+inputlines_offset,'L',2+inputlines_offset,'R',3+inputlines_offset,'c',4+inputlines_offset,'l',5+inputlines_offset,'r',6+inputlines_offset);
        ...
        if timing.use_timeouts == 0, timing.timeout_time = inf; end
        ...
        r = rand;
        % 210120 sjv combined violation penalties because bpod can only
        % handle 20 sounds
...
%         end
        ...
        if r < params.p_forceRight
            first_choice_lights = right2led;
            left_sub_state = 'violation_penalty';
            indicator_state  = 'force_right';
        elseif r > 1 - params.p_forceLeft;
            first_choice_lights = left2led;
            right_sub_state = 'violation_penalty';
            indicator_state  = 'force_left';
        else
            first_choice_lights = left2led + right2led;
            indicator_state  = 'free_choice';
        ...
        sma = add_state(sma,'name','choice_state_1',...
            'self_timer',timing.timeout_time,...
            'output_actions',{'DOut',first_choice_lights},...
            'input_to_statechange',{'lhi','left_choice_1','rhi','right_choice_1','Tup','timeout_penalty'});
        ...
        sma = add_state(sma,'name','left_state',...
            ...
            'input_to_statechange',{'Chi','nose_in_center_2l','Lhi','early_sidepokes_penalty_2l','Rhi','early_sidepokes_penalty_2l','Tup','timeout_penalty'});
        % add timeout
        sma = add_state(sma,'name','right_state',...
            'self_timer',timing.timeout_time,...
            'output_actions',{'SoundOut', right_state_sound,'DOut',pre_cpoke_led2},...
            'input_to_statechange',{'Chi','nose_in_center_2r','Lhi','early_sidepokes_penalty_2r','Rhi','early_sidepokes_penalty_2r','Tup','timeout_penalty'});
        ...
        % 210120 sjv combine violation penalties since bpod can only handle
        % 20 sounds
        % add timeout
%         sma = add_state(sma,'name','choice_state_2l',...
%             'self_timer',timing.timeout_time,...
%             'output_actions',{'DOut',left_choice_lights},...
%             'input_to_statechange',{'Lhi',post_correct_left,'Rhi','violation_penalty2','Tup','timeout_penalty'});
%         % add timeout
%         sma = add_state(sma,'name','choice_state_2r',...
%             'self_timer',timing.timeout_time,...
%             'output_actions',{'DOut',right_choice_lights},...
%             'input_to_statechange',{'Rhi',post_correct_right,'Lhi','violation_penalty2','Tup','timeout_penalty'});

        sma = add_state(sma,'name','choice_state_2l',...
            'self_timer',timing.timeout_time,...
            'output_actions',{'DOut',left_choice_lights},...
            'input_to_statechange',{'Lhi',post_correct_left,'Rhi','violation_penalty','Tup','timeout_penalty'});
        % add timeout
        sma = add_state(sma,'name','choice_state_2r',...
            'self_timer',timing.timeout_time,...
            'output_actions',{'DOut',right_choice_lights},...
            'input_to_statechange',{'Rhi',post_correct_right,'Lhi','violation_penalty','Tup','timeout_penalty'});
        
        %% Reward, unreward, and violation states
        % sjv 20210112 hardcoding enabled softdrink because bpod doesn't
        % support wave refractions
        if ~timing.softdrink_enabled
            warning('Bpod does not support wave refractions. Defaulting to enable softdrink');
        end
        if 1 %timing.softdrink_enabled
        ...
        % old code with wave refractions
...
%         sma = add_scheduled_wave(sma,'name','right_small_reward_wav','preamble',timing.reward_delay,'sustain',RightWValveTime_small,'DOut',right1water,'refraction',timing.drink_time);
        ...
        sma = add_scheduled_wave(sma,'name','left_reward_wav','preamble',timing.reward_delay,'sustain',LeftWValveTime,'DOut',left1water);
        sma = add_scheduled_wave(sma,'name','right_reward_wav','preamble',timing.reward_delay,'sustain',RightWValveTime,'DOut',right1water);
        sma = add_scheduled_wave(sma,'name','left_small_reward_wav','preamble',timing.reward_delay,'sustain',LeftWValveTime_small,'DOut');
        sma = add_scheduled_wave(sma,'name','right_small_reward_wav','preamble',timing.reward_delay,'sustain',RightWValveTime_small,'DOut');
        ...
        % Violation states
        % 210120 sjv combine violation penalties because bpod can only
        % handle 20 sounds
...
%         sma = PunishInterface(obj, 'add_sma_states', 'violation_penalty2', sma, 'exitstate', 'cleanup_state');
        sma = PunishInterface(obj, 'add_sma_states', 'violation_penalty', sma, 'exitstate', 'cleanup_state');
        sma = PunishInterface(obj, 'add_sma_states', 'timeout_penalty', sma, 'exitstate', 'cleanup_state');
