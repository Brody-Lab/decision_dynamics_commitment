...
switch action
    ...
    case 'init',
        ...
        SoloParamHandle(obj,'timeout_viol_history','value',[]);              % sjv add timeout history
        SoloParamHandle(obj,'left_reward_prob_history','value',[]);
        SoloParamHandle(obj,'right_reward_prob_history','value',[]);
        SoloParamHandle(obj,'leftwatertime','value',[]);                     % sjv add left water time history
        SoloParamHandle(obj,'rightwatertime','value',[]);                    % sjv add right water time history
        lf = 0.65;
        ...
        next_row(y);
        DispParam(obj,'timeout_violation_frac',0,x,y,'labelfraction',lf);    % sjv add timeout fraction to GUI
        next_row(y);                                                         %
        DispParam(obj,'timeout_violations',0,x,y,'labelfraction',lf);        %
        next_row(y);                                                         %
        ...
        newaxes = axes;
        SoloParamHandle(obj, 'axes1', 'saveable', 0, 'value', double(newaxes));
        ...
        newaxes = axes;
        SoloParamHandle(obj, 'axes2', 'saveable', 0, 'value', double(newaxes));
        ...
        newaxes = axes;
        SoloParamHandle(obj, 'axes3', 'saveable', 0, 'value', double(newaxes));
    ...
    case 'next_trial'
        ...
        if value(nTrials) > 0 % If the trial that we're preparing is NOT the first trial, record all the history stuff
            ...
            right_small_reward_visited = isfield(parsed_events.states, 'right_small_reward_state') && rows(parsed_events.states.right_small_reward_state) > 0;  % sjv updated to correctly find small reward states
            left_small_reward_visited = isfield(parsed_events.states, 'left_small_reward_state') && rows(parsed_events.states.left_small_reward_state) > 0;     %
            
            % sjv add water valve open times (for Chuck)
            right_water = nan; left_water = nan;
            if right_reward_visited && isfield(parsed_events.waves,'right_reward_wav')
                right_water = mean(parsed_events.waves.right_reward_wav);
            end
            if left_reward_visited && isfield(parsed_events.waves,'left_reward_wav')
                left_water = mean(parsed_events.waves.left_reward_wav);
            end
            if right_small_reward_visited && isfield(parsed_events.waves,'right_small_reward_wav')
                right_water =  mean(parsed_events.waves.right_small_reward_wav);
            end
            if left_small_reward_visited && isfield(parsed_events.waves,'left_small_reward_wav')
                left_water =  mean(parsed_events.waves.left_small_reward_wav);
            end
            ...
            % sjv - Update timeout violation counters
            if isfield(parsed_events.states, 'timeout_penalty') && rows(parsed_events.states.timeout_penalty) > 0
                timeout_violations.value = timeout_violations + 1;
                timeout_viol = 1;
            else
                timeout_viol = 0;
            end
            ...
            timeout_viol_history.value = [timeout_viol_history(:);timeout_viol];           % sjv add timeout history
            trial_type_history.value = [trial_type_history(:);trial_type];
            leftwatertime.value = [leftwatertime(:);left_water];
            rightwatertime.value = [rightwatertime(:);right_water];
            ...
            timeout_violation_frac.value = mean(timeout_viol_history(:));
    ...
    case 'update_plot'
        try
            ...
            if length(timeout_viol_history) ~= length(reward_history)                  % sjv update for timeout history
                timeout_viol_history.value = zeros(length(reward_history),1);          %
            end                                                                        %
            
            
            % get trials by outcome and trial type
            rewards_free = find(reward_history==1 & trial_type_history == 'f');
            unrewards_free = find(reward_history==0 & viol_history==0 & viol1_history==0 & timeout_viol_history==0 & trial_type_history == 'f');
            rewards_forced = find(reward_history==1 & trial_type_history ~= 'f');
            unrewards_forced = find(reward_history==0 & viol_history==0 & viol1_history==0 & timeout_viol_history==0 & trial_type_history ~= 'f');
            violations = find(viol_history==1 | viol1_history==1 | timeout_viol_history==1);                                         % sjv update for timout history
            ...
            leftviols = find(sides_history_1 == 'l' & (viol1_history == 1 | timeout_viol_history == 1));                             % sjv update for timeout history
            rightviols = find(sides_history_1 == 'r' & (viol1_history == 1 | timeout_viol_history == 1));                            %
            timeoutviols = find(timeout_viol_history == 1 & sides_history_1 ~= 'r' & sides_history_1 ~= 'l');
            ...
            yvals(rightrights) = 1;
            yvals(timeoutviols) = 2.5;                                                               % sjv update for timeout history
    ...
    case 'get_all'
        ...
        x.timeout_viol_history = value(timeout_viol_history);                    % sjv added timeout history
        ...
        x.leftwatertime = value(leftwatertime);                                  % sjv added left water valve time
        x.rightwatertime = value(rightwatertime);                                % sjv added right water valve time
