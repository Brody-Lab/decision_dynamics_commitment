...
switch action
    ...
    case 'init',
        ...
        % timeout params
        ToggleParam(obj,'use_timeouts',0,x,y,'OnString','Use timeouts during trial','OffString','Do not use timeouts during trial');
        next_row(y)
        set_callback(use_timeouts, {mfilename, 'toggle_use_timeouts'});
        set_callback_on_load(use_timeouts,1);
        NumeditParam(obj,'timeout_time',inf,x,y,'TooltipString','How long until the timeout happens (seconds)');
        next_row(y);
    ...
    case 'toggle_use_timeouts'
        if value(use_timeouts)
            enable(timeout_time);
        else
            disable(timeout_time);
        end
    ...
    case 'get_timing'
        ...
        x.use_timeouts = value(use_timeouts);
        x.timeout_time = value(timeout_time);
    otherwise,
