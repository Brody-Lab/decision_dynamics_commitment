...
switch action,
  ...
  case 'init'
    ...
    set(value(myfig),'Position',[1 screen_size(4)-740, 1000 700]); % put fig at top right
    set(gcf, 'Visible', 'off');
    x=10;y=10;
         
    SoloParamHandle(obj, 'ax', 'saveable', 0, ...
                   'value', axes('Position', [0.01 0.45 0.5 0.5]));
    ...
    SoloParamHandle(obj, 'axperf', 'saveable', 0, ...
                   'value', axes('Position', [0.5 0.45 0.5 0.5]));
    ylabel('log_e \sigma_2','FontSize',16,'FontName','Cambria Math');  
...
%     next_row(y);
    ...
    NumeditParam(obj,'s2_s1_ratio',2.7,x,y,'label','s2_s1_ratio','TooltipString','Intensity index i.e. Ind=(S1-S2)/(S1+S2)');
    next_row(y);
    %next_row(y);
        ...
        case 2,
            ...
            if value(trials_in_stage_on) > 1
                if violation_history(n_done_trials)~=1
                    ...
                    eval(sprintf('perfClass%d.value=(perfClass%d * (nTrialsClass%d - 1) +%d)/nTrialsClass%d;',thisclass(n_done_trials),thisclass(n_done_trials),...
                        thisclass(n_done_trials),hit_history(n_done_trials),thisclass(n_done_trials)));
                end
...
%      Xtick=get(value(ax),'XtickLabel');
    set(value(ax),'ytick',((yd(1:1+value(midclass_pairs):end/2))),'xtick',((xd(1:1+value(midclass_pairs):end/2))));
    set(value(ax),'yticklabel',num2str(exp(yd(1:1+value(midclass_pairs):end/2)),2),'xticklabel',num2str(exp(xd(1:1+value(midclass_pairs):end/2)),2));
...
%      Ytick=get(value(ax),'YtickLabel');
%      Xtick=get(value(ax),'XtickLabel');
    set(value(axperf),'ytick',((yd(1:value(numClass)))),'xtick',((xd(1:value(numClass)))));
    set(value(axperf),'yticklabel',num2str(exp(yd(1:value(numClass))),2),'xticklabel',num2str(exp(xd(1:value(numClass))),2));
    ylabel('\sigma_2 in log scale','FontSize',16,'FontName','Cambria Math');  
    set(value(axperf),'Fontsize',15)
    xlabel('\sigma_1 in log scale','FontSize',16,'FontName','Cambria Math')
    ...
    case 'get_class_perform'
        if nargout > 0,
            for ii=1:numClass*2+(numClass-1)*midclass_pairs
    ...
    case 'update_pair_history'
        ...
        ps(n_done_trials+1) = value(thisclass(n_done_trials+1));
