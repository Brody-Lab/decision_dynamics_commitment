...
switch action
    ...
    case 'prepare_next_trial',
        ...
        obj_water_off = bSettings('get', 'DIOLINES', 'D12'); %Ch6
        shutter       = bSettings('get', 'DIOLINES', 'bit0');  
        obj_water_on  = bSettings('get', 'DIOLINES', 'bit1_12'); %Ch8
        stimDIO       = bSettings('get', 'DIOLINES', 'bit1');    %Ch8
        %obj_water_off = bSettings('get', 'DIOLINES', 'bit2_12'); %Ch9
        ...
        %% THIS ISN'T WORKING
        SoundInterface(obj, 'set','NICSound','Dur1', value(CP_duration));
...
%         end
    ...
    %Convert trial into 15 bit binary word
    digitalword = dec2bin(value(n_done_trials+1), 15);
    %Parse into 5 words, each 3 bits
    bit1 = bin2dec(digitalword(1:3));
    bit2 = bin2dec(digitalword(4:6));
    bit3 = bin2dec(digitalword(7:9));
    bit4 = bin2dec(digitalword(10:12));
    bit5 = bin2dec(digitalword(13:15));
    word = [bit1 bit2 bit3 bit4 bit5];
    word = word+1;
    
    %Each word gets converted into an analog voltage
    ao_range = [0:1/7:1];
    word = ao_range(word);
    
    %Each word will be 4ms long (with 2kHz sampling rate, 4ms = 8 samples)
    binarywave = ones(8, length(word(1,:)));
    wavemultiplier = repmat(word, 8, 1);
    binarywave = binarywave.*wavemultiplier;
    binarywave = reshape(binarywave, 8*length(word(1,:)), 1);
    
    % Make digital handshake that is 4ms long
    handshakeword = [1 0 1];
    handshakewave = ones(8, length(handshakeword));
    handshakemult = repmat(handshakeword, 8, 1);
    handshakewave = handshakewave.*handshakemult;
    handshakewave = reshape(handshakewave, 8*length(handshakeword(1,:)),1);
    
    %Combine handshake and binary stamp:
    binarywave = [handshakewave; binarywave];
    %% I have been having trouble extracting the trialnum and I think it's 
    %% the analog voltages are exceeding the dynamic range of input voltages 
    %% for the scanimage NIDAQ card.  Will try to scale it down.
    binarywave = binarywave./2;
    ...
    %% ----------------Piston Commands-------------------------------------- 
       if value(CP_duration) > .6;
           ...
           late = (value(CP_duration)-0.6)*2000;
           latewaveform=ones(1,late)*value(thispressure);
           waveform = [early, latewaveform];%2000 = 1sec on this rig
       else
           waveform = ones(1,value(CP_duration)*1000)*value(thispressure);
       end
       
       if value(deploy_practicepistons)==1;
           practicewaveform=[ones(1,2000)*0.15];  %2000 = 1sec on this rig
       elseif value(deploy_practicepistons)==0;
           practicewaveform = [zeros(1,2000)];
       end
       
       sma = add_scheduled_wave(sma, 'name', 'pistons','ao_line', 1, 'analog_waveform', waveform, 'loop', 0);
       sma = add_scheduled_wave(sma, 'name', 'pistonspractice','ao_line', 1, 'analog_waveform', practicewaveform, 'loop', 0);
       sma = add_scheduled_wave(sma, 'name', 'shutter', 'preamble', 0.4, 'sustain', CP_duration-0.4,'DOut', shutter, 'loop', 0 );
       sma = add_scheduled_wave(sma, 'name', 'add_obj_water', 'preamble', 0.001, 'sustain', 0.5, 'DOut', obj_water_on, 'loop', 0);
       sma = add_scheduled_wave(sma, 'name', 'remove_obj_water', 'preamble', CP_duration-0.4, 'sustain', 0.4, 'DOut', obj_water_off, 'loop', 0);
       
       %% -----------Synchronizing Bcontrol and ScanImage
       sma = add_scheduled_wave(sma, 'name', 'talktoscanimage', 'ao_line', 2, 'analog_waveform', binarywave', 'loop', 0);
       ...
       if n_done_trials==0
        ...
        switch value(training_stage)
            case 0,
                if side=='l',
                    sma = add_scheduled_wave(sma, 'name', 'reward_delivery', 'preamble', 0, ...
        ...
        sma = add_scheduled_wave(sma, 'name', 'stimA1', 'preamble', PreStim_time,...
            'sustain', A1_time, 'sound_trig', A1_sound_id, 'DOut', stimDIO);
		        
            
        sma = add_scheduled_wave(sma, 'name', 'stimA2', 'preamble', PreStim_time+A1_time+Del_time, ...
		        'sustain', A2_time,'sound_trig', A2_sound_id, 'DOut', stimDIO);
		...
		switch value(training_stage)
            ...
            case 1  %% Quickly grow delay
                ...
                sma = add_state(sma,'name','cpslotonly','self_timer', SettlingIn_time, ...
                    'output_actions', {'SchedWaveTrig', 'center_poke + settling_period + go +pistons +talktoscanimage'}, ...
                    'input_to_statechange', {'Tup', 'cp_legal_cbreak_period_slot', ...
                    'Blo','violation_state', ...
                    'center_poke_Out', 'wait_for_collecting_reward_slot', ...
                    'Rin',  'violation_state', ...
			...
			case 2  %% Task begins
				...
				sma = add_state(sma,'name','cp','self_timer', SettlingIn_time+0.00001, ...
					'output_actions', {'SchedWaveTrig', 'center_poke + settling_period +stimA1+stimA2 +talktoscanimage + go'}, ...
					'input_to_statechange', {'Tup', 'cp_legal_cbreak_period', ...
					    'Clo','violation_state', ...
					    'center_poke_Out', 'wait_for_collecting_reward', ...
						'Rin',  'violation_state', ...
                    ...
                    if value(water_objective)==1
                        
%                         sma = add_state(sma,'name','cpslotonly','self_timer', SettlingIn_time+0.00001, ...
%                             'output_actions', {'SchedWaveTrig', 'center_poke +settling_period +stimA1+stimA2 +pistons +talktoscanimage +go'}, ...
%                             'input_to_statechange', {'Tup', 'cp_legal_cbreak_period_slot', ...
%                             'Blo','violation_state', ...
%                             'Dlo', 'release',...
%                             'center_poke_Out', 'wait_for_collecting_reward_slot', ...
...
%                             'Lout', 'violation_state'});
                        ...
                        sma = add_state(sma,'name','cpslotonly','self_timer', SettlingIn_time+0.00001, ...
                            'output_actions', {'SchedWaveTrig', 'center_poke +settling_period +stimA1+stimA2 +pistons +talktoscanimage +shutter +go +add_obj_water +remove_obj_water'}, ...
                            'input_to_statechange', {'Tup', 'cp_legal_cbreak_period_slot', ...
                            'Blo','violation_state', ...
                            'Dlo', 'release',...
                            'center_poke_Out', 'wait_for_collecting_reward_slot', ...
                    ...
                    sma = add_state(sma,'name','cpslotonly','self_timer', SettlingIn_time+0.00001, ...
					'output_actions', {'SchedWaveTrig', 'center_poke + settling_period +stimA1+stimA2 +pistons +talktoscanimage +shutter +go'}, ...
					'input_to_statechange', {'Tup', 'cp_legal_cbreak_period_slot', ...
					    'Blo','violation_state', ...
                        'Dlo', 'release',...
					    'center_poke_Out', 'wait_for_collecting_reward_slot', ...
                ...
                sma = add_state(sma,'name','cp_legal_cbreak_period', 'self_timer', 10000, ...
                    'input_to_statechange', {'Dlo', 'release', 'Clo', 'violation_state', ...
                    'center_poke_Out', 'wait_for_collecting_reward', ...
                    'Rin',  'violation_state', ...
                    'Rout', 'violation_state', ...
                    'Lin', 'violation_state', ...
                ...
                sma = add_state(sma,'name','cp_legal_cbreak_period_slot', 'self_timer', 10000, ...
                    'input_to_statechange', {'Dlo', 'release', 'Blo', 'violation_state',...
                    'center_poke_Out', 'wait_for_collecting_reward_slot', ...
                    'Rin',  'violation_state', ...
                    'Rout', 'violation_state', ...
                    'Lin', 'violation_state', ...
                ...
                sma = add_state(sma, 'name', 'wait_for_collecting_reward_slot', 'self_timer', 30000, ...
                    'output_actions', {'SchedWaveTrig', '-pistons','DOut', LEDOn*SideLight}, ...
                    'input_to_statechange',{HitEvent, HitState, ErrorEvent, 'second_hit_state',...
                    'Tup', 'nomove'});
				...
				sma = add_state(sma, 'self_timer', LeftWValveTime, ...
					'output_actions', {'DOut', SideLight+left1water,'SchedWaveTrig',...
                    'pistonspractice'},'input_to_statechange',{'Tup','hit_state'});
				...
				sma = add_state(sma, 'self_timer', RightWValveTime, ...
					'output_actions', {'DOut', SideLight+right1water,'SchedWaveTrig',...
                    'pistonspractice'},'input_to_statechange',{'Tup','hit_state'});
                ...
                sma = add_state(sma, 'self_timer', reward_delay, ...
                    ...
                    'SchedWaveTrig', 'pistonspractice'},'input_to_statechange', {'Tup', 'current_state+1'});
				sma = add_state(sma, 'self_timer', LeftWValveTime, ...
        ...
        if value(water_objective==1)
            sma = add_state(sma, 'name', 'release', 'self_timer', 0.001, 'output_actions',...
                {'SchedWaveTrig', '-pistons-add_obj_water-remove_obj_water-shutter'},'input_to_statechange',...
                {'Tup', 'violation_state'});
        else
            sma = add_state(sma, 'name', 'release', 'self_timer', 0.001, 'output_actions',...
                {'SchedWaveTrig', '-pistons-shutter'},'input_to_statechange',...
                {'Tup', 'violation_state'});
        end
        ...
        if stimuli_on ==0
            ...
            if value(water_objective)==1
                sma = add_state(sma,'name','violation_state','self_timer',viol_snd_duration,...
                    'output_actions',{'SchedWaveTrig', '-center_poke-stimA1-stimA2-pistons-go-add_obj_water-remove_obj_water-shutter', ...
                    'SoundOut',viol_sound_id},...
                    'input_to_statechange', {'Tup', 'current_state+1'});
            else
                sma = add_state(sma,'name','violation_state','self_timer',viol_snd_duration,...
                    'output_actions',{'SchedWaveTrig', '-center_poke-stimA1-stimA2-pistons-go-shutter', ...
                    'SoundOut',viol_sound_id},...
                    'input_to_statechange', {'Tup', 'current_state+1'});
            end
        end
		...
		prepare_next_trial_states = {'hit_state','second_hit_state','second_hit_state_slot','violation_state','timeout_state', 'nomove', 'movestage'};
