...
%% init
  case 'init',
    ...
    SoloFunctionAddVars('PBupsHeadFixDev', 'rw_args', {'previous_sides',...
                                                 'previous_sounds',...
...
%% make_and_send_summary
  case 'make_and_send_summary',
    ...
    for tx=1:min([numel(pd.hits),numel(pd.bupsdata)]) %lengths can be different if it's a live save or from a reconstituted ASV file -Chuck 
         nl(tx)=sum(pd.bupsdata{tx}.left);
         nr(tx)=sum(pd.bupsdata{tx}.right);     
    end
    ...
    for i = 1:numel(parsed_events_history)
        ...
        if pd.sides(i) == 'r'
            right = parsed_events_history{i}.waves.direct_reward;
            left = [];
        end
...
%% update_plot
  case 'update_plot',
    ...
    previous_plot.value = [hb; hg; hr; hk; hst];
