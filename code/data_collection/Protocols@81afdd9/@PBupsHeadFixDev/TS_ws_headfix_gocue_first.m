...
function varargout = TS_ws_headfix_gocue_first(obj, action, varargin)
        ...
        switch curriculum_stage_number
            case 1
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    ...
                    StimRewardWait.value = 1; % animal can never lick during stim

                    slick_timeout.value = 10;

                    T_min.value = 0.1; %use T values to control stim on->go sound length
                    T_max.value = 0.2;
            ...
            case 2
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    ...
                    miss_iti.value = 5; % long ITI if animal licks pre go cue

                    stim_delay_Min.value = 0.1;
                    stim_delay_Max.value = 0.2;
                end
            ...
            case 4
                stage_description.value = 'give on, but punish incorrect responses,lick cor spout after stim ends for reward';
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    PunishSideChoice.value = 1; % punish incorrect responses
                    give_hint.value = 1; %give hint on
                    give_perc.value = 0.3; % perc of total reward to give
                end
            case 5
    ...
    case 'get_eod_logic'
        ...
        switch curriculum_stage_number
            ...
            case 2
                ...
                if value(nRewarded) > 100 && value(nViolation)/value(nTrials) < 0.2 %animal must be successfully waiting to lick until go cue 
            ...
            case 3
                if value(nRewarded) > 100 && value(nViolation)/value(nTrials) < 0.2 % same req with sound on (new sounds may induce early licking)
                    TrainingSection(obj, 'increment_stage');
                end
            case 4
                if value(nRewarded) > 100 && value(mean_hitfrac) > 0.8 && value(nViolation)/value(nTrials) < 0.2
                        TrainingSection(obj, 'increment_stage');
                end
            case 5
                if value(nRewarded) > 100 && value(mean_hitfrac) > 0.8 && value(nViolation)/value(nTrials) < 0.2% EDIT THIS TO BE 120 && value(n_days_stage) > 3 % animal must successfully use rule for 3 days
                        TrainingSection(obj, 'increment_stage');
                end
            case 6
                if value(nRewarded) > 120 && value(mean_hitfrac) > 0.75 && value(nViolation)/value(nTrials) < 0.2 %if daily trials is high and hitfrac is high, make harder 
            ...
            case 7
                if value(nRewarded) > 150 && value(mean_hitfrac) > 0.75  && value(nViolation)/value(nTrials) < 0.2 %if daily trials is high and hitfrac is high, make harder
