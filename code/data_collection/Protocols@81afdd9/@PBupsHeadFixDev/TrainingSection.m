...
%         if ~isfolder(curriculum_folder); addpath(curriculum_folder); end
        ...
        % curriculum vars in GUI
        NumeditParam(obj, 'block_count', 1, x, y, 'position', [x y 200 20],...
            'label', 'blk count', 'labelfraction',0.5,...
            'TooltipString', 'Number of blocks completed today');
        next_row(y,1);
        ...
        next_row(y,1);
        % DB 3 -> 15?
        NumeditParam(obj, 'block_size', 50, x, y, 'labelfraction',0.5,...
            'TooltipString', 'number of trials in a block',...
            'label', 'blk size', 'position', [x y 100 20]);
        % DB 15 -> 80
        NumeditParam(obj, 'blocks_end', 200, x, y, 'labelfraction',0.5,...
            'TooltipString', 'number of trials in stage where blocks turn off',...
            'label', 'blk end', 'position', [x+100 y 100 20]);            
        next_row(y,1);
        NumeditParam(obj, 'next_complete_trial_goal', 0, x, y, 'labelfraction',0.5,...
            'TooltipString', 'next nCorrect value to trigger block switch (updated to allow for block change within session/stage)',...
            'label', 'rew goal', 'position', [x y 100 20]);        
        ...
        SoloFunctionAddVars('RewardsSection', 'ro_args',...
          ...
          'stage_name_persist'});
        SoloFunctionAddVars('RewardsSection', 'rw_args',...
        ...
        training_section_send_out_vars = get_name(get_sphandle('owner', '@PBupsHeadFixDev', 'funcowner', 'TrainingSection'));
