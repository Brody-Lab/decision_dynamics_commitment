...
function varargout = TS_ws_headfix_gocue_block_rule(obj, action, varargin)
        ...
        switch curriculum_stage_number
            case 1
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    ...
                    T_min.value = 0.3; %use T values to control stim on->go sound length
                    T_max.value = 0.3;
            ...
            case 4
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    PunishStimLicks.value = 1; % punish pre go licks
                    PunishSideChoice.value = 1; % no cues so no punishments
        ...
        switch curriculum_stage_number
                ...
                if value(nRewarded) > 120 && mean(value(LtHits)) > 0.7 && mean(value(RtHits)) > 0.7 && value(nViolation)/value(nTrials) < 0.3%if daily trials is high and hitfrac is high, make harder 
                    if value(T_min) >= 0.5 && value(T_max) >= 1.5 
                        if value(viol_grace_dur) == 0
                            ...
                            viol_grace_dur.value = max(0,value(viol_grace_dur)-.1);
                        end                 
                    end
            ...
            case 8
                if value(nRewarded) > 120 && mean(value(LtHits)) > 0.7 && mean(value(RtHits)) > 0.7  && value(nViolation)/value(nTrials) < 0.3  %if daily trials is high and hitfrac is high, make harder 
                    ...
                    if value(T_max) >= 1.5 && value(viol_grace_dur) > 0
                        viol_grace_dur.value = max(0,value(viol_grace_dur)-.1);
                    end
                    if value(T_max) < 1.5
                        T_max.value = value(T_max) + .1;
