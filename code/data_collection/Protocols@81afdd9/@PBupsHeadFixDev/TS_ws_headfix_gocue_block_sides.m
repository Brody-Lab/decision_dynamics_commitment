...
function varargout = TS_ws_headfix_gocue_block_sides(obj, action, varargin)
...
switch action,
    ...
    case 'get_eod_logic'
        ...
        switch curriculum_stage_number
            ...
            case 4
                if value(nRewarded) > 120 && value(LtHits) > 0.7 && value(RtHits) > 0.7 && value(nViolation)/value(nTrials) < 0.3% EDIT THIS TO BE 120 && value(n_days_stage) > 3 % animal must successfully use rule for 3 days
            ...
            case 5
                if value(nRewarded) > 120 && value(LtHits) > 0.7 && value(RtHits) > 0.7 && value(nViolation)/value(nTrials) < 0.3 %if daily trials is high and hitfrac is high, make harder 
