...
switch action,
        ...
        switch curriculum_stage_number
            case 1
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    ...
                    T_min.value = 0.001; %use T values to control stim on->go sound length
                    T_max.value = 0.001;
            ...
            case 2
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    ...
                    % increase vals to be non trivial -- total time between stim start off and go cue start is 0.15-0.2s (add stim start delay)
    ...
    case 'get_eod_logic'
        ...
        switch curriculum_stage_number
            ...
            case 4 % dynamically give hints on next session if the mouse is performing poorly
                if value(give_hint) == 0 % if we are not giving hints
                    if value(nRewarded) > 100 && value(mean_hitfrac) > 0.8 && value(nViolation)/value(nTrials) < 0.3 %check performance and move on if good
                            TrainingSection(obj, 'increment_stage'); 
                    end
                    % if value(mean_hitfrac) < 0.6 || value(nViolation)/value(nTrials) > 0.7 %if performance is especially poor, give hints next session
                    %     give_hint.value = 1;
                    % end
                end
                if value(give_hint) == 1 % if hints are on, and performance is good, turn off hints. otherwise leave on.
                    if value(nRewarded) > 100 && value(mean_hitfrac) > 0.8 && value(nViolation)/value(nTrials) < 0.3
                            give_hint.value = 0;
                    end
                end
            ...
            case 6 
                if value(nRewarded) > 120 && value(mean_hitfrac) > 0.75 && value(nViolation)/value(nTrials) < 0.3
                    TrainingSection(obj, 'increment_stage');
                end                    
            case 7
