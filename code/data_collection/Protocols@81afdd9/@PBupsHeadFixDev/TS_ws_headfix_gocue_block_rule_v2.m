...
function varargout = TS_ws_headfix_gocue_block_rule_v2(obj, action, varargin)
...
switch action,
        ...
        switch curriculum_stage_number
            case 1
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    ...
                    stim_window.value = 1; %stimulus window with mean trial length
                ...
                elseif value(nRewarded) == 10
                    give_hint.value = 0; %give hint off after 10 trials, don't make > so that it can be turned back on during training
                end
            case 2
            ...
            case 4
                ...
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    ...
                    PunishStimLicks.value = 1; % punish pre go licks but add long grace period
                    viol_grace_dur.value = 0.4; % 400ms grace period, to discourage continuous licking
                    ...
                    block_size.value = 20; % block size
                    blocks_end.value = 200;
                    block_count.value = 1;
                    next_complete_trial_goal.value = value(block_size); %first goal is block size
                ...
                if value(nCorrect) == value(next_complete_trial_goal) && value(nCorrect) < value(blocks_end)
                    block_count.value = value(block_count) + 1;
                    if mod(value(block_count),2) == 1 && value(block_size) > 0
                        block_size.value = value(block_size) - 5;
                    end
                    if value(in_left_block) == 1
                        ...
                        L_pprobs.value = 0;
                        R_pprobs.value = 1;
                        next_complete_trial_goal.value = value(next_complete_trial_goal) + value(block_size);
                ...
                if value(nCorrect) > value(blocks_end) || value(block_size) == 0
                    in_left_block.value = 0;
                    in_right_block.value = 0;
                    L_pprobs.value = 0.5;
                    R_pprobs.value = 0.5;
            ...
            case 8
                stage_description.value = 'change frac of gammas';
                if value(n_trials_stage) < 2 & value(n_days_stage) == 1
                    hardest.value = 0.5;
                    easiest.value = 3.5;
                end
            case 9
                stage_description.value = 'end state';
        end
    ...
    case 'get_eod_logic'
        ...
        switch curriculum_stage_number
            ...
            case 2 
                if value(nRewarded) > 120 && value(nViolation)/value(nTrials) < 0.3 %animal must be successfully waiting to lick until go cue 
                    if value(stim_window) >= 0.5
                        T_max.value = value(stim_window);
                        viol_grace_dur.value = value(T_max)-.1;
                        TrainingSection(obj, 'increment_stage');                      
                    end
                    if round(value(stim_window),1) < 0.5 % If we made the task easier, we need to then grow times again for subsequent stage
                        ...
                        viol_grace_dur.value = stim_window-0.1; % set grace period to be 100ms less than stim window
                    end
                end
            case 3
            ...
            case 4
                if value(nRewarded) > 120 && mean(value(LtHits)) > 0.7 && mean(value(RtHits)) > 0.7 && value(nViolation)/value(nTrials) < 0.3 && value(block_size) < 5%animal must be successfully waiting to lick until go cue 
                    TrainingSection(obj, 'increment_stage');
                else
                    block_size.value = 20;
                    next_complete_trial_goal.value = value(block_size);
                    block_count.value = 1;
                end
            case 5
