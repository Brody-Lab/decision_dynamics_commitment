...
function varargout = TS_ws_headfix_v2(obj, action, varargin)
...
switch action,
    ...
    case 'get_update'
        ...
        switch curriculum_stage_number
            ...
            case 4
                stage_description.value = 'lick cor spout after stim ends for reward, start stepping down stim';
                if value(n_trials_stage) < 2
            ...
            case 6
                stage_description.value = 'decrease stim min dur to 0.5 slowly';
                if value(n_trials_stage) < 2
                    T_min.value = 1; %ensures we're starting at 1 sec
                end
            case 7
        ...
        switch curriculum_stage_number
            ...
            case 3
                if value(nRewarded) > 120 && value(mean_hitfrac) > 0.8
                    if value(T_min) > 2 && value(T_max) > 5
                        T_min.value = value(T_min) - 1;
                        T_max.value = value(T_max) - 1;
                    elseif value(T_min) <= 2 && value(T_max) <= 5
                        TrainingSection(obj, 'increment_stage');
                    end
                end
            case 4
            ...
            case 5
                if value(nTrials) > 150 && value(mean_hitfrac) > 0.75 %if daily trials is high and hitfrac is high, make harder
                    if value(easiest) <= 4 && value(hardest) <= 2
                        TrainingSection(obj, 'increment_stage');
                    end
                    if value(easiest) > 4
