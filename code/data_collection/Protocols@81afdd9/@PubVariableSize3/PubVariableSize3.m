...
function [obj] = PubVariableSize3(varargin)
...
%% INIT 
  case 'init'
    ...
    SoloParamHandle(obj,'SnugCount','value',3);
    ...
    for i = 2:2:(value(SnugCount)*2)
        lineorder{end+1} = alldio{i,1}(1:end-3); %#ok<AGROW>
    end
    ...
    for i = 1:value(SnugCount)
        ...
        elseif ~isnan(waterdio(i)) && ~isempty(activerats{i})
            ...
            PubVariableSize3(obj,'set_slot',{i,activerats{i}});
    ...
    for i = 1:value(SnugCount)
        if ~isempty(activerats{i})
            PubVariableSize3(obj,'get_rat_data',i);
        end
    end
    ...
    PubVariableSize3(obj,'manual_test');
  ...
  case 'prepare_next_trial'
    ...
    if n_done_trials > 0
        ...
        if sum(strcmp(s,'reset_state')) > 0 && ~isempty(parsed_events.states.reset_state)
            ...
            pause(1)
            PubVariableSize3(obj,'end_session');
            return;
        end
    ...
    if n_done_trials == 1
        ...
        if value(DoneReboot) == 0 %#ok<NODEF>
            if value(DoneTraining) == 0  %#ok<NODEF>
                ...
                PubVariableSize3(obj,'wait_for_training_end'); 
            end
            ...
            for i = 1:numel(wst)
                if isnan(wst(i)) && aw(i) == 1
                    ...
                    PubVariableSize3(obj,'update_bars',{i,'rig_water'});
            ...
            for i = 1:value(SnugCount)
                PubVariableSize3(obj,'update_bars',{i,''});
            end

            if all(dr <= 0)
                %RunningSection(dispatcher,'RunButtonCallback');
                PubVariableSize3(obj,'wait_for_reboot');
...
%% set_slot1_callback
    case 'set_slot1_callback'
        PubVariableSize3(obj,'set_slot',1);
        
%% set_slot2_callback
    case 'set_slot2_callback'
        PubVariableSize3(obj,'set_slot',2);
        
%% set_slot3_callback
    case 'set_slot3_callback'
        PubVariableSize3(obj,'set_slot',3);
        
%% set_slot4_callback
    case 'set_slot4_callback'
        PubVariableSize3(obj,'set_slot',4);
        
%% set_slot5_callback
    case 'set_slot5_callback'
        PubVariableSize3(obj,'set_slot',5);

%% set_slot6_callback
    case 'set_slot6_callback'
        PubVariableSize3(obj,'set_slot',6);
...
%% session1
  case 'Session1_callback'
      PubVariableSize3(obj,'update_session',1);  
        
%% session1
  case 'Session2_callback'
      PubVariableSize3(obj,'update_session',2);  
        
%% session1
  case 'Session3_callback'
      PubVariableSize3(obj,'update_session',3);  
        
%% session1
  case 'Session4_callback'
      PubVariableSize3(obj,'update_session',4);  
        
%% session1
  case 'Session5_callback'
      PubVariableSize3(obj,'update_session',5);  
        
%% session1
  case 'Session6_callback'
      PubVariableSize3(obj,'update_session',6);
        
%% session1
  case 'Session7_callback'
      PubVariableSize3(obj,'update_session',7);  
        
%% session1
  case 'Session8_callback'
      PubVariableSize3(obj,'update_session',8);  
        
%% session1
  case 'Session9_callback'
      PubVariableSize3(obj,'update_session',9);  
...
%% update_session
  ...
  for j = 1:value(SnugCount)
      ...
      PubVariableSize3(obj,'get_rat_data',j);
  end
...
%% end_session    
  ...
  case 'end_continued'
      ...
      if value(stopping_process_completed) %This is provided by dispatcher
          ...
          %This is where we should restart matlab with PubVariableSize3
          %running
