...
%% INIT 
  case 'init'
    ...
    SoloParamHandle(obj,'IsRatRig','value',is_rat_rig);   %1 if it's a rat pub, 0 if it's a mouse pub
    
    if is_rat_rig
        SoloParamHandle(obj,'WaterDropVol',  'value',36);   %Volume in ul of each water drop
        SoloParamHandle(obj,'WaterPulse',    'value',2);    %Time in seconds between successive water drops
        SoloParamHandle(obj,'MinWaterPerent','value',3);    %Minimum percent body mass the animal must drink
    else
        % This is a mouse rig, lower water amts
        SoloParamHandle(obj,'WaterDropVol',  'value',3);    %Volume in ul of each water drop
        SoloParamHandle(obj,'WaterPulse',    'value',3);    %Time in seconds between successive water drops
        SoloParamHandle(obj,'MinWaterPerent','value',4);    %Minimum percent body mass the animal must drink
    end
    ...
    %Set the number of snugs
    SoloParamHandle(obj,'SnugCount','value',4);
  
    MP = get(0,'MonitorPositions');
    set(value(myfig), 'Position', [1 (MP(4)/2)-150 MP(3) 415]);
    
    W = MP(3)/value(SnugCount);
    F = (MP(3)/6)/320;
    SoloParamHandle(obj,'FontScale','value',F);
    
    
    for i=1:value(SnugCount) - 1
    ...
    for i = 1:value(SnugCount)
        ...
        %This line will now be drawn at MinWaterPerent rather than hard coded at 3%
        ...
        %button to click when broken snug is fixed
        ...
        if i == value(SnugCount)
            %tech name menu
            MenuParam(obj,'TechMenu',allexp,1,0,0,'position',[(W*(i-1))+(W/2) 350 (W/2)-10 35],'labelfraction',0.02);   
            set(get_ghandle(TechMenu),'Fontsize',24*F,'BackgroundColor',[1 1 1]); %#ok<NODEF>
            set_callback(TechMenu,{mfilename,'update_tech_callback'});
        end
    end
      
    for i=1:9
        %Session buttons
        PushbuttonParam(obj,['Session',num2str(i)],0,0,'label',num2str(i),'position',[((MP(3)/15)*(i-1))+5 350 (MP(3)/15)-10 60]);
        ...
        %Bar to show the active session
        SubheaderParam(obj,['SessionBox',num2str(i)],'',0,0,'position',[((MP(3)/15)*(i-1))+5 342 (MP(3)/15)-10 7]);
        set(get_ghandle(eval(['SessionBox',num2str(i)])),'BackgroundColor',[1 1 1]);
    end
    
    %The big red button that does everything
    i = 10;
    PushbuttonParam(obj,'RedButton',0,0,'label','Click to Load Pub','position',[((MP(3)/15)*(i-1))+5 350 340 60]);
    set(get_ghandle(eval('RedButton')),'Fontsize',22*F,'BackgroundColor',[0 1 1]);    
    set_callback(eval('RedButton'),{mfilename,'RedButton_callback'});
    
    SoloParamHandle(obj,'RedButtonPort',     'value',bSettings('get','INPUTLINES','X'));   %Port for input X
    ...
    [diosort,dioorder] = sortrows(cell2mat(alldio(:,2))); %#ok<ASGLU>
    dioname = alldio(dioorder,1);
    
    lineorder = cell(0);
    for i = 2:2:(value(SnugCount)*2)
        lineorder{end+1} = dioname{i}(1:end-3); %#ok<AGROW>
    end
    SoloParamHandle(obj,'LineOrder','value',lineorder);
    
    %possible_inputs = {'A','B','C','D','E','F'};
    possible_inputs = {'L','C','R','D','E','F'};
    pokeorder        = possible_inputs(1:value(SnugCount));
    pokeorder{end+1} = 'X';
    SoloParamHandle(obj,'PokeOrder','value',pokeorder);
    
    line_names_temp = value(PokeOrder);
    line_names = [];
    for i = 1:numel(line_names_temp)
        line_names(end+1) = line_names_temp{i}; %#ok<AGROW>
    end
    line_names = char(line_names);
    SoloParamHandle(obj,'LineNames','value',line_names);
    
    waterdio = [];
    leddio   = [];
    order = value(LineOrder);
    for j = 1:value(SnugCount)
        for i=1:size(alldio,1)
            if ~isempty(strfind(lower(alldio{i}),order{j}))
                if ~isempty(strfind(lower(alldio{i}),'water')) %#ok<STREMP>
                    waterdio(end+1) = alldio{i,2}; %#ok<AGROW>
                end
                if ~isempty(strfind(lower(alldio{i}),'led')) %#ok<STREMP>
    ...
    SoloParamHandle(obj,'RatMass',        'value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'WaterEarned',    'value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'WaterEarnedHere','value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'DropsRemaining', 'value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'DropsNotDrunk',  'value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'WaterStartTime', 'value',  nan(1,value(SnugCount)));
    SoloParamHandle(obj,'TrainingEndTime','value',  nan(1,value(SnugCount)));
    SoloParamHandle(obj,'CurrSess',       'value',0);
    SoloParamHandle(obj,'IsBroken',       'value',zeros(1,value(SnugCount)));
    
    SoloParamHandle(obj,'CompRats',       'value',cell(0));
    
    SoloParamHandle(obj,'Complete',       'value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'DoneReboot',     'value',0);
    SoloParamHandle(obj,'DoneTraining',   'value',0);
    SoloParamHandle(obj,'FinalCheck',     'value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'UpdatedRigwater','value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'PokeDetected',   'value',zeros(1,value(SnugCount)));
    SoloParamHandle(obj,'TechInitials',   'value','');
    
    WRIGS = str2num(bSettings('get','WATERRIG','water_rig_ids')); %#ok<ST2NM>
    if numel(WRIGS) < value(SnugCount); WRIGS(end+1:value(SnugCount)) = nan; end
    ...
    %enter a loop to hold and update rats in snugs until we hit the red button
    update_time = now - 1;
    while strcmp(get(get_ghandle(RedButton),'string'),'Click to Load Pub') ||...
          strcmp(get(get_ghandle(RedButton),'string'),'Select Your Name')  
        if (now - update_time) * 24 * 3600 < 60
            if all(get(get_ghandle(eval('RedButton')),'BackgroundColor') == [0,1,1])
                set(get_ghandle(eval('RedButton')),'BackgroundColor',[0 1 0]);
            else
                set(get_ghandle(eval('RedButton')),'BackgroundColor',[0 1 1]);
            end
            pause(1);
        else
            set(get_ghandle(RedButton),'enable','off','BackgroundColor',[1 1 0],'string','Updating...'); pause(0.1);
            output = WM_assign_rats_to_snugs;
            
            CurrSess.value     = output.currsess;
            RatWaterList.value = output.RWL;
            currrats           = output.currrats;
            comp               = output.comp;
            broken_snugs       = output.broken_snugs;
            
            for i=1:9
                if comp(i) == 1; set(get_ghandle(eval(['Session',num2str(i)])),'BackgroundColor',[0 1 1]); 
                else,            set(get_ghandle(eval(['Session',num2str(i)])),'BackgroundColor',[1 1 1]);
                end
                set(get_ghandle(eval(['SessionBox',num2str(i)])),'BackgroundColor',[1 1 1]);
            end
            set(get_ghandle(eval(['SessionBox',num2str(value(CurrSess))])),'BackgroundColor',[0 0 0]);

            activerats = cell(1,4);
            BRK = zeros(1,4);
            for i = 1:4
                activerats{i} = '';
                for j = 1:size(output.assignments,1)
                    if output.assignments{j,3} == WRIGS(i)
                        activerats{i} = output.assignments{j,1};
                        
                        rig = ['Rig',sprintf('%03d',output.assignments{j,2})]; %#ok<NASGU>
                        eval(['WaterRig',num2str(i),'.value = [num2str(WRIGS(i)),''  '',rig];']);
                    end
                    if sum(broken_snugs == WRIGS(i)) > 0
                        set(get_ghandle(eval(['Status',num2str(i)])),'string','BROKEN',...
                            'backgroundcolor',[1,0,0]);
                        set(get_ghandle(eval(['Fix',num2str(i)])),'visible','on');
                    else
                        set(get_ghandle(eval(['Status',num2str(i)])),'string','Waiting...',...
                            'backgroundcolor',[1,1,1]);
                        set(get_ghandle(eval(['Fix',num2str(i)])),'visible','off');
                    end
                end
                Pub4(obj,'set_slot',{i,activerats{i}});
                x = find(strcmp(AllRats,activerats(i))==1);
                set(get_ghandle(eval(['Menu',num2str(i)])),'value',x);
                if sum(broken_snugs == WRIGS(i)) > 0; BRK(i) = 1; else, BRK(i) = 0; end
            end
            
            prev_pos = get(get_ghandle(TechMenu),'value');
            curr_tech = bdata('select top from ratinfo.training_room where tower=2');
            curr_tech = curr_tech{1};
            pos = find(curr_tech == ' ',1,'first');
            if isempty(pos); pos = numel(curr_tech)+1; end
            curr_tech = curr_tech(1:pos - 1);
            pos = find(strcmp(allexp,curr_tech),1,'first');
            if isempty(pos); pos = 1; end
             
            if pos == 1 && prev_pos ~= 1; pos = prev_pos; end
            set(get_ghandle(TechMenu),'value',pos);
            
            initials = bdata(['select initials from ratinfo.contacts where experimenter="',curr_tech,'"']);
            if ~isempty(initials)
                TechInitials.value = initials{1};
            else
                TechInitials.value = 'XX';
            end
                
            set(get_ghandle(RedButton),'BackgroundColor',[0 1 0],'string','Click to Load Pub'); 
            
            if pos == 1; set(get_ghandle(RedButton),'enable','off','string','Select Your Name');
            else,        set(get_ghandle(RedButton),'enable','on','string','Click to Load Pub');
            end
            pause(0.1);
            update_time = now;
    ...
    if value(CurrSess) >= 8 && str2num(datestr(now,'HH')) > 12 %#ok<ST2NM>
        %Session 8 gets loaded before midnight but watered after therefore
        ...
        %If it's before noon we may be watering 8 a little late, i.e. load
    ...
    CompRats.value = comprats;
    IsBroken.value = BRK;
    
    for i = 1:value(SnugCount)
        if ~isempty(activerats{i})
            Pub4(obj,'get_rat_data',i);
        end
    end
    ...
    SoloFunctionAddVars('SMASection', 'ro_args', {'WaterDIO','LEDDIO','ActiveWater',...
        'ValveOpenTimes','WaterPulse','CycleDur','TrialDur','IsBroken','LineOrder',...
        'PokeOrder','LineNames','SnugCount'});
    ...
    for i = 1:value(SnugCount)
        if ~isnan(waterdio(i)) && BRK(i) == 0 %~isempty(activerats{i})
            ...
            set(get_ghandle(eval(['Status',num2str(i)])),'string','Start Manual Test','fontsize',24*value(FontScale));
        end
    end
    ...
    SoloParamHandle(obj,'WateringIDs','value',zeros(1,value(SnugCount))*nan);
    
    update_riginfo;
    
    Pub4(obj,'manual_test');
    ...
    if n_done_trials == 1
        ...
        for i=1:value(SnugCount)
            ...
            if ~isempty(activerat) && strcmp(activerat,'BROKEN')==0
                ...
                ctemp = bdata(['select complete from ratinfo.rigwater where ratname="',activerat,'" and dateval="',datestr(now,'yyyy-mm-dd'),'"']);
                if ~isempty(ctemp) && any(ctemp == 1)
                    comp(i) = 1;
                    Complete.value = comp;
                end
                ...
                if comp(i)==0
                    ...
                    %Let's do the write to MySQL ratinfo.water table here
                    %so we don't need to run a trial to have them have an
                    %entry in that table
                    ...
                    bdata(['insert into ratinfo.water set date="',wdate,...
        ...
        if value(DoneReboot) == 0 %#ok<NODEF>
            if value(DoneTraining) == 0  %#ok<NODEF>
                ...
                Pub4(obj,'wait_for_training_end'); 
            end
            dr      = value(DropsRemaining); %#ok<NODEF>
            dnd     = value(DropsNotDrunk); %#ok<NODEF>
            ...
            if n_done_trials == 2
                ...
                for i = 1:value(SnugCount)
                    ...
                    if ~contains(currstatus{i},'COMPLETE') && ~strcmp(currstatus{i},'BROKEN')
                        set(get_ghandle(eval(['Status',num2str(i)])),'string','Wait 03:00');
                    end
                end
                hardpausedur = 0.5;
                w = hardpausedur;
                while w > 0
                    loopstart = now;
                    w = hardpausedur - ((now - hardpausestart) * 24 * 60);
                    if w > 0
                        m = floor(w);
                        s = floor((w - m)*60);
                ...
                for i = 1:value(SnugCount)
                    ...
                    if ~isempty(activerat) && strcmp(activerat,'BROKEN')==0
                        ...
                        %chance the 3 minute hard pause will run past the
            ...
            elseif n_done_trials > 2
                ...
                for i = 1:value(SnugCount)
                    ...
                    activerat = get(get_ghandle(eval(['Rat',num2str(i)])),'string');
                    if ~strcmp(currstr,'Still Training') && ~contains(currstr,'COMPLETE') && ~strcmp(activerat,'BROKEN') &&...
                ...
                if rem(n_done_trials,10) == 0
                    ...
                    for i = 1:value(SnugCount)
                        if aw_old(i) == 1 && comp(i) == 0
                            ...
                            bdata('call ratinfo.update_water_all("{S}","{S}","{S}","{S}","{Si}")',...
                                datestr(now,'HH:MM:SS'),... %stoptime
                                weh(i),...                  %volume
                                (we(i)/rm(i))*100,...       %percent_bodymass
            ...
            DropsNotDrunk.value  = dnd;
            DropsRemaining.value = dr; %This is new, don't know why we didn't have this before, may break things?
            ...
            for i = 1:numel(value(WaterDIO))
                ...
                elseif dr(i) > 0 && comp(i)==0
                    ...
                    set(get_ghandle(eval(['Status',num2str(i)])),'string',...
                        [sprintf('%2.3f',dr(i)*value(WaterDropVol)/1000),'ml | ',sprintf('%02.0f',m),':',sprintf('%02.0f',s)],...
                ...
                elseif ~isempty(get(get_ghandle(eval(['Rat',num2str(i)])),'string')) &&...
                    ...
                    %complete
                    % goal: if times up, will dispaly how much water was left 
                    if dnd(i) < 0; dnd(i) = 0; end
                    set(get_ghandle(eval(['Status',num2str(i)])),'string',...
                        ['COMPLETE ',sprintf('%2.3f',dnd(i)*value(WaterDropVol)/1000),'ml'],...
                        'ForegroundColor',[0 0 0],'BackgroundColor',[1 1 1]);
                    ...
                    if comp(i) == 0 || isnan(WID(i))
                        ...
                        try %#ok<TRYNC>
                            ...
                            if isempty(id)
                                ...
                                bdata('call ratinfo.update_rigwater_complete("{Si}","{Si}")',1,id(end));
                            end
                        end
                        comp(i) = 1;
                    end
            ...
            for i = 1:numel(wst)
                if isnan(wst(i)) && aw(i) == 1
                    ...
                    Pub4(obj,'update_bars',{i,'rig_water'});
            ...
            for i = 1:value(SnugCount)
                Pub4(obj,'update_bars',{i,''});
            end

            if all(dr <= 0)
                %RunningSection(dispatcher,'RunButtonCallback');
                Pub4(obj,'wait_for_reboot');
...
case 'send_empty_state_machine'
        ...
        is_bpod_rig = bSettings('get','RIGS','bpod');
        if isnan(is_bpod_rig); is_bpod_rig = 0; end
...
%% get_rat_data
  case 'get_rat_data'
    ...
    waterearnedhere = value(WaterEarnedHere); %#ok<NODEF>
    ...
    if ~isempty(activerat) && ~strcmp(activerat,'BROKEN')
        ...
        if value(IsRatRig) == 1
            ...
            m = bdata(['select mass from ratinfo.mass where ratname="',activerat,'" order by date']);
            m(m == 0) = [];
            if numel(m) >= 7
                ratmass(i) = max(m(1:7));
            elseif ~isempty(m)
        ...
        %Let's see if the rat has been in the pub and gotten any water there
        temp = bdata(['select volume from ratinfo.water where rat="',...
            activerat,'" and date="',datestr(now,'yyyy-mm-dd'),'"']);
        if ~isempty(temp)
            temp = sum(temp);
        else
        ...
        dropsremaining(i) = ceil((((P/100) * ratmass(i)) - (waterearned(i)+temp)) / (value(WaterDropVol) / 1000));
        ...
        if (numel(ndt(ndt~=0)) > 7 && max(ndt) > 50 && exclude == 0 && ~isempty(rg)) ||...
           (value(IsRatRig) == 0 && exclude == 0 && numel(m) >= 3)
    ...
    WaterEarnedHere.value = waterearnedhere;
...
%% update_bars
  case 'update_bars'
      ...
      if ~isempty(activerat) && ~strcmp(activerat,'BROKEN')
          ...
          temp = sbp(1) + ((( value(MinWaterPerent)/100) / (P/100)) * sbp(3)) - 1;
...
%% wait_for_training_end
  case 'wait_for_training_end'
    ...
    %delayed.  Let's just pass through
    ...
    done = zeros(1,value(SnugCount)); done(:) = nan;
    for i=1:value(SnugCount)
        activerat = get(get_ghandle(eval(['Rat',num2str(i)])),'string');
        if ~isempty(activerat) && strcmp(activerat,'BROKEN') == 0 
            done(i) = 0; 
...
%% update    
  case 'update'
      ...
      for i = 1:value(SnugCount)
          ...
          if strcmp(temp,'Start Manual Test')
              states = fields(parsed_events.states);
              if sum(strcmp(states,'earlypause')) > 0 && ~isempty(parsed_events.pokes.X)
                  set(get_ghandle(eval(['Status',num2str(i)])),'String','Manual Test Running');
              end
          elseif strcmp(temp,'Manual Test Running')
              states = fields(parsed_events.states);
              if sum(strcmp(states,'earlypause')) > 0 && ~isempty(parsed_events.states.trialend)
                  set(get_ghandle(eval(['Status',num2str(i)])),'String','Manual Test Stopping');
              end
          end
          
          if ~contains(temp,'COMPLETE') && ~isempty(temp)
...
%% set_slot        
    case 'set_slot'
        ...
        %Reset snug i to complete=0. get_rat_data will set back to 1 if rat
...
%% session1
  case 'Session1_callback'
      Pub4(obj,'update_session',1);  
        
%% session1
  case 'Session2_callback'
      Pub4(obj,'update_session',2);  
        
%% session1
  case 'Session3_callback'
      Pub4(obj,'update_session',3);  
        
%% session1
  case 'Session4_callback'
      Pub4(obj,'update_session',4);  
        
%% session1
  case 'Session5_callback'
      Pub4(obj,'update_session',5);  
        
%% session1
  case 'Session6_callback'
      Pub4(obj,'update_session',6);
        
%% session1
  case 'Session7_callback'
      Pub4(obj,'update_session',7);  
        
%% session1
  case 'Session8_callback'
      Pub4(obj,'update_session',8);  
        
%% session1
  case 'Session9_callback'
      Pub4(obj,'update_session',9);  
        
%% RedButton
  case 'RedButton_callback'
      set(get_ghandle(RedButton),'BackgroundColor',[1 1 0])
      pause(0.1);
      ManualOverride('IP', value(RedButtonPort));
      pause(0.1);
      ManualOverride('IP', value(RedButtonPort));
      pause(0.1);
      set(get_ghandle(RedButton),'BackgroundColor',[1 0 0])
      
      curr_string = get(get_ghandle(RedButton),'string');
      if strcmp(curr_string,'Click to Load Pub')
          newstring = 'Start Manual Test';
      elseif strcmp(curr_string,'Start Manual Test')
          newstring = 'Stop Manual Test';
      elseif strcmp(curr_string,'Stop Manual Test')
          newstring = 'Click 2x to Start';
      elseif strcmp(curr_string,'Click 2x to Start')
          newstring = 'Click 1x to Start';
      elseif strcmp(curr_string,'Click 1x to Start')
          newstring = 'Click 2x to Restart';   
      elseif strcmp(curr_string,'Click 2x to Restart')
          newstring = 'Click 1x to Restart';     
      else
          newstring = curr_string;
      end
      set(get_ghandle(RedButton),'string',newstring);
...
%% update_session
  ...
  activerats     = cell( 1,value(SnugCount));
  activewater    = zeros(1,value(SnugCount));
  Complete.value = zeros(1,value(SnugCount)); %#ok<STRNU> %need to reset this for new session
...
%% set_tech
  case 'set_tech'
    ...
    if ~isempty(initials{1})
        TechMenu.value = find(strcmp(value(AllExp),tech)==1,1,'first'); %#ok<STRNU>
    end
      
%% update_tech_callback
    case 'update_tech_callback'
        tech = value(TechMenu); %#ok<NODEF>
        initials = bdata(['select initials from ratinfo.contacts where experimenter="',tech,'"']);
        if isempty(initials); initials = {''}; end
        TechInitials.value = initials{1}; %#ok<STRNU>
        if get(get_ghandle(TechMenu),'value') == 1; set(get_ghandle(RedButton),'enable','off','string','Select Your Name');
        else,                                       set(get_ghandle(RedButton),'enable','on', 'string','Click to Load Pub');
        end
...
%% end_session    
  case 'end_session'
      ...
      %Let's log a note that the pub is being emptied
      note = ['Watering ended in Pub ',num2str(value(RigID)),' for session ',num2str(value(CurrSess)),' animals ']; %#ok<NODEF>
      ar = value(ActiveRats); %#ok<NODEF>
      for i = 1:numel(ar)
          if i < numel(ar)
              note = [note,ar{i},', ']; %#ok<AGROW>
          else
              note = [note,ar{i}]; %#ok<AGROW>
          end
      end
            
      bdata('insert ratinfo.technotes (datestr, timestr, techinitials, note) values ("{S}","{S}","{S}","{S}")',...
        datestr(now,'yyyy-mm-dd'),datestr(now,'HH:MM:SS'),value(TechInitials),note); %#ok<NODEF>
  ...
  case 'end_continued'
      ...
      if value(stopping_process_completed) %This is provided by dispatcher
          ...
          %This is where we should restart matlab with Pub4
          %running
          cd(bSettings('get','GENERAL','Rigscripts_Directory'));
          
          %Do a check on RatWaterList and see if no more animals need pub
          RWL = value(RatWaterList); %#ok<NODEF>
          comp = ones(numel(RWL)-1,1);
          for i = numel(value(CurrSess))+1:numel(value(RatWaterList))-1 %#ok<NODEF>
              rwl = RWL{i};
              for j = 1:numel(rwl)
                  id = bdata(['select watering from ratinfo.water where rat="',rwl{j},'" and date="',datestr(now,'yyyy-mm-dd'),'"']);
                  if isempty(id)
                      comp(i) = 0;
                  end
              end
          ...
          if value(CurrSess) >= 7 || all(comp == 0)
          ...
          elseif any(comp == 0)
              ...
              %all sessions have been watered in the pub. Kill matlab and
              %restart it but don't load pub protocol
              !auto_restart_matlab.bat
          end
      end
...
id = bdata(['select maintenance_id from ratinfo.rig_maintenance where rigid=',...
    num2str(rigids(rigpos)),' and isbroken=1']);
if ~isempty(id)
    bdata('call ratinfo.mark_rigfixed("{S}","{S}","{S}","{S}")',...
        id(end),' ',datestr(now,'yyyy-mm-dd HH:MM'),'snug fixed');
end
