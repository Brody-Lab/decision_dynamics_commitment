...
function [obj] = AcuteLaserControl(varargin)
...
%% INIT 
  case 'init'
    ...
    NumeditParam(obj, 'Laser_Duration',   5,  x, y,  'labelfraction', 0.8); next_row(y);
    NumeditParam(obj, 'Taper_Duration',   0,  x, y,  'labelfraction', 0.8); next_row(y);
    NumeditParam(obj, 'Cycle_Duration',   6,  x, y,  'labelfraction', 0.8); next_row(y);
    ...
    SoloParamHandle(obj,'LaserDigitalLine','value',bSettings('get','DIOLINES','stim1'));
    SoloParamHandle(obj,'NeuralynxLine',   'value',bSettings('get','DIOLINES','stim2'));
    SoloParamHandle(obj,'LaserAnalogLine', 'value',bSettings('get','DIOLINES','stim3'));
...
%% prepare_next_trial    
  ...
  case 'prepare_next_trial'
    ...
    sma = add_scheduled_wave(sma, 'name', 'laser_analog_wave',...
                                  'ao_line', 1,...
                                  'analog_waveform',AWF); 
                              
    sma = add_scheduled_wave(sma, 'name', 'laser_digital_wave',...
                                  'preamble',value(Cycle_Duration) - value(Laser_Duration),...
                                  'sustain', value(Laser_Duration),...
                                  'DOut',    value(LaserDigitalLine),...
                                  'loop',    value(Number_Replicates),...
                                  'trigger_on_up','laser_analog_wave');
    ...
    if rem(n_done_trials,2)==1                          
        sma = add_state(sma, 'name', 'laser_stim', 'self_timer', (value(Cycle_Duration) * value(Number_Replicates))+0.1,...
              'output_actions', {'SchedWaveTrig', '+laser_digital_wave+neuralynx_wave'},...
              'input_to_statechange', {'Tup', 'reset';'Cout','reset'});
          
        sma = add_state(sma,'name','reset','self_timer',0.01,...
              'output_actions', {'SchedWaveTrig', '-laser_digital_wave-neuralynx_wave-laser_analog_wave'},...
