...
obj = class(struct, mfilename, saveload, water, ...
    ...
    distribui, poissonbups2, comments, sqlsummary, AdLibGUI, cerebro2);
...
%% init
  case 'init'
    ...
    %   History of `reward_type`:
    SoloParamHandle(obj, 'reward_type_history',      'value', strings(0));
    DeclareGlobals(obj, 'ro_args', 'reward_type_history');

    %   Let RewardsSection, the part that parses what happened at the end of
    %   a trial, write to hit_history:
    SoloFunctionAddVars('RewardsSection', 'rw_args', {'hit_history'; 'violation_history'; 'cpoke1_tup_history'; 'reward_type_history'});
  ...
  case 'prepare_next_trial'
    ...
    elseif strcmp(varargin{1},'crashed')
        ...
        if value(n_prepped_trials) < value(n_started_trials)
            ...
            reward_type_history.value  = [reward_type_history(:)  ; ""];
            ...
            PBupsSection(obj,'push_history_stimulatorspecs');
