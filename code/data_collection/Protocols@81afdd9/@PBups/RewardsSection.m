...
switch action
  case 'init',
    ...
    MenuParam(obj, 'reward_type', {'gamma'; 'delta clicks';'free_choice';'random_reward'}, 1, x, y, ...
        'labelfraction', 0.4, ...
  ...
  case 'next_trial',
    if n_done_trials > 0,
        ...
        elseif isfield(parsed_events.states, 'cpoke1_tup_state') && rows(parsed_events.states.cpoke1_tup_state) > 0,
            hit_history.value = [hit_history(:) ; NaN];
        else
            hit_history.value = [hit_history(:) ; 0];
        end;
        
        if isfield(parsed_events.states, 'cpoke1_tup_state') && rows(parsed_events.states.cpoke1_tup_state) > 0,
            cpoke1_tup_history.value = [cpoke1_tup_history(:) ; 1];          
        else
            cpoke1_tup_history.value = [cpoke1_tup_history(:) ; 0];
        end
    
        reward_type_history.value = [reward_type_history(:); string(value(reward_type))];
