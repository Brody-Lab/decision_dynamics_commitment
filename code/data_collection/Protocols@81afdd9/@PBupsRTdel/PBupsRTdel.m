% PBupsRTdel protocol adapted from PBups protocol 
% here the idea is to delay the reward based on delta clicks
% DG, June 2018

function [obj] = PBupsRTdel(varargin)
...
obj = class(struct, mfilename, saveload, water, ...
    ...
    distribui, poissonbups5, comments, sqlsummary,AdLibGUI);
...
%% init
  case 'init'
      rossm = [];
      SoloParamHandle(obj,'Rossm','value',[]);
	getSessID(obj);
    ...
    % Additive offsets for left and right reward volumes in the RT water task
    NumeditParam(obj, 'left_wtr_offset', 0, x, y, ...
        'position', [x y 100 20], ...
        'labelfraction', 0.7, ...
        'TooltipString', 'all left reward volumes have this offset'); 
    NumeditParam(obj, 'right_wtr_offset', 0, x, y, ...
        'position', [x+100 y 100 20], ...
        'labelfraction', 0.7, ...
        'TooltipString', 'all right reward volumes have this offset');
    next_row(y, 1.5);
...
%     end
%     if n_done_trials > 1
%         if ~isempty(parsed_events_history{end}.states.check_next_trial_ready) && diff(parsed_events_history{end}.states.check_next_trial_ready) > 2
%             s = profile('info');
%             dr = [bSettings('get','GENERAL','Main_Data_Directory'),filesep,'PBupsRT_profile',filesep];
%             [expmtr, rname]=SavingSection(obj, 'get_info');
%             fl = [datestr(now,'yymmdd_HHMMSS'),'_',rname,'_',num2str(n_done_trials)];
%             save([dr,fl],'s');
%         end
%     end
%     profile clear 
    ...
    [sides, sounds] = SidesSection(obj,'get_previous_sides_and_sounds'); %#ok<NASGU>
    ...
    % set water volume for fixed delta clicks task
    if value(watermod_task) == 1
        delclicks = PBupsSection(obj, 'get_delclicks');
        WaterValvesSection(obj, 'set_water_amounts', 24*tanh(0.06*delclicks), 24*tanh(0.06*delclicks));
    end
    ...
    if n_done_trials==1
        [expmtr, rname]=SavingSection(obj, 'get_info');
        prot_title.value=['PBupsRT - on rig ' get_hostname ' : ' expmtr ', ' rname  '.  Started at ' datestr(now, 'HH:MM')];
    end
