...
%% init    
  case 'init',
      ...
      NumeditParam(obj, 'C2SGap_2', 0, x, y, 'position', [x+100 y 100 20], ...
          'labelfraction', 0.7, ...
          'TooltipString', sprintf(['An additional center-to-side gap that follows C2SGap where there is no punishment for poking anywhere, \n' ...
                            'this has to be set to zero in a RT task']));
      ...
      ToggleParam(obj, 'watermod_task', 0, x, y, ...
            'OffString', 'Non-Water modulation task', ...
            'OnString',  'Water Modulation task', ...
            'TooltipString', sprintf(['\n' ...
            'If water modulation task, then reward is delivered based on the delta clicks in that particular trial.\n']));
      next_row(y);
    ...
    % Lets StateMatrixSectio have read access to the task type  
    SoloFunctionAddVars('StateMatrixSection', 'ro_args', ...
          {'legal_cbreak'; 'nic_end_sound' ; ...
          'nose_in_center'; 'rt_task'; 'watermod_task' ; ...
      ...
      %   Let PBupsRTdel Section have read access to the task type
      SoloFunctionAddVars('PBupsRTdel', 'ro_args', {'watermod_task'});
      
      %   Let SidesSection have read access to the task type
      SoloFunctionAddVars('SidesSection', 'ro_args', {'rt_task'; 'min_rt'; 'watermod_task'});
